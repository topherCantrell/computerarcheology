...
from computerarcheology.ascode.block_line import Block
from computerarcheology.ascode.directive_line import Directive
from computerarcheology.ascode.header_line import HeaderLine
from computerarcheology.ascode.list_line import ListLine
import computerarcheology.ascode.markdown_line
import computerarcheology.ascode.markdown_utils
from computerarcheology.ascode.memory_table import MemoryTable
from computerarcheology.ascode.paragraph_line import Paragraph
from computerarcheology.ascode.table_line import Table
import computerarcheology.web.ENVIRONMENT as ENV
from computerarcheology.web.id_mgr import IDMgr
from computerarcheology.web.nav_tree import NavTree
import computerarcheology.web.nav_tree
...
def process_markdown(lines, site_nav_node, fp_content):
        ...
        if type(md) is Directive:
            ...
            if md.directive.startswith('cpu'):
                ...
                if name == '6809':
                    ...
                    code_info['cpu'] = opcodetools.cpu.cpu_6809.CPU_6809()
                elif name == '6502':
                    import opcodetools.cpu.cpu_6502
                    code_info['cpu'] = opcodetools.cpu.cpu_6502.CPU_6502()
                elif name == 'DVG':
                    import opcodetools.cpu.cpu_DVG
                    code_info['cpu'] = opcodetools.cpu.cpu_DVG.CPU_DVG()
                elif name == 'Z80':
                    import opcodetools.cpu.cpu_Z80
                    code_info['cpu'] = opcodetools.cpu.cpu_Z80.CPU_Z80()
                elif name == '6803':
                    import opcodetools.cpu.cpu_6803
                    code_info['cpu'] = opcodetools.cpu.cpu_6803.CPU_6803()
                elif name == 'Z80GB':
                    import opcodetools.cpu.cpu_Z80GB
                    code_info['cpu'] = opcodetools.cpu.cpu_Z80GB.CPU_Z80GB()
                else:
            ...
            if md.directive.startswith('binary '):
                continue
    ...
    cr[0] = computerarcheology.web.nav_tree.NavNode(None, 1, 'Home', '', None)
    crumbs = ''
    for n in cr:
        tt = n.text
        if tt.startswith('*'):
    ...
    # Collapse everything
    computerarcheology.web.nav_tree.collapse_all(cpy)
    ...
    ret['SITE_TREE'] = computerarcheology.web.nav_tree.to_html(cpy)
    ...
    for dep in current_node.children:
        ...
        if os.path.isdir(src):
            ...
            if da.startswith('#') or src.endswith('-'):
                ...
                md, _, _ = computerarcheology.ascode.markdown_utils.load_file(src)
...
def load_site_directory(dev_mode=False):
    ...
    def _load_site_directory_rec(level, tree, current_node, dev_mode):
        ...
        lines, _, _ = computerarcheology.ascode.markdown_utils.load_file(
            os.path.join(src, 'README.md'))
        info = computerarcheology.ascode.markdown_utils.get_deploy(lines)
