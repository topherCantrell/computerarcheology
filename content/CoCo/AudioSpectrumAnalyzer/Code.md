...
C11C: BD C5 15        JSR     $C515               ; {code.DrawSplashScreen}
...
C163: B7 FF C2        STA     $FFC2               ; {hard.dispMode+2}
C166: B7 FF C5        STA     $FFC5               ; {hard.dispMode+5}
...
C2CE: 25 F9           BCS     $C2C9               ; {}
C2D0: 03 F0           COM     <$F0                
C2D2: B7 FF C4        STA     $FFC4               ; {hard.dispMode+4}
C2D5: B7 FF C3        STA     $FFC3               ; {hard.dispMode+3}
...
C509: 20 F7           BRA     $C502               ; {code.PrintString} Keep printing
C50B: 39              RTS                         ; Out
...
C512: 27 F8           BEQ     $C50C               ; {code.NextColorBlock} ... block
C514: 39              RTS                         ; Out
...
C522: B7 01 67        STA     $0167               ; {ram.m0167} ... to CONSOLE OUT vector
...
C530: BD C5 02        JSR     $C502               ; {code.PrintString} Print the splash text
...
C53D: 8D CD           BSR     $C50C               ; {code.NextColorBlock} Make next color
...
C54E: 8D BC           BSR     $C50C               ; {code.NextColorBlock} Next color block
...
C55B: 8D AF           BSR     $C50C               ; {code.NextColorBlock} Next block
...
C56C: 8D 9E           BSR     $C50C               ; {code.NextColorBlock} Next color block
