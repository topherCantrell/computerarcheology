		...
		if(command[x].charAt(0)=='#') {
			...
			for(var yy=0;yy<height;++yy) {
				for(var xx=0;xx<width;++xx) {
					...
					drawTileCommand(context,xo+xx*pixWidth*gridX+xx*gridPad,yo+yy*pixHeight*gridY+yy*gridPad,gridX,gridY,lastCommand);
				}
			}			
			--x;
			xo = xo + pixWidth*width*gridX;
		} else {			
			drawTileCommand(context,xo,yo,gridX,gridY,command[x]);
 ...
 */
function drawTileCommand(context,xo,yo,gridX,gridY,com) {
	...
	var pixelData = window[getTileDataFunction](tileAddress);
