...
>>> binary 8000:Zelda.nes[10010:14010]
...
907B: A9 06         LDA   #$06                ;
907D: 9D 2C 04      STA   $042C,X             ;
9080: 60            RTS                       ;
9081: A9 04         LDA   #$04                ;
9083: 8D 02 06      STA   $0602               ; 
9086: 60            RTS                       ;
9087: FF                              ;
9088: FF                              ;
9089: FF                              ;
...
91D0: 95 AC         STA   $AC,X               ;
91D2: 60            RTS                       ;
91D3: BD 2C 04      LDA   $042C,X             ;
91D6: D0 45         BNE   $921D               ; 
91D8: 20 D0 79      JSR   $79D0               ; 
91DB: BD 2C 04      LDA   $042C,X             ;
91DE: F0 08         BEQ   $91E8               ; 
91E0: A9 40         LDA   #$40                ;
91E2: 85 AC         STA   <$AC                ; 
91E4: A9 00         LDA   #$00                ;
...
944C: A9 10         LDA   #$10                ;
944E: 8D 01 06      STA   $0601               ; ??SND_601??
9451: A9 B0         LDA   #$B0                ;
9453: 95 70         STA   $70,X               ;
9455: A9 80         LDA   #$80                ;
9457: 95 84         STA   $84,X               ;
...
969A: 04                              ;
969B: D0 03         BNE   $96A0               ; 
969D: FE 2C 04      INC   $042C,X             ;
96A0: BD 1F 04      LDA   $041F,X             ;
96A3: DD 37 04      CMP   $0437,X             ;
96A6: D0 42         BNE   $96EA               ; 
96A8: BD 2C 04      LDA   $042C,X             ;
...
96BE: C9 FF         CMP   #$FF                ;
96C0: D0 03         BNE   $96C5               ; 
96C2: DE 2C 04      DEC   $042C,X             ;
96C5: BD 1F 04      LDA   $041F,X             ;
96C8: DD 37 04      CMP   $0437,X             ;
96CB: D0 1D         BNE   $96EA               ; 
96CD: BD 2C 04      LDA   $042C,X             ;
...
9938: FE 2C 04      INC   $042C,X             ;
993B: BD 2C 04      LDA   $042C,X             ;
993E: C9 02         CMP   #$02                ;
9940: 90 0C         BCC   $994E               ; 
...
9A3E: 85 08         STA   <$08                ; 
9A40: A0 01         LDY   #$01                ;
9A42: B9 00 00      LDA   $0000,Y             ;
9A45: 38            SEC                       ;
9A46: F9 02 00      SBC   $0002,Y             ;
9A49: C5 06         CMP   <$06                ; 
9A4B: 10 09         BPL   $9A56               ; 
9A4D: C5 07         CMP   <$07                ; 
9A4F: 30 05         BMI   $9A56               ; 
...
9D2A: FE 2C 04      INC   $042C,X             ;
9D2D: BD 2C 04      LDA   $042C,X             ;
9D30: C9 60         CMP   #$60                ;
9D32: 90 0A         BCC   $9D3E               ; 
9D34: A9 00         LDA   #$00                ;
9D36: 8D 76 06      STA   $0676               ; 
9D39: A9 C0         LDA   #$C0                ;
9D3B: 9D 2C 04      STA   $042C,X             ;
...
A19B: B9 2D 04      LDA   $042D,Y             ;
A19E: 69 00         ADC   #$00                ;
A1A0: 99 2D 04      STA   $042D,Y             ;
A1A3: 88            DEY                       ;
A1A4: 10 EC         BPL   $A192               ; 
A1A6: A9 00         LDA   #$00                ;
...
A380: BD 2C 04      LDA   $042C,X             ;
A383: D0 0C         BNE   $A391               ; 
A385: A9 80         LDA   #$80                ;
A387: 9D 44 04      STA   $0444,X             ;
A38A: A9 C0         LDA   #$C0                ;
A38C: 15 18         ORA   $18,X               ;
A38E: 9D 2C 04      STA   $042C,X             ;
A391: A5 15         LDA   <$15                ; 
A393: 4A            LSR   A                   ;
A394: 90 03         BCC   $A399               ; 
A396: DE 2C 04      DEC   $042C,X             ;
...
A464: D0 0B         BNE   $A471               ; 
A466: A9 02         LDA   #$02                ;
A468: 8D 01 06      STA   $0601               ; 
A46B: 20 54 7C      JSR   $7C54               ; 
A46E: 20 0D 80      JSR   $800D               ; 
A471: A9 01         LDA   #$01                ;
A473: 8D 04 06      STA   $0604               ; 
A476: 60            RTS                       ;
...
AD4C: 60            RTS                       ;
AD4D: FE 2C 04      INC   $042C,X             ;
AD50: BD 2C 04      LDA   $042C,X             ;
AD53: D0 05         BNE   $AD5A               ; 
AD55: A9 FF         LDA   #$FF                ;
AD57: 9D 2C 04      STA   $042C,X             ;
AD5A: C9 50         CMP   #$50                ;
AD5C: 90 EB         BCC   $AD49               ; 
AD5E: D0 1A         BNE   $AD7A               ; 
...
AD75: A9 02         LDA   #$02                ;
AD77: 8D 00 06      STA   $0600               ; 
AD7A: 20 9F AF      JSR   $AF9F               ; 
AD7D: BD 2C 04      LDA   $042C,X             ;
AD80: C9 A0         CMP   #$A0                ;
AD82: 90 67         BCC   $ADEB               ; 
AD84: D0 0E         BNE   $AD94               ; 
...
B00F: 60            RTS                       ;
B010: A9 02         LDA   #$02                ;
B012: 8D 01 06      STA   $0601               ; 
B015: A9 80         LDA   #$80                ;
B017: 8D 03 06      STA   $0603               ; 
B01A: 60            RTS                       ;
...
BF54: 8D 00 20      STA   $2000               ; -_P_CNTRL_1 ... truns off NMIs
