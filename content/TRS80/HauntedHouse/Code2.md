...
4363: CD 33 00        CALL    $0033               ; {hard.PrintChar} ... the screen
4366: 3E 01           LD      A,$01               ; Starting room is ...
4368: 32 2F 49        LD      ($492F),A           ; {ram.CurrentRoom} ... 1 (???)
436B: 21 69 4E        LD      HL,$4E69            ; YOU_DROP_EVERYTHING_YOU_HAD_TO_CLIMB_THE_ROPE.__YOU_REACH_THE___SECOND_FLOOR.[CR]
436E: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print message
4371: CD 7C 49        CALL    $497C               ; {code.DescribeRoom} Print room description
;
4374: 97              SUB     A,A                 ; Make a zero
4375: 32 80 46        LD      ($4680),A           ; {ram.InputNoun} Clear noun
4378: 32 81 46        LD      ($4681),A           ; {ram.InputVerb} Clear verb
437B: 32 82 46        LD      ($4682),A           ; {ram.GrammarType} Clear grammar
437E: CD 2F 44        CALL    $442F               ; {code.ParseUserInput} ParseUserInput
4381: 3A 2F 49        LD      A,($492F)           ; {ram.CurrentRoom} Current room
...
438E: 66              LD      H,(HL)              ; ... to ...
438F: 6F              LD      L,A                 ; ... HL
4390: CD D8 43        CALL    $43D8               ; {code.ProcessRoomScript} Process the room script
4393: C2 A5 43        JP      NZ,$43A5            ; {} ZF clear ... script was successful. Move on.
4396: 21 1A 4B        LD      HL,$4B1A            ; General script
4399: CD D8 43        CALL    $43D8               ; {code.ProcessRoomScript} Process the script
439C: C2 A5 43        JP      NZ,$43A5            ; {} ZF clear ... script was successful. Move on
439F: 21 F0 4D        LD      HL,$4DF0            ; "NO"
43A2: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print packed error message
;
...
43B6: C2 AB 43        JP      NZ,$43AB            ; {code.GetObjectInfo} Bit is set ... find where the parent is
...
43D1: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Find object
...
43E0: 3A 81 46        LD      A,($4681)           ; {ram.InputVerb} Verb word
...
43EC: C3 D8 43        JP      $43D8               ; {code.ProcessRoomScript} Back to process next command
;
43EF: CD F6 43        CALL    $43F6               ; {code.RunScript} Run the command
43F2: C0              RET     NZ                  ; Not zero if the command passed
43F3: C3 D8 43        JP      $43D8               ; {code.ProcessRoomScript} The command failed ... keep running the list of commands
```
...
4423: CD F6 43        CALL    $43F6               ; {code.RunScript} Run the list of commands
4426: E5              PUSH    HL                  ; Points passed the list just executed
4427: CA 0B 44        JP      Z,$440B             ; {code.ScriptCommandPASS} Subscript failed ... but keep going in current script
...
```code                     
ParseUserInput: 
442F: CD 0A 46        CALL    $460A               ; {code.PromptAndReadLine} Prompt and read line
4432: CD DA 44        CALL    $44DA               ; {code.ParseInputLine} Parse the input string
4435: 2A AF 45        LD      HL,($45AF)          ; {ram.NounData} Pointer to noun data
4438: 3A AD 45        LD      A,($45AD)           ; {ram.NounDataSize} Number of bytes ...
443B: 47              LD      B,A                 ; ... in noun data
443C: 3A 82 46        LD      A,($4682)           ; {ram.GrammarType} Grammar type
443F: FE 03           CP      $03                 ; Value 3 means nothing in buffer
4441: CA 2F 44        JP      Z,$442F             ; {code.ParseUserInput} Ignore blank lines
4444: 3A 80 46        LD      A,($4680)           ; {ram.InputNoun} Noun number
4447: A7              AND     A                   ; Was there a noun on the input line?
4448: C2 6A 44        JP      NZ,$446A            ; {} Yes ... check for verb-noun things
444B: 3A 81 46        LD      A,($4681)           ; {ram.InputVerb} Verb
...
4455: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print the error
4458: C3 2F 44        JP      $442F               ; {code.ParseUserInput} Back to the top
;
445B: 3A 82 46        LD      A,($4682)           ; {ram.GrammarType} Grammar type
...
4464: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print error (verb expected noun)
4467: C3 2F 44        JP      $442F               ; {code.ParseUserInput} Back to top of input loop
;
; We have a verb and a noun
446A: 22 AF 45        LD      ($45AF),HL          ; {ram.NounData} Pointer into noun data
446D: 3A D9 44        LD      A,($44D9)           ; {ram.LoneObject} Did we have ...
4470: A7              AND     A                   ; ... lone noun last parse?
4471: C2 C6 44        JP      NZ,$44C6            ; {} Yes ... use that now
;
4474: 7E              LD      A,(HL)              ; Next word-for-object value
4475: 23              INC     HL                  ; Bump ...
4476: 22 AF 45        LD      ($45AF),HL          ; {ram.NounData} ... word-data pointer
4479: 1E FF           LD      E,$FF               ; Look in backpack first
447B: C5              PUSH    BC                  ; Hold
447C: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Find object
447F: C1              POP     BC                  ; Restore
4480: CA BE 44        JP      Z,$44BE             ; {} Found object. Use it.
4483: 3A 82 46        LD      A,($4682)           ; {ram.GrammarType} Grammar type
...
4495: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Do find
4498: C1              POP     BC                  ; Restore
4499: CA BE 44        JP      Z,$44BE             ; {} Object is in room. Use it.
;
449C: 2A AF 45        LD      HL,($45AF)          ; {ram.NounData} Noun data
449F: 05              DEC     B                   ; All tried?
44A0: C2 74 44        JP      NZ,$4474            ; {} No ... try next word
44A3: 3A 82 46        LD      A,($4682)           ; {ram.GrammarType} Grammar type
...
44DE: 32 AE 45        LD      ($45AE),A           ; {ram.DecodeEmpty} Nothing in buffer to start with
44E1: 32 82 46        LD      ($4682),A           ; {ram.GrammarType} Grammar type
;
...
;
44F6: 22 8A 46        LD      ($468A),HL          ; {ram.PtrHold} Hold on to pointer (never read)
;
44F9: A7              AND     A                   ; Reached the end ...
44FA: CA 84 45        JP      Z,$4584             ; {} ... of the line
44FD: 3E 01           LD      A,$01               ; Flag that something ...
44FF: 32 AE 45        LD      ($45AE),A           ; {ram.DecodeEmpty} ... is in the buffer
...
4508: 32 90 46        LD      ($4690),A           ; {ram.Stack+4} Store word data
450B: E6 07           AND     $07                 ; Size of text ...
450D: 4F              LD      C,A                 ; ... to C
450E: 32 8C 46        LD      ($468C),A           ; {ram.Stack} Text count
4511: 3A 90 46        LD      A,($4690)           ; {ram.Stack+4} Word data again
...
452B: 3A 8C 46        LD      A,($468C)           ; {ram.Stack} Text count
...
; Word matched
454B: 3A 90 46        LD      A,($4690)           ; {ram.Stack+4} Word data
454E: E6 C0           AND     $C0                 ; Top two bits ... word type
4550: CA 5D 45        JP      Z,$455D             ; {} Word is a noun
4553: 32 82 46        LD      ($4682),A           ; {ram.GrammarType} Store grammar type
4556: 1A              LD      A,(DE)              ; Get word number
4557: 32 81 46        LD      ($4681),A           ; {ram.InputVerb} Store verb
455A: C3 6E 45        JP      $456E               ; {} Check for more to parse
;Word is a noun
455D: 1A              LD      A,(DE)              ; Noun number from the word's data
455E: 32 80 46        LD      ($4680),A           ; {ram.InputNoun} Hold word number
4561: EB              EX      DE,HL               ; Store ...
4562: 22 AF 45        LD      ($45AF),HL          ; {ram.NounData} ... pointer to noun data
4565: EB              EX      DE,HL               ; Restore pointers
4566: 78              LD      A,B                 ; Number of data bytes
4567: 32 AD 45        LD      ($45AD),A           ; {ram.NounDataSize} Hold this for later
456A: 97              SUB     A                   ; No longer remember ...
456B: 32 D9 44        LD      ($44D9),A           ; {ram.LoneObject} ... a past lone object
...
; End of input
4584: 3A AE 45        LD      A,($45AE)           ; {ram.DecodeEmpty} Something in ...
4587: A7              AND     A                   ; ... the buffer?
4588: C0              RET     NZ                  ; Yes ... done
4589: 3E 03           LD      A,$03               ; Grammar type 3 means ...
458B: 32 82 46        LD      ($4682),A           ; {ram.GrammarType} ... nothing in buffer
458E: C9              RET                         ; Done
...
4590: 97              SUB     A                   ; Clear ...
4591: 32 81 46        LD      ($4681),A           ; {ram.InputVerb} ... verb ...
4594: 32 80 46        LD      ($4680),A           ; {ram.InputNoun} ... and noun
...
45BA: CD BC 46        CALL    $46BC               ; {code.UnpackMessage} Unpack and print
...
45C7: CD 04 46        CALL    $4604               ; {code.PrintChar} Print the character
...
45E4: CD 04 46        CALL    $4604               ; {code.PrintChar} Print character
45E7: E1              POP     HL                  ; Increment ...
45E8: 23              INC     HL                  ; ... message pointer
45E9: C3 D5 45        JP      $45D5               ; {code.PrintMessage} Next character in message
;           
...
45F4: 3A 83 46        LD      A,($4683)           ; {ram.InputEntroy} Bump ...
45F7: 3C              INC     A                   ; ... some ...
45F8: 32 83 46        LD      ($4683),A           ; {ram.InputEntroy} ... ??? counter
45FB: CD 2B 00        CALL    $002B               ; {hard.ScanKeyboard} Get a key
...
4605: CD 33 00        CALL    $0033               ; {hard.PrintChar} Print the character
...
460D: CD 04 46        CALL    $4604               ; {code.PrintChar} Print the prompt
...
4618: CD F3 45        CALL    $45F3               ; {code.WaitForKey} Wait for a key
...
4625: CD 04 46        CALL    $4604               ; {code.PrintChar} Send the character to the screen
...
463F: CD 04 46        CALL    $4604               ; {code.PrintChar} ... off the screen
...
4655: 47              LD      B,A                 ; ... to ...
4656: CD 04 46        CALL    $4604               ; {code.PrintChar} ... screen
...
4973: 32 2F 49        LD      ($492F),A           ; {ram.CurrentRoom} Change rooms
```
...
4976: CD 7C 49        CALL    $497C               ; {code.DescribeRoom} Print the room description
4979: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS
```
...
497F: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print the message
4982: 3A 2F 49        LD      A,($492F)           ; {ram.CurrentRoom} Room number
...
498D: 66              LD      H,(HL)              ; ... and ...
498E: 6F              LD      L,A                 ; ... MSB
498F: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print the description
4992: 06 00           LD      B,$00               ; B runs the object numbers
4994: 04              INC     B                   ; Next object (1 on first pass)
4995: 3A 2F 49        LD      A,($492F)           ; {ram.CurrentRoom} Room number
...
499D: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Get object if in this room
...
49AF: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print description
...
49BC: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Get object info
49BF: CA 0B 44        JP      Z,$440B             ; {code.ScriptCommandPASS} ScriptCommandPASS
49C2: C3 1E 44        JP      $441E               ; {code.ScriptCommandFAIL} ScriptCommandFAIL
```
...
49C9: 3A 2F 49        LD      A,($492F)           ; {ram.CurrentRoom} Current room ...
49CC: 5F              LD      E,A                 ; ... to E for find
49CD: 78              LD      A,B                 ; Object number for find
49CE: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Look for the object in current room
49D1: CA 0B 44        JP      Z,$440B             ; {code.ScriptCommandPASS} Found it ... ScriptCommandPASS
...
49D9: 3A 2F 49        LD      A,($492F)           ; {ram.CurrentRoom} Room number ...
49DC: 5F              LD      E,A                 ; ... to E for find
49DD: 7E              LD      A,(HL)              ; Get object number
49DE: 23              INC     HL                  ; New script ...
49DF: E5              PUSH    HL                  ; ... pointer
49E0: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Find the object
49E3: CA 0B 44        JP      Z,$440B             ; {code.ScriptCommandPASS} Found it in room ... ScriptCommandPASS
49E6: C3 1E 44        JP      $441E               ; {code.ScriptCommandFAIL} ScriptCommandFAIL
```
...
ScriptCommand14: 
; GetNounToPack()
49E9: 3A 80 46        LD      A,($4680)           ; {ram.InputNoun} Input Noun
49EC: 1E FF           LD      E,$FF               ; Is object already ...
49EE: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} ... in backpack?
49F1: C2 F7 49        JP      NZ,$49F7            ; {} No ... do the get
49F4: C3 79 4A        JP      $4A79               ; {code.ScriptCommand12} Print OK and PASS
;
49F7: 3A 80 46        LD      A,($4680)           ; {ram.InputNoun} User input ...
49FA: 47              LD      B,A                 ; ... noun
...
4A05: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Unpack and print
4A08: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS
```
...
;
4A0F: 3A 2F 49        LD      A,($492F)           ; {ram.CurrentRoom} Current Room number
4A12: 5F              LD      E,A                 ; To E
4A13: 78              LD      A,B                 ; Object number to A
4A14: CD D1 43        CALL    $43D1               ; {} Move object
4A17: C3 79 4A        JP      $4A79               ; {code.ScriptCommand12} Print OK and PASS
```
...
4A22: D2 0B 44        JP      NC,$440B            ; {code.ScriptCommandPASS} Yes ... ScriptCommandPASS
4A25: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Find the object
...
4A3D: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print the description
...
4A48: 3A 80 46        LD      A,($4680)           ; {ram.InputNoun} Input noun
4A4B: B8              CP      B                   ; Are they the same
4A4C: C2 1E 44        JP      NZ,$441E            ; {code.ScriptCommandFAIL} No ... ScriptCommandFAIL
4A4F: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} Yes ... ScriptCommandPASS
```
...
;
4A57: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Find the object
...
4A63: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print error message
4A66: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS (was handled here)
;
4A69: 78              LD      A,B                 ; Object number
4A6A: 1E FF           LD      E,$FF               ; Location is backpack
4A6C: CD D1 43        CALL    $43D1               ; {} Move the object
4A6F: C3 79 4A        JP      $4A79               ; {code.ScriptCommand12} Print OK and PASS
```
...
4A72: 3A 80 46        LD      A,($4680)           ; {ram.InputNoun} User input
4A75: 47              LD      B,A                 ; To B
...
4A7C: CD B3 45        CALL    $45B3               ; {code.PrintPacked} Print OK
4A7F: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS
```
...
4A94: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS
```
...
ScriptCommand07: 
; EndlessLoop()
4A97: C3 97 4A        JP      $4A97               ; {code.ScriptCommand07} Infinite loop
```
