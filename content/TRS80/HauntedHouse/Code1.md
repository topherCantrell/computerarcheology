...
4381: CD 2F 44        CALL    $442F               ; {code.ParseUserInput} Get user input line and parse
4384: 3A E2 48        LD      A,($48E2)           ; {ram.CurrentRoom} Room number
...
4391: 66              LD      H,(HL)              ; ... to ...
4392: 6F              LD      L,A                 ; ... HL
4393: CD D8 43        CALL    $43D8               ; {code.ProcessRoomScript} Process the room script
4396: C2 A8 43        JP      NZ,$43A8            ; {} ZF clear ... script was successful. Move on.
4399: 21 F2 4A        LD      HL,$4AF2            ; General script
439C: CD D8 43        CALL    $43D8               ; {code.ProcessRoomScript} Process the script
...
```code          
ParseUserInput: 
442F: CD 05 46        CALL    $4605               ; {code.PromptAndReadLine} Prompt and read line
4432: CD DA 44        CALL    $44DA               ; {code.ParseInputLine} Parse the input string
4435: 2A AC 45        LD      HL,($45AC)          ; {ram.NounData} Pointer to noun data
4438: 3A AA 45        LD      A,($45AA)           ; {ram.NounDataSize} Number of bytes ...
443B: 47              LD      B,A                 ; ... in noun data
443C: 3A 7D 46        LD      A,($467D)           ; {ram.GrammarType} Grammar type
443F: FE 03           CP      $03                 ; Value 3 means nothing in buffer
4441: CA 2F 44        JP      Z,$442F             ; {code.ParseUserInput} Ignore blank lines
4444: 3A 7B 46        LD      A,($467B)           ; {ram.InputNoun} Noun number
4447: A7              AND     A                   ; Was there a noun on the input line?
4448: C2 6A 44        JP      NZ,$446A            ; {} Yes ... check for verb-noun things
444B: 3A 7C 46        LD      A,($467C)           ; {ram.InputVerb} Verb
...
;
445B: 3A 7D 46        LD      A,($467D)           ; {ram.GrammarType} Grammar type
...
446A: 22 AC 45        LD      ($45AC),HL          ; {ram.NounData} Pointer into noun data
...
4475: 23              INC     HL                  ; Bump ...
4476: 22 AC 45        LD      ($45AC),HL          ; {ram.NounData} ... word-data pointer
...
4483: 3A 7D 46        LD      A,($467D)           ; {ram.GrammarType} Grammar type
4486: FE 40           CP      $40                 ; 01 ... noun must be in pack
4488: CA 9C 44        JP      Z,$449C             ; {} Validate noun
448B: 2A AC 45        LD      HL,($45AC)          ; {ram.NounData} Pointer to noun data
448E: 2B              DEC     HL                  ; 
448F: 3A E2 48        LD      A,($48E2)           ; {ram.CurrentRoom} Current room
...
;
449C: 2A AC 45        LD      HL,($45AC)          ; {ram.NounData} Noun data
...
;
44BE: 2A AC 45        LD      HL,($45AC)          ; {ram.NounData} Noun data
44C1: 2B              DEC     HL                  ; Back to word pointer
44C2: 7E              LD      A,(HL)              ; Get the for-object value
44C3: 32 7B 46        LD      ($467B),A           ; {ram.InputNoun} Hold the value
;
...
44E8: 22 7F 46        LD      ($467F),HL          ; {ram.NextWord} Pointer to next word to check
...
44FC: 32 AB 45        LD      ($45AB),A           ; {ram.DecodeEmpty} ... is in the buffer
...
4505: 32 82 46        LD      ($4682),A           ; {ram.CurrentWord} Store word data
4508: E6 07           AND     $07                 ; Size of text ...
450A: 4F              LD      C,A                 ; ... to C
450B: 32 81 46        LD      ($4681),A           ; {ram.WordSize} Text count
450E: 3A 82 46        LD      A,($4682)           ; {ram.CurrentWord} Word data again
...
4518: 22 7F 46        LD      ($467F),HL          ; {ram.NextWord} ... command table ...
...
4528: 3A 81 46        LD      A,($4681)           ; {ram.WordSize} Text count
...
; Word matched
4548: 3A 82 46        LD      A,($4682)           ; {ram.CurrentWord} Word data
...
455B: 32 7B 46        LD      ($467B),A           ; {ram.InputNoun} Hold word number
455E: EB              EX      DE,HL               ; Store ...
455F: 22 AC 45        LD      ($45AC),HL          ; {ram.NounData} ... pointer to noun data
...
45B5: CD B9 46        CALL    $46B9               ; {code.UnpackMessage} Unpack and print
...
45C2: CD FF 45        CALL    $45FF               ; {code.PrintChar} Print the character
45C5: E1              POP     HL                  ; Restore HL
45C6: 7E              LD      A,(HL)              ; Get next byte from unpacked
45C7: FE 0A           CP      $0A                 ; Mark for another packing?
45C9: 23              INC     HL                  ; 
45CA: CA AE 45        JP      Z,$45AE             ; {code.PrintPacked} Yes ... start again
...
45DF: CD FF 45        CALL    $45FF               ; {code.PrintChar} Print character
45E2: E1              POP     HL                  ; Increment ...
45E3: 23              INC     HL                  ; ... message pointer
45E4: C3 D0 45        JP      $45D0               ; {code.PrintMessage} Next character in message
;           
45E7: 06 0D           LD      B,$0D               ; CR character
45E9: 78              LD      A,B                 ; To A
45EA: CD FF 45        CALL    $45FF               ; {code.PrintChar} Print character
45ED: C9              RET                         
```
...
4600: CD 33 00        CALL    $0033               ; {hard.PrintChar} Print the character
...
4608: CD FF 45        CALL    $45FF               ; {code.PrintChar} Print the prompt
...
4613: CD EE 45        CALL    $45EE               ; {code.WaitForKey} Wait for a key
...
4620: CD FF 45        CALL    $45FF               ; {code.PrintChar} Send the character to the screen
...
463A: CD FF 45        CALL    $45FF               ; {code.PrintChar} ... off the screen
...
46B9: 32 7B 47        LD      ($477B),A           ; {ram.Unpack1}
46BC: 3E 01           LD      A,$01               ; 
...
46DE: 22 7C 47        LD      ($477C),HL          ; {ram.Unpack2}
...
46FA: 44              LD      B,H                 ; 
46FB: 85              ADD     A,L                 ; 
46FC: 2A 7C 47        LD      HL,($477C)          ; {ram.Unpack2}
46FF: 95              SUB     A,L                 ; 
4700: 4F              LD      C,A                 ; 
4701: 78              LD      A,B                 ; 
4702: 9C              SBC     A,H                 ; 
...
471F: 3A 7E 47        LD      A,($477E)           ; {ram.Unpack4}
4722: A7              AND     A                   ; 
...
472D: E5              PUSH    HL                  ; 
472E: 78              LD      A,B                 ; 
472F: CD FF 45        CALL    $45FF               ; {code.PrintChar} Print character
4732: E1              POP     HL                  ; 
4733: 23              INC     HL                  ; 
4734: 1D              DEC     E                   ; 
...
473B: EB              EX      DE,HL               ; 
473C: 13              INC     DE                  ; 
473D: 3A 7E 47        LD      A,($477E)           ; {ram.Unpack4}
4740: A7              AND     A                   ; 
4741: C2 47 47        JP      NZ,$4747            ; {}
4744: 13              INC     DE                  ; 
4745: 13              INC     DE                  ; 
4746: 13              INC     DE                  ; 
4747: 3A 7B 47        LD      A,($477B)           ; {ram.Unpack1}
474A: 3D              DEC     A                   ; 
474B: 32 7B 47        LD      ($477B),A           ; {ram.Unpack1}
...
4924: 32 E2 48        LD      ($48E2),A           ; {ram.CurrentRoom} Change rooms
```
...
4927: CD 2D 49        CALL    $492D               ; {code.DescribeRoom} Print the room description
492A: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS
```
...
4930: CD AE 45        CALL    $45AE               ; {code.PrintPacked} Print the message
4933: 3A E2 48        LD      A,($48E2)           ; {ram.CurrentRoom} Room number
...
493E: 66              LD      H,(HL)              ; ... and ...
493F: 6F              LD      L,A                 ; ... MSB
4940: CD AE 45        CALL    $45AE               ; {code.PrintPacked} Print the description
4943: 06 00           LD      B,$00               ; B runs the object numbers
4945: 04              INC     B                   ; Next object (1 on first pass)
4946: 3A E2 48        LD      A,($48E2)           ; {ram.CurrentRoom} Room number
...
494E: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Get object if in this room
...
4960: CD AE 45        CALL    $45AE               ; {code.PrintPacked} Print description
...
496D: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Get object info
4970: CA 0B 44        JP      Z,$440B             ; {code.ScriptCommandPASS} ScriptCommandPASS
4973: C3 1E 44        JP      $441E               ; {code.ScriptCommandFAIL} ScriptCommandFAIL
```
...
497A: 3A E2 48        LD      A,($48E2)           ; {ram.CurrentRoom} Current room ...
497D: 5F              LD      E,A                 ; ... to E for find
497E: 78              LD      A,B                 ; Object number for find
497F: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Look for the object in current room
4982: CA 0B 44        JP      Z,$440B             ; {code.ScriptCommandPASS} Found it ... ScriptCommandPASS
...
498A: 3A E2 48        LD      A,($48E2)           ; {ram.CurrentRoom} Room number ...
498D: 5F              LD      E,A                 ; ... to E for find
498E: 7E              LD      A,(HL)              ; Get object number
498F: 23              INC     HL                  ; New script ...
4990: E5              PUSH    HL                  ; ... pointer
4991: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Find the object
4994: CA 0B 44        JP      Z,$440B             ; {code.ScriptCommandPASS} Found it in room ... ScriptCommandPASS
4997: C3 1E 44        JP      $441E               ; {code.ScriptCommandFAIL} ScriptCommandFAIL
```
...
ScriptCommand14: 
; GetNounToPack()
499A: 3A 7B 46        LD      A,($467B)           ; {ram.InputNoun} Input Noun
499D: 1E FF           LD      E,$FF               ; Is object already ...
499F: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} ... in backpack?
49A2: C2 A8 49        JP      NZ,$49A8            ; {} No ... do the get
49A5: C3 2A 4A        JP      $4A2A               ; {code.ScriptCommand12} Print OK and PASS
;
49A8: 3A 7B 46        LD      A,($467B)           ; {ram.InputNoun} User input ...
49AB: 47              LD      B,A                 ; ... noun
...
49B6: CD AE 45        CALL    $45AE               ; {code.PrintPacked} Unpack and print
49B9: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS
```
...
;
49C0: 3A E2 48        LD      A,($48E2)           ; {ram.CurrentRoom} Current Room number
49C3: 5F              LD      E,A                 ; To E
49C4: 78              LD      A,B                 ; Object number to A
49C5: CD D1 43        CALL    $43D1               ; {} Move object
49C8: C3 2A 4A        JP      $4A2A               ; {code.ScriptCommand12} Print OK and PASS
```
...
49D3: D2 0B 44        JP      NC,$440B            ; {code.ScriptCommandPASS} Yes ... ScriptCommandPASS
49D6: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Find the object
...
49EE: CD AE 45        CALL    $45AE               ; {code.PrintPacked} Print the description
...
49F9: 3A 7B 46        LD      A,($467B)           ; {ram.InputNoun} Input noun
49FC: B8              CP      B                   ; Are they the same
49FD: C2 1E 44        JP      NZ,$441E            ; {code.ScriptCommandFAIL} No ... ScriptCommandFAIL
4A00: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} Yes ... ScriptCommandPASS
```
...
;
4A08: CD AB 43        CALL    $43AB               ; {code.GetObjectInfo} Find the object
...
4A14: CD AE 45        CALL    $45AE               ; {code.PrintPacked} Print error message
4A17: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS (was handled here)
;
4A1A: 78              LD      A,B                 ; Object number
4A1B: 1E FF           LD      E,$FF               ; Location is backpack
4A1D: CD D1 43        CALL    $43D1               ; {} Move the object
4A20: C3 2A 4A        JP      $4A2A               ; {code.ScriptCommand12} Print OK and PASS
```
...
4A23: 3A 7B 46        LD      A,($467B)           ; {ram.InputNoun} User input
4A26: 47              LD      B,A                 ; To B
...
4A2D: CD AE 45        CALL    $45AE               ; {code.PrintPacked} Print OK
4A30: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS
```
...
4A45: C3 0B 44        JP      $440B               ; {code.ScriptCommandPASS} ScriptCommandPASS
```
...
ScriptCommand07: 
; EndlessLoop()
4A48: C3 48 4A        JP      $4A48               ; {code.ScriptCommand07} Infinite loop
```
