...
000B: D3 40           OUT     ($40),A             ; {-2_AY_WATA} ... value to the AY register
000D: C9              RET                         ; Done
000E: FF FF  

0010: C3 B7 02        JP      $02B7               ; {ReadAmplitude} Read voice amplitude
0013: FF FF FF FF FF                 

0018: C3 7C 02        JP      $027C               ; {SetAmplitude} Set voice amplitude
001B: FF FF FF FF FF                 

0020: C3 C7 02        JP      $02C7               ; {Filter00} Remove all voice capacitor filters
0023: FF FF FF FF FF               

0028: C3 3C 02        JP      $023C               ; {WriteTune} Write to voice tune registers (coarse/fine)
002B: FF FF FF FF FF                 

0030: C3 60 02        JP      $0260               ; {EnableTone} Enable tone
...
0040: CD C1 02        CALL    $02C1               ; {ReadAY} ... AY IO port A
0043: B7              OR      A                   ; Reset everything?
0044: 28 2B           JR      Z,$71               ; {ClearCommands} Yes ... go reset all command info and out
...
0058: 18 3D           JR      $97                 ; {CmdRequest} Process original command as-is
;
005A: 7A              LD      A,D                 ; Original command
005B: E6 0F           AND     $0F                 ; Just the lower bits
005D: 20 38           JR      NZ,$97              ; {CmdRequest} Lower 4 is not 0 ... processes lower 4 as command
...
0067: 28 2E           JR      Z,$97               ; {CmdRequest} No ... use this as command
0069: CB BF           RES     7,A                 ; Reset the bit
006B: 18 13           JR      $80                 ; {StopCommand} Stop the requested command
...
StopCommand: 
0080: CD E6 00        CALL    $00E6               ; {ReinitCommand} Attempt to reinit running command
0083: D0              RET     NC                  ; Return if not there
0084: CD 8C 00        CALL    $008C               ; {FindDat} Find command data
0087: AF              XOR     A                   ; Zero
...
; request is ignored.
0097: 32 46 40        LD      ($4046),A           ; {-1_cmdRequest} Hold requested command
009A: CD E6 00        CALL    $00E6               ; {ReinitCommand} Reinit command if running
009D: D8              RET     C                   ; Command now running ... out
009E: CD E6 00        CALL    $00E6               ; {ReinitCommand} If not found, A=0. So look for a idle voice
00A1: 38 38           JR      C,$DB               ; Found idle voice ... use it
00A3: 3A 40 40        LD      A,($4040)           ; {-1_v1Command} Voice 1 command
00A6: CD 02 01        CALL    $0102               ; {GetComPriority} Priority of current ...
00A9: 47              LD      B,A                 ; ... voice 1 command to B
00AA: 3A 42 40        LD      A,($4042)           ; {-1_v2Command} Priority of  ...
00AD: CD 02 01        CALL    $0102               ; {GetComPriority} ... current ...
00B0: 4F              LD      C,A                 ; ... voice 2 command to C
00B1: 3A 44 40        LD      A,($4044)           ; {-1_v3Command} Priority of ...
00B4: CD 02 01        CALL    $0102               ; {GetComPriority} ... current ...
00B7: 32 49 40        LD      ($4049),A           ; {-1_v3priority} ... voice 3 command to 4049
00BA: 3A 46 40        LD      A,($4046)           ; {-1_cmdRequest} Requested command
00BD: CD 02 01        CALL    $0102               ; {GetComPriority} Priority of requested command ...
00C0: 5F              LD      E,A                 ; ... to E
...
00DA: 7B              LD      A,E                 
;
00DB: CD 8C 00        CALL    $008C               ; {FindDat} Find voice structure
00DE: 3A 46 40        LD      A,($4046)           ; {-1_cmdRequest} Store command ...
...
011C: CD 6D 02        CALL    $026D               ; {AYEnable} ... port A and B inputs, all noise and voice off
...
015C: CD E8 01        CALL    $01E8               ; {VCommandCont} Do continuation command
...
0171: CD E8 01        CALL    $01E8               ; {VCommandCont} Do continuation command
...
0186: CD E8 01        CALL    $01E8               ; {VCommandCont} Do continuation command
0189: 18 AB           JR      $136                ; {MainLoop} Back to top of loop
;
018B: 7E              LD      A,(HL)              ; Get command number
018C: CD D9 01        CALL    $01D9               ; {VCommandInit} Do the initialization command
018F: 18 CE           JR      $15F                ; Back to voice 2
;
0191: 7E              LD      A,(HL)              ; Get command number
0192: CD D9 01        CALL    $01D9               ; {VCommandInit} Do the initialization command
0195: 18 DD           JR      $174                ; Back to voice 3
;
0197: 7E              LD      A,(HL)              ; Get command number
0198: CD D9 01        CALL    $01D9               ; {VCommandInit} Do the initialization command
019B: 18 99           JR      $136                ; {MainLoop} Back to the top of the loop
...
01DC: CD 9D 01        CALL    $019D               ; {JumpTabler} Do initialization function
01DF: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Voice number
01E2: CD 8C 00        CALL    $008C               ; {FindDat} Get the voice command structure
...
0227: CD 9C 02        CALL    $029C               ; {ShutdownVoice} Not "continue" ... shutdown voice
022A: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Voice number
022D: C3 84 00        JP      $0084               ; Clear out 2-byte voice command (with 0,0)

AmplitudeOff: 
0230: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Voice number
...
023C: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Voice number (1,2, or 3)
023F: 3D              DEC     A                   ; Make it 0 based
0240: 87              ADD     A,A                 ; Voice * 2
0241: 47              LD      B,A                 ; To B
0242: CD 09 03        CALL    $0309               ; {WriteAYAL} Write L to voice's fine register
0245: 04              INC     B                   ; Bump to ...
0246: 78              LD      A,B                 ; ... coarse register
0247: D3 80           OUT     ($80),A             ; {-2_AY_ADDR} Write register address
0249: 7C              LD      A,H                 ; Coarse value
024A: D3 40           OUT     ($40),A             ; {-2_AY_WATA} Write coarse value
024C: C9              RET                         ; Done

ReadTune: 
; Read voice's coarse/fine registers to HL
024D: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Voice number (1,3, or 3)
0250: 3D              DEC     A                   ; Make it 0 based
0251: 87              ADD     A,A                 ; Voiced * 2
0252: 47              LD      B,A                 ; To B
0253: D3 80           OUT     ($80),A             ; {-2_AY_ADDR} Latch address
0255: DB 40           IN      A,($40)             ; {-2_AY_RDATA} Read fine value
0257: 6F              LD      L,A                 ; To L
0258: 04              INC     B                   ; Bump to ...
0259: 78              LD      A,B                 ; ... coarse register
025A: D3 80           OUT     ($80),A             ; {-2_AY_ADDR} Latch address
025C: DB 40           IN      A,($40)             ; {-2_AY_RDATA} Read coarse value
025E: 67              LD      H,A                 ; To H
025F: C9              RET                         ; Done
...
0263: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Roll the one target bit ...
...
026F: D3 80           OUT     ($80),A             ; {-2_AY_ADDR} Select register in AY chip
0271: 3A 4C 40        LD      A,($404C)           ; {-1_curEnable} Get current enable value
0274: A2              AND     D                   ; Mask bits off
0275: B3              OR      E                   ; OR bits on
0276: 32 4C 40        LD      ($404C),A           ; {-1_curEnable} New enable value
0279: D3 40           OUT     ($40),A             ; {-2_AY_WATA} Write value to AY chip
027B: C9              RET                         ; Done

SetAmplitude: 
; Set the voice's amplitude to B
027C: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Voice number
...
029C: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Voice number
...
02A8: CD 6D 02        CALL    $026D               ; {AYEnable} Turn off a voice (NOISE and TONE)
02AB: CD 30 02        CALL    $0230               ; {AmplitudeOff} Turn off voice amplitude
02AE: 18 17           JR      $2C7                ; {Filter00} Set voice's cap filtering to "none"
...
ReadAmplitude: 
02B7: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Voice number
02BA: E6 03           AND     $03                 ; Any requested?
02BC: C8              RET     Z                   ; No ... skip
02BD: C6 07           ADD     A,$07               ; Offset to amplitude
02BF: 18 00           JR      $2C1                ; {ReadAY} Read voice's amplitude register

ReadAY: 
; Read AY register (A) to B (and A).
02C1: D3 80           OUT     ($80),A             ; {-2_AY_ADDR} Latch the address
02C3: DB 40           IN      A,($40)             ; {-2_AY_RDATA} Read the value
...
02CD: 3A 4B 40        LD      A,($404B)           ; {-1_voiceNum} Voice number
...
02E7: CB 19           RR      C                   ; ...
;
02E9: 2A 4E 40        LD      HL,($404E)          ; {-1_curFilter} Current value of capacitor filter
...
02F2: B1              OR      C                   ; ...
02F3: 6F              LD      L,A                 ; ...
02F4: 22 4E 40        LD      ($404E),HL          ; {-1_curFilter} New capacitor filter value
...
0309: D3 80           OUT     ($80),A             ; {-2_AY_ADDR} Write address
030B: 7D              LD      A,L                 ; Write ...
030C: D3 40           OUT     ($40),A             ; {-2_AY_WATA} ... value
030E: C9              RET                         ; Out
...
0372: 36 03           LD      (HL),$03            
0374: CD 4D 02        CALL    $024D               ; {ReadTune} 
0377: B7              OR      A                   
...
03C2: 06 0E           LD      B,$0E               
03C4: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
03C7: 21 70 00        LD      HL,$0070            
03CA: CD 3C 02        CALL    $023C               ; {WriteTune} 
03CD: F7              RST     $30                 
03CE: CD 04 03        CALL    $0304               ; {Filter10} Set filtering 0.047uF
03D1: C9              RET                         
...
03EC: C9              RET                         
03ED: 06 00           LD      B,$00               
03EF: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
03F2: C9              RET                         
03F3: 21 3C 00        LD      HL,$003C            
03F6: CD 3C 02        CALL    $023C               ; {WriteTune} 
03F9: 06 0B           LD      B,$0B               
03FB: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
03FE: AF              XOR     A                   
03FF: C9              RET                         
0400: CD 04 03        CALL    $0304               ; {Filter10} 
0403: 3E 80           LD      A,$80               
0405: 32 5E 40        LD      ($405E),A           ; 
0408: 06 0A           LD      B,$0A               
040A: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
040D: 21 FC 00        LD      HL,$00FC            
0410: CD 3C 02        CALL    $023C               ; {WriteTune} 
0413: F7              RST     $30                 
0414: AF              XOR     A                   
...
0420: FE 41           CP      $41                 
0422: 38 0B           JR      C,$42F              ; 
0424: CD 4D 02        CALL    $024D               ; {ReadTune} 
0427: 2D              DEC     L                   
0428: 2D              DEC     L                   
0429: EF              RST     $28                 
042A: AF              XOR     A                   
...
0433: B7              OR      A                   
0434: 28 11           JR      Z,$447              ; 
0436: CD 4D 02        CALL    $024D               ; {ReadTune} 
0439: 2C              INC     L                   
043A: 2C              INC     L                   
043B: EF              RST     $28                 
043C: AF              XOR     A                   
...
044F: CD 3C 02        CALL    $023C               ; {WriteTune} 
0452: 3E 80           LD      A,$80               
...
;I03 Die in road
045D: CD C7 02        CALL    $02C7               ; {Filter00} 
0460: CD 60 02        CALL    $0260               ; {EnableTone} 
0463: 21 00 01        LD      HL,$0100            
0466: CD 3C 02        CALL    $023C               ; {WriteTune} 
0469: 06 0A           LD      B,$0A               
046B: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
...
047E: 3E FF           LD      A,$FF               
0480: C8              RET     Z                   
0481: CD 4D 02        CALL    $024D               ; {ReadTune} 
0484: 11 03 00        LD      DE,$0003            
0487: 19              ADD     HL,DE               
0488: CD 3C 02        CALL    $023C               ; {WriteTune} 
048B: AF              XOR     A                   
048C: C9              RET                         
...
;I15 Snake on ground
0505: CD 04 03        CALL    $0304               ; {Filter10} 
...
05D1: 23              INC     HL                  
05D2: 36 68           LD      (HL),$68            
05D4: CD B0 02        CALL    $02B0               ; {EnableNoise} 
05D7: 3E 06           LD      A,$06               
05D9: 06 18           LD      B,$18               
05DB: CF              RST     $08                 ; Write to AY
05DC: 06 04           LD      B,$04               
05DE: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
05E1: CD C7 02        CALL    $02C7               ; {Filter00} 
05E4: C9              RET                         
...
05F9: 36 04           LD      (HL),$04            
05FB: 06 00           LD      B,$00               
05FD: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
0600: 0E 01           LD      C,$01               
0602: 18 10           JR      $614                ; 
...
060E: 47              LD      B,A                 
060F: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
0612: 0E 01           LD      C,$01               
...
06A0: 36 03           LD      (HL),$03            
06A2: CD 4D 02        CALL    $024D               ; {ReadTune} 
06A5: B7              OR      A                   
...
06FC: 06 0D           LD      B,$0D               
06FE: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
0701: 21 50 00        LD      HL,$0050            
0704: CD 3C 02        CALL    $023C               ; {WriteTune} 
0707: CD 60 02        CALL    $0260               ; {EnableTone} 
070A: CD C7 02        CALL    $02C7               ; {Filter00} 
070D: C9              RET                         
...
0729: 36 24           LD      (HL),$24            
072B: 06 0D           LD      B,$0D               
072D: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
0730: 18 25           JR      $757                ; 
0732: CD 4D 02        CALL    $024D               ; {ReadTune} 
...
0742: 3E 19           LD      A,$19               
0744: CD 3C 02        CALL    $023C               ; {WriteTune} 
0747: 0E 02           LD      C,$02               
...
0751: 36 09           LD      (HL),$09            
0753: 47              LD      B,A                 
0754: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
0757: 0E 01           LD      C,$01               
...
076C: 06 00           LD      B,$00               
076E: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
0771: 21 84 41        LD      HL,$4184            
0774: CB D6           SET     2,(HL)              
0776: C9              RET                         
0777: 06 00           LD      B,$00               
0779: CD 7C 02        CALL    $027C               ; {SetAmplitude} 
077C: 3E FF           LD      A,$FF               
077E: C9              RET                         
...
07A8: CD B7 07        CALL    $07B7               ; {Music} Process music command
...
07E3: CA 6A 08        JP      Z,$086A             ; {MusicREST} All zero ... rest command
07E6: FE 1F           CP      $1F                 ; All ones?
07E8: C2 84 08        JP      NZ,$0884            ; {MusicNOTE} No ... note command
...
14C1: CD 4D 02        CALL    $024D               ; {ReadTune} 
