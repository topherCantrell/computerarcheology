...
0001: 32 80 A1        LD      ($A180),A           ; {hard.irqEnable} clear the vblank-interrupt-enable latch -- hold the per-frame interrupt off until state exists
...
0092: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} boot entry -- kick the watchdog
...
00BB: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} kick the watchdog between banks
00BE: 7B              LD      A,E                 
...
00E0: 3A E0 A0        LD      A,($A0E0)           ; {hard.dsw0} read DIP bank 0
...
loc_0103:
0103: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} kick the watchdog
...
012C: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} kick the watchdog
012F: 3E 01           LD      A,$01               
0131: 32 87 A1        LD      ($A187),A           ; {hard.flipScreen} select the upright screen at the hardware flip latch
...
014D: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} kick the watchdog
0150: 3A 00 A0        LD      A,($A000)           ; {hard.dsw1watchdog} read DIP bank 1
...
0176: 3A 00 A0        LD      A,($A000)           ; {hard.dsw1watchdog} read DIP bank 1 again for the lives setting
...
0189: 3A E0 A0        LD      A,($A0E0)           ; {hard.dsw0} read DIP bank 0 -- the coinage bank
018C: 47              LD      B,A                 
...
01A4: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} kick the watchdog
...
01AE: 3E 01           LD      A,$01               
01B0: 32 80 A1        LD      ($A180),A           ; {hard.irqEnable} enable the vblank interrupt -- the per-frame heartbeat begins
...
01C8: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} kick the watchdog
...
loc_01da:
01DA: 0B              DEC     BC                  
01DB: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} kick the watchdog while waiting
01DE: 3A 80 A0        LD      A,($A080)           ; {hard.in0} poll the input port
...
0208: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} kick the watchdog through the wait
...
0606: 0F              RRCA                        
0607: 0F              RRCA                        
0608: 32 BF 86        LD      ($86BF),A           ; {hard.videoRam+2BF} write the tens digit tile to the credit display

loc_060b:
060B: 78              LD      A,B                 
060C: E6 0F           AND     $0F                 
060E: 32 9F 86        LD      ($869F),A           ; {hard.videoRam+29F}
...
067C: 32 80 A1        LD      ($A180),A           ; {hard.irqEnable} block a re-entrant NMI while this frame's work runs
...
06B1: 32 00 A0        LD      ($A000),A           ; {hard.dsw1watchdog} kick the watchdog timer -- the written value is immaterial, only the periodic write matters
...
06C9: 3A C0 A0        LD      A,($A0C0)           ; {hard.in2} read the P2 control port (active-low)
06CC: 2F              CPL                         ; invert so a pressed control reads as a set bit
06CD: 77              LD      (HL),A              ; store this frame's P2 controls
06CE: 2B              DEC     HL                  
06CF: 3A A0 A0        LD      A,($A0A0)           ; {hard.in1}
06D2: 2F              CPL                         
06D3: 77              LD      (HL),A              ; store this frame's P1 controls
06D4: 2B              DEC     HL                  
06D5: 3A 80 A0        LD      A,($A080)           ; {hard.in0} read the coin/start/service port
06D8: 2F              CPL                         
...
06FD: 32 87 A1        LD      ($A187),A           ; {hard.flipScreen} latch the flip-screen line from the orientation flag
0700: FD E1           POP     IY                  
0702: DD E1           POP     IX                  
0704: E1              POP     HL                  
0705: D1              POP     DE                  
...
070C: C1              POP     BC                  
070D: 3E 01           LD      A,$01               
070F: 32 80 A1        LD      ($A180),A           ; {hard.irqEnable} write 1 back to re-arm the vblank NMI for the next frame
0712: F1              POP     AF                  
0713: C9              RET                         
...
loc_0929:
0929: 40              LD      B,B                 
092A: 38 0B           JR      C,$0937             ; {hard.rom+937}
...
095E: 20 16           JR      NZ,$0976            ; {hard.rom+976}
...
sendSoundCommand:
0E8F: 32 00 A1        LD      ($A100),A           ; {hard.soundLatch} drop the command byte into the audio processor's one-byte mailbox (port 0xa100)
0E92: 3E 01           LD      A,$01               ; the high level to strobe onto the audio-interrupt line
0E94: 32 81 A1        LD      ($A181),A           ; {hard.soundAttention} raise the audio-interrupt line -- its rising edge wakes the audio processor to read the mailbox
...
0E9E: 32 81 A1        LD      ($A181),A           ; {hard.soundAttention} lower the audio-interrupt line to rest -- arms the next command
0EA1: C9              RET                         
...
1061: A7              AND     A                   
1062: 3A A0 A0        LD      A,($A0A0)           ; {hard.in1} flipped screen: read the player-1 input port
1065: 20 03           JR      NZ,$106A            ; {code.loc_106a} game not in play here -- skip the control read
1067: 3A C0 A0        LD      A,($A0C0)           ; {hard.in2} read the player control port
...
1106: 79              LD      A,C                 
1107: 32 F2 85        LD      ($85F2),A           ; {hard.videoRam+1F2} paint the hundreds digit
...
12AB: 32 43 87        LD      ($8743),A           ; {hard.videoRam+343} update the stage-countdown HUD digit
12AE: C9              RET                         
...
1530: 32 E9 85        LD      ($85E9),A           ; {hard.videoRam+1E9} paint the hundreds digit
...
1E81: A7              AND     A                   
1E82: 3A A0 A0        LD      A,($A0A0)           ; {hard.in1} player 1's control port
1E85: 20 03           JR      NZ,$1E8A            ; {code.loc_1e8a} upright
1E87: 3A C0 A0        LD      A,($A0C0)           ; {hard.in2} player 2's control port (cocktail)
...
1F0B: 32 83 84        LD      ($8483),A           ; {hard.videoRam+83} stash it for the label
...
1F76: 32 43 87        LD      ($8743),A           ; {hard.videoRam+343} mirror it to the stage-digit cell
1F79: AF              XOR     A                   
...
2396: 3A 43 83        LD      A,($8343)           ; {hard.colorRam+343} add its partner
...
247A: A7              AND     A                   
247B: 20 06           JR      NZ,$2483            ; {hard.rom+2483} branch when that actor is present
...
27DE: 32 08 85        LD      ($8508),A           ; {hard.videoRam+108} write a demo marker tile
...
283C: 28 03           JR      Z,$2841             ; {hard.rom+2841} neither set: skip the status tile
283E: 3E 10           LD      A,$10               ; load the launch status tile
2840: 32 08 85        LD      ($8508),A           ; {hard.videoRam+108} clear the demo marker tile
...
29C7: 3A 43 83        LD      A,($8343)           ; {hard.colorRam+343} arena tile occupied?
29CA: A7              AND     A                   
...
2B81: A7              AND     A                   
2B82: 28 4E           JR      Z,$2BD2             ; {hard.rom+2BD2} no -> skip the swap
...
3274: C2 99 07        JP      NZ,$0799            ; {hard.rom+799} no -> the trap
3277: C9              RET                         
...
32AD: C3 D4 76        JP      $76D4               ; {hard.rom+76D4} no match -> the tamper trap
...
32B6: C3 29 38        JP      $3829               ; {hard.rom+3829} mismatch -> the tamper trap
...
32E4: C2 40 1F        JP      NZ,$1F40            ; {hard.rom+1F40} mismatch -> the tamper path
32E7: C9              RET                         
...
32F9: 3A 83 80        LD      A,($8083)           ; {hard.colorRam+83} arena cell occupied?
32FC: A7              AND     A                   
...
50F4: A7              AND     A                   
50F5: 20 22           JR      NZ,$5119            ; {hard.rom+5119} flag set on a tampered board -- divert off the normal path
...
5AAA: 3C              INC     A                   
5AAB: 32 83 A1        LD      ($A183),A           ; {hard.coinCounter0} drive the coin-meter output high
5AAE: C9              RET                         
...
5AB7: 32 83 A1        LD      ($A183),A           ; {hard.coinCounter0} drop the coin-meter output
5ABA: C9              RET                         
...
5ACE: 3C              INC     A                   
5ACF: 32 84 A1        LD      ($A184),A           ; {hard.coinCounter1} drive the second coin-meter output high
5AD2: C9              RET                         
...
5ADB: 32 84 A1        LD      ($A184),A           ; {hard.coinCounter1} drop the second coin-meter output
5ADE: C9              RET                         
...
662A: C2 84 52        JP      NZ,$5284            ; {hard.rom+5284} mismatch -- the screen was altered
...
665E: C2 14 60        JP      NZ,$6014            ; {hard.rom+6014} mismatch -- the screen was altered
6661: 15              DEC     D                   ; the high half
6662: C2 05 20        JP      NZ,$2005            ; {hard.rom+2005} second checkpoint failed
6665: C9              RET                         
...
68DF: C3 D4 76        JP      $76D4               ; {hard.rom+76D4} no match -- take the fault branch
...
68E8: C3 29 38        JP      $3829               ; {hard.rom+3829} mismatch -- take the fault branch
...
6B06: C2 29 38        JP      NZ,$3829            ; {hard.rom+3829} mismatch -- run the tamper handler
...
6E7C: C2 92 4C        JP      NZ,$4C92            ; {hard.rom+4C92} tampered -- freeze the machine
...
754E: C2 E1 43        JP      NZ,$43E1            ; {hard.rom+43E1} wrong -> stop the machine (tamper reflex)
7551: 15              DEC     D                   
7552: C2 2C 46        JP      NZ,$462C            ; {hard.rom+462C} high byte off -> stop the machine
...
780B: C2 34 23        JP      NZ,$2334            ; {hard.rom+2334} mismatch -- divert to the tamper handler
...
798C: C2 0B 7A        JP      NZ,$7A0B            ; {hard.rom+7A0B} signature miss -- refuse to run on a tampered image
798F: 7A              LD      A,D                 
7990: DD BE 01        CP      (IX+$01)            ; plain-sum high vs its signature
7993: C2 A0 0F        JP      NZ,$0FA0            ; {hard.rom+FA0} miss -- divert
7996: 7D              LD      A,L                 
7997: DD BE 02        CP      (IX+$02)            ; companion-sum low vs its signature
799A: C2 88 13        JP      NZ,$1388            ; {hard.rom+1388}
799D: 7C              LD      A,H                 
...
79CC: 20 04           JR      NZ,$79D2            ; {hard.rom+79D2}
...
7A01: C2 D0 07        JP      NZ,$07D0            ; {hard.rom+7D0} low-byte miss -- hard integrity trap
7A04: 7A              LD      A,D                 
7A05: 23              INC     HL                  
