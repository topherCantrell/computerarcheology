...
in actual (not rotated) position. The comments will refer to Xn (X not-rotated) and Yn. Sometimes
...
0003: C3 D4 18        JP      $18D4               ; {code.init} Continue startup at 18D4
...
0016: 32 72 20        LD      ($2072),A           ; {ram.vblankStatus} ... on the lower half of the screen to draw/move
0019: 21 C0 20        LD      HL,$20C0            ; {+ram.isrDelay} Decrement ...
001C: 35              DEC     (HL)                ; ... the general countdown (used for pauses)
001D: CD CD 17        CALL    $17CD               ; {code.CheckHandleTilt} Check and handle TILT
0020: DB 01           IN      A,($01)             ; {hard.INP1} Read coin switch
...
0026: 3A EA 20        LD      A,($20EA)           ; {ram.coinSwitch} Switch is now open. Was it ...
0029: A7              AND     A                   ; ... closed last time?
002A: CA 42 00        JP      Z,$0042             ; {} No ... skip registering the credit
;
; Handle bumping credit count
002D: 3A EB 20        LD      A,($20EB)           ; {ram.numCoins} Number of credits in BCD
...
0038: 32 EB 20        LD      ($20EB),A           ; {ram.numCoins} New number of credits
003B: CD 47 19        CALL    $1947               ; {code.DrawNumCredits} Draw credits on screen
003E: AF              XOR     A                   ; Credit switch ...
003F: 32 EA 20        LD      ($20EA),A           ; {ram.coinSwitch} ... has opened
;
0042: 3A E9 20        LD      A,($20E9)           ; {ram.suspendPlay} Are we moving ...
0045: A7              AND     A                   ; ... game objects?
0046: CA 82 00        JP      Z,$0082             ; {} No ... restore registers and out
0049: 3A EF 20        LD      A,($20EF)           ; {ram.gameMode} Are we in ...
004C: A7              AND     A                   ; ... game mode?
004D: C2 6F 00        JP      NZ,$006F            ; {} Yes ... go process game-play things and out
0050: 3A EB 20        LD      A,($20EB)           ; {ram.numCoins} Number of credits
...
0057: CD BF 0A        CALL    $0ABF               ; {code.ISRSplTasks} Process ISR tasks for splash screens
005A: C3 82 00        JP      $0082               ; {} Restore registers and out
;
; At this point no game is going and there are credits
005D: 3A 93 20        LD      A,($2093)           ; {ram.waitStartLoop} Are we in the ...
0060: A7              AND     A                   ; ... "press start" loop?
0061: C2 82 00        JP      NZ,$0082            ; {} Yes ... restore registers and out
0064: C3 65 07        JP      $0765               ; {code.WaitForStart} Start the "press start" loop
;
; Mark credit as needing registering
0067: 3E 01           LD      A,$01               ; Remember switch ...
0069: 32 EA 20        LD      ($20EA),A           ; {ram.coinSwitch} ... state for debounce
006C: C3 3F 00        JP      $003F               ; {} Continue
;
; Main game-play timing loop
006F: CD 40 17        CALL    $1740               ; {code.TimeFleetSound} Time down fleet sound and sets flag if needs new delay value
0072: 3A 32 20        LD      A,($2032)           ; {ram.obj2TimerExtra} Use rolling shot's timer to sync ...
0075: 32 80 20        LD      ($2080),A           ; {ram.shotSync} ... other two shots
0078: CD 00 01        CALL    $0100               ; {code.DrawAlien} Draw the current alien (or exploding alien)
007B: CD 48 02        CALL    $0248               ; {code.RunGameObjs} Process game objects (including player object)
007E: CD 13 09        CALL    $0913               ; {code.TimeToSaucer} Count down time to saucer
...
008D: 32 72 20        LD      ($2072),A           ; {ram.vblankStatus} ... objects on the upper half of screen to draw/move
0090: 3A E9 20        LD      A,($20E9)           ; {ram.suspendPlay} Are we moving ...
0093: A7              AND     A                   ; ... game objects?
0094: CA 82 00        JP      Z,$0082             ; {} No ... restore and return
0097: 3A EF 20        LD      A,($20EF)           ; {ram.gameMode} Are we in ...
009A: A7              AND     A                   ; ... game mode?
009B: C2 A5 00        JP      NZ,$00A5            ; {} Yes .... process game objects and out
009E: 3A C1 20        LD      A,($20C1)           ; {ram.isrSplashTask} Splash-animation tasks
...
00AB: CD 41 01        CALL    $0141               ; {code.CursorNextAlien} Advance cursor to next alien (move the alien if it is last one)
...
; player's data bank.
;
00B1: CD 86 08        CALL    $0886               ; {code.GetAlRefPtr} 2xFC Get current player's ref-alien position pointer
00B4: E5              PUSH    HL                  ; Hold pointer
00B5: 7E              LD      A,(HL)              ; Get player's ...
00B6: 23              INC     HL                  ; ... ref-alien ...
00B7: 66              LD      H,(HL)              ; ...
00B8: 6F              LD      L,A                 ; ... coordinates
00B9: 22 09 20        LD      ($2009),HL          ; {ram.refAlienYr} Set game's reference alien's X,Y
00BC: 22 0B 20        LD      ($200B),HL          ; {ram.alienPosLSB} Set game's alien cursor bit position
...
00C8: 32 08 20        LD      ($2008),A           ; {ram.refAlienDXr} Store alien deltaY
...
00D3: 32 0D 20        LD      ($200D),A           ; {ram.rackDirection} Store rack direction
00D6: C9              RET                         ; Done

00D7: 3E 02           LD      A,$02               ; Set ...
00D9: 32 FB 21        LD      ($21FB),A           ; {ram.p1RefAlienDX} ... player 1 and 2 ...
00DC: 32 FB 22        LD      ($22FB),A           ; {ram.p2RefAlienDX} ... alien delta to 2 (right 2 pixels)
...
0105: C2 38 15        JP      NZ,$1538            ; {code.AExplodeTime} Yes ... go time it down and out
;
0108: E5              PUSH    HL                  ; 2002 on the stack
0109: 3A 06 20        LD      A,($2006)           ; {ram.alienCurIndex} Get alien index ...
010C: 6F              LD      L,A                 ; ... for the 21xx or 22xx pointer
010D: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Get MSB ...
...
012E: 2A 0B 20        LD      HL,($200B)          ; {ram.alienPosLSB} Pixel position
0131: 06 10           LD      B,$10               ; 16 rows in alien sprites
0133: CD D3 15        CALL    $15D3               ; {code.DrawSprite} Draw shifted sprite
;
0136: AF              XOR     A                   ; Let the ISR routine ...
0137: 32 00 20        LD      ($2000),A           ; {ram.waitOnDraw} ... advance the cursor to the next alien
013A: C9              RET                         ; Out
;
...
;
0141: 3A 68 20        LD      A,($2068)           ; {ram.playerOK} Is the player ...
0144: A7              AND     A                   ; ... blowing up?
0145: C8              RET     Z                   ; Yes ... ignore the aliens
0146: 3A 00 20        LD      A,($2000)           ; {ram.waitOnDraw} Still waiting on ...
0149: A7              AND     A                   ; ... this alien to be drawn?
014A: C0              RET     NZ                  ; Yes ... leave cursor in place
014B: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Load alien-data ...
014E: 67              LD      H,A                 ; ... MSB (either 21xx or 22xx)
014F: 3A 06 20        LD      A,($2006)           ; {ram.alienCurIndex} Load the xx part of the alien flag pointer
...
0157: CC A1 01        CALL    Z,$01A1             ; {code.MoveRefAlien} Yes ... move the bottom/right alien and reset index to 0
015A: 6F              LD      L,A                 ; HL now points to alien flag
015B: 46              LD      B,(HL)              ; Is alien ...
015C: 05              DEC     B                   ; ... alive?
015D: C2 54 01        JP      NZ,$0154            ; {} No ... skip to next alien
0160: 32 06 20        LD      ($2006),A           ; {ram.alienCurIndex} New alien index
0163: CD 7A 01        CALL    $017A               ; {code.GetAlienCoords} Calculate bit position and type for index
0166: 61              LD      H,C                 ; The calculation returns the MSB in C
0167: 22 0B 20        LD      ($200B),HL          ; {ram.alienPosLSB} Store new bit position
...
0171: 32 04 20        LD      ($2004),A           ; {ram.alienRow} ... row index
0174: 3E 01           LD      A,$01               ; Set the wait-flag for the ...
0176: 32 00 20        LD      ($2000),A           ; {ram.waitOnDraw} ... draw-alien routine to clear
0179: C9              RET                         ; Done
...
01A2: CA CD 01        JP      Z,$01CD             ; {code.ReturnTwo} Return out of TWO call frames (only used if no aliens left)
...
01AE: CD D9 01        CALL    $01D9               ; {code.AddDelta} Move alien
...
01EC: C3 32 1A        JP      $1A32               ; {code.BlockCopy} Copy [DE]->[HL] and return
```
...
0200: CD 32 1A        CALL    $1A32               ; {code.BlockCopy} Block copy DE to HL (B bytes)
...
0217: C3 1E 02        JP      $021E               ; {code.CopyShields} Shuffle-shields player 2
...
;
021E: 32 81 20        LD      ($2081),A           ; {ram.tmp2081} Remember copy/restore flag
...
022B: 3A 81 20        LD      A,($2081)           ; {ram.tmp2081} Get back copy/restore flag
022E: A7              AND     A                   ; Not zero ...
022F: C2 42 02        JP      NZ,$0242            ; {} ... means remember shidles
0232: CD 69 1A        CALL    $1A69               ; {code.RestoreShields} Restore player's shields
...
;
0242: CD 7C 14        CALL    $147C               ; {code.RememberShields} Remember player's shields
...
029A: AF              XOR     A                   ; 0
029B: 32 68 20        LD      ($2068),A           ; {ram.playerOK} Player is NOT OK ... player is blowing up
029E: 32 69 20        LD      ($2069),A           ; {ram.enableAlienFire} Alien fire is disabled
02A1: 3E 30           LD      A,$30               ; Reset count ...
02A3: 32 6A 20        LD      ($206A),A           ; {ram.alienFireDelay} ... till alien shots are enabled
...
02AB: C2 9B 03        JP      NZ,$039B            ; {code.DrawPlayerDie} Still blowing up ... go draw next sprite
;
; Blow up finished
02AE: 2A 1A 20        LD      HL,($201A)          ; {ram.playerYr} Player's coordinates
02B1: 06 10           LD      B,$10               ; 16 Bytes
02B3: CD 24 14        CALL    $1424               ; {code.EraseSimpleSprite} Erase simple sprite (the player)
02B6: 21 10 20        LD      HL,$2010            ; Restore player ...
02B9: 11 10 1B        LD      DE,$1B10            ; ... structure ...
02BC: 06 10           LD      B,$10               ; ... from ...
02BE: CD 32 1A        CALL    $1A32               ; {code.BlockCopy} ... ROM mirror
02C1: 06 00           LD      B,$00               ; Turn off ...
02C3: CD DC 19        CALL    $19DC               ; {code.SoundBits3Off} ... all sounds
02C6: 3A 6D 20        LD      A,($206D)           ; {ram.invaded} Has rack reached ...
02C9: A7              AND     A                   ; ... the bottom of the screen?
02CA: C0              RET     NZ                  ; Yes ... done here
02CB: 3A EF 20        LD      A,($20EF)           ; {ram.gameMode} Are we in ...
...
02D4: CD D7 19        CALL    $19D7               ; {code.DsableGameTasks} Disable game tasks
...
02E6: 3A CE 20        LD      A,($20CE)           ; {ram.twoPlayers} Multi-player game
02E9: A7              AND     A                   ; Only one player?
02EA: CA 2C 03        JP      Z,$032C             ; {} Yes ... remove a ship from player's stash and reenter game loop
02ED: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Player data MSB
...
02F5: CD 0E 02        CALL    $020E               ; {code.RememberShields2} No ... go store player 2 shields
...
0301: 00              NOP                         ; ** Why?
0302: CD E4 01        CALL    $01E4               ; {code.CopyRAMMirror} Copy RAM mirror (getting ready to switch players)
...
0312: 32 67 20        LD      ($2067),A           ; {ram.playerDataMSB} Change players
0315: CD B6 0A        CALL    $0AB6               ; {code.TwoSecDelay} Two second delay
0318: AF              XOR     A                   ; Clear the player-object ...
0319: 32 11 20        LD      ($2011),A           ; {ram.obj0TimerLSB} ... timer (player can move instantly after switching players)
031C: 78              LD      A,B                 ; Cocktail bit to A
031D: D3 05           OUT     ($05),A             ; {hard.SOUND2} Set the cocktail mode
031F: 3C              INC     A                   ; Fleet sound 1 (first tone)
0320: 32 98 20        LD      ($2098),A           ; {ram.soundPort5} Set the port 5 hold
0323: CD D6 09        CALL    $09D6               ; {code.ClearPlayField} Clear center window
0326: CD 7F 1A        CALL    $1A7F               ; {code.RemoveShip} Remove a ship and update indicators
0329: C3 F9 07        JP      $07F9               ; {} Tell the players that the switch has been made
;
032C: CD 7F 1A        CALL    $1A7F               ; {code.RemoveShip} Remove a ship and update indicators
032F: C3 17 08        JP      $0817               ; {} Continue into game loop
;
0332: CD 09 02        CALL    $0209               ; {code.RememberShields1} Remember the shields for player 1
...
034A: 3A 1B 20        LD      A,($201B)           ; {ram.playerXr} Current player coordinates
034D: 47              LD      B,A                 ; Hold it
034E: 3A EF 20        LD      A,($20EF)           ; {ram.gameMode} Are we in ...
0351: A7              AND     A                   ; ... game mode?
0352: C2 63 03        JP      NZ,$0363            ; {} Yes ... use switches as player controls
; 
0355: 3A 1D 20        LD      A,($201D)           ; {ram.nextDemoCmd} Get demo command
0358: 0F              RRCA                        ; Is it right?
0359: DA 81 03        JP      C,$0381             ; {code.MovePlayerRight} Yes ... do right
035C: 0F              RRCA                        ; Is it left?
035D: DA 8E 03        JP      C,$038E             ; {code.MovePlayerLeft} Yes ... do left
0360: C3 6F 03        JP      $036F               ; {} Skip over movement (draw player and out)
; Player is in control
0363: CD C0 17        CALL    $17C0               ; {code.ReadInputs} Read active player controls
0366: 07              RLCA                        ; Test for ...
0367: 07              RLCA                        ; ... right button
0368: DA 81 03        JP      C,$0381             ; {code.MovePlayerRight} Yes ... handle move right
036B: 07              RLCA                        ; Test for left button
036C: DA 8E 03        JP      C,$038E             ; {code.MovePlayerLeft} Yes ... handle move left
; Draw player sprite
036F: 21 18 20        LD      HL,$2018            ; Active player descriptor
0372: CD 3B 1A        CALL    $1A3B               ; {code.ReadDesc} Load 5 byte sprite descriptor in order: EDLHB
0375: CD 47 1A        CALL    $1A47               ; {code.ConvToScr} Convert HL to screen coordinates
0378: CD 39 14        CALL    $1439               ; {code.DrawSimpSprite} Draw player
037B: 3E 00           LD      A,$00               ; Clear the task timer. Nobody changes this but it could have ...
037D: 32 12 20        LD      ($2012),A           ; {ram.obj0TimerExtra} ... been speed set for the player with a value other than 0 (not XORA)
0380: C9              RET                         ; Out
...
0388: 32 1B 20        LD      ($201B),A           ; {ram.playerXr} New X coordinate
...
0395: 32 1B 20        LD      ($201B),A           ; {ram.playerXr} New X coordinate
...
039E: 32 15 20        LD      ($2015),A           ; {ram.playerAlive} Hold current state
03A1: 07              RLCA                        ; *2
03A2: 07              RLCA                        ; *4
03A3: 07              RLCA                        ; *8
03A4: 07              RLCA                        ; *16
...
03AA: 22 18 20        LD      ($2018),HL          ; {ram.plyrSprPicL} New blow-up sprite picture
...
03BE: CD 06 1A        CALL    $1A06               ; {code.CompYToBeam} Compare to screen-update location
...
03C9: CA FA 03        JP      Z,$03FA             ; {code.InitPlyShot} Yes ... go initiate shot
;
03CC: FE 02           CP      $02                 ; Progressing normally?
03CE: CA 0A 04        JP      Z,$040A             ; {code.MovePlyShot} Yes ... go move it
;
03D1: 23              INC     HL                  ; 2026
...
03D8: CA 36 04        JP      Z,$0436             ; {code.EndOfBlowup} If done then
...
03E0: CD 30 04        CALL    $0430               ; {code.ReadPlyShot} Read shot descriptor
03E3: CD 52 14        CALL    $1452               ; {code.EraseShifted} Erase the sprite
...
03F4: CD 30 04        CALL    $0430               ; {code.ReadPlyShot} Read player shot structure
03F7: C3 00 14        JP      $1400               ; {code.DrawShiftedSprite} Draw sprite and out
;
InitPlyShot:
03FA: 3C              INC     A                   ; Type is now ...
03FB: 77              LD      (HL),A              ; ... 2 (in progress)
03FC: 3A 1B 20        LD      A,($201B)           ; {ram.playerXr} Players Y coordinate
03FF: C6 08           ADD     A,$08               ; To center of player
0401: 32 2A 20        LD      ($202A),A           ; {ram.obj1CoorXr} Shot's Y coordinate
0404: CD 30 04        CALL    $0430               ; {code.ReadPlyShot} Read 5 byte structure
0407: C3 00 14        JP      $1400               ; {code.DrawShiftedSprite} Draw sprite and out
;
MovePlyShot:
040A: CD 30 04        CALL    $0430               ; {code.ReadPlyShot} Read the shot structure
040D: D5              PUSH    DE                  ; Hold pointer to sprite image
040E: E5              PUSH    HL                  ; Hold sprite coordinates
040F: C5              PUSH    BC                  ; Hold sprite size (in B)
0410: CD 52 14        CALL    $1452               ; {code.EraseShifted} Erase the sprite from the screen
0413: C1              POP     BC                  ; Restore size
0414: E1              POP     HL                  ; Restore coords
0415: D1              POP     DE                  ; Restore pointer to sprite image
0416: 3A 2C 20        LD      A,($202C)           ; {ram.shotDeltaX} DeltaX for shot
0419: 85              ADD     A,L                 ; Move the shot ...
041A: 6F              LD      L,A                 ; ... up the screen
041B: 32 29 20        LD      ($2029),A           ; {ram.obj1CoorYr} Store shot's new X coordinate
041E: CD 91 14        CALL    $1491               ; {code.DrawSprCollision} Draw sprite with collision detection
0421: 3A 61 20        LD      A,($2061)           ; {ram.collision} Test for ...
0424: A7              AND     A                   ; ... collision
0425: C8              RET     Z                   ; No collision ... out
;
; Collision with alien detected
0426: 32 02 20        LD      ($2002),A           ; {ram.alienIsExploding} Set to not-0 indicating ...
...
042D: C3 36 04        JP      $0436               ; {code.EndOfBlowup} Anything else erases the shot and removes it from duty

ReadPlyShot:
0430: 21 27 20        LD      HL,$2027            ; Read 5 byte sprite structure for ...
0433: C3 3B 1A        JP      $1A3B               ; {code.ReadDesc} ... player shot

EndOfBlowup:
0436: CD 30 04        CALL    $0430               ; {code.ReadPlyShot} Read the shot structure
0439: CD 52 14        CALL    $1452               ; {code.EraseShifted} Erase the player's shot
043C: 21 25 20        LD      HL,$2025            ; Reinit ...
043F: 11 25 1B        LD      DE,$1B25            ; ... shot structure ...
0442: 06 07           LD      B,$07               ; ... from ...
0444: CD 32 1A        CALL    $1A32               ; {code.BlockCopy} ... ROM mirror
0447: 2A 8D 20        LD      HL,($208D)          ; {ram.sauScoreLSB} Get pointer to saucer-score table
...
0453: 22 8D 20        LD      ($208D),HL          ; {ram.sauScoreLSB} New score pointer
0456: 2A 8F 20        LD      HL,($208F)          ; {ram.shotCountLSB} Increments with every shot ...
0459: 2C              INC     L                   ; ... but only LSB ** ...
045A: 22 8F 20        LD      ($208F),HL          ; {ram.shotCountLSB} ... used for saucer direction
;
045D: 3A 84 20        LD      A,($2084)           ; {ram.saucerActive} Is saucer ...
...
047A: 32 32 20        LD      ($2032),A           ; {ram.obj2TimerExtra} ... ROM mirror (value 2)
047D: 2A 38 20        LD      HL,($2038)          ; {ram.rolShotCFirLSB} Get pointer to ...
...
0486: 22 38 20        LD      ($2038),HL          ; {ram.rolShotCFirLSB} Store new counter value (run the shot next time)
0489: C9              RET                         ; And out

048A: 11 35 20        LD      DE,$2035            ; Rolling-shot data structure
048D: 3E F9           LD      A,$F9               ; Last picture of "rolling" alien shot
048F: CD 50 05        CALL    $0550               ; {code.ToShotStruct} Set code to handle rolling-shot
0492: 3A 46 20        LD      A,($2046)           ; {ram.pluShotStepCnt} Get the plunger-shot step count
0495: 32 70 20        LD      ($2070),A           ; {ram.otherShot1} Hold it
0498: 3A 56 20        LD      A,($2056)           ; {ram.squShotStepCnt} Get the squiggly-shot step count
049B: 32 71 20        LD      ($2071),A           ; {ram.otherShot2} Hold it
049E: CD 63 05        CALL    $0563               ; {code.HandleAlienShot} Handle active shot structure
04A1: 3A 78 20        LD      A,($2078)           ; {ram.aShotBlowCnt} Blow up counter
04A4: A7              AND     A                   ; Test if shot has cycled through blowing up
04A5: 21 35 20        LD      HL,$2035            ; Rolling-shot data structure
04A8: C2 5B 05        JP      NZ,$055B            ; {code.FromShotStruct} If shot is still running, copy the updated data and out
...
04B3: C3 32 1A        JP      $1A32               ; {code.BlockCopy} ... from ROM mirror and out
...
04B7: 3A 6E 20        LD      A,($206E)           ; {ram.skipPlunger} One alien left? Skip plunger shot?
04BA: A7              AND     A                   ; Check
04BB: C0              RET     NZ                  ; Yes. Only one alien. Skip this shot.
04BC: 3A 80 20        LD      A,($2080)           ; {ram.shotSync} Sync flag (copied from GO-2's timer value)
...
04C7: CD 50 05        CALL    $0550               ; {code.ToShotStruct} Copy the plunger alien to the active structure
04CA: 3A 36 20        LD      A,($2036)           ; {ram.rolShotStepCnt} Step count from rolling-shot
04CD: 32 70 20        LD      ($2070),A           ; {ram.otherShot1} Hold it
04D0: 3A 56 20        LD      A,($2056)           ; {ram.squShotStepCnt} Step count from squiggly shot
04D3: 32 71 20        LD      ($2071),A           ; {ram.otherShot2} Hold it
04D6: CD 63 05        CALL    $0563               ; {code.HandleAlienShot} Handle active shot structure
04D9: 3A 76 20        LD      A,($2076)           ; {ram.aShotCFirLSB} LSB of column-firing table
...
04E4: 32 76 20        LD      ($2076),A           ; {ram.aShotCFirLSB} ... so reset pointer into firing-column table
04E7: 3A 78 20        LD      A,($2078)           ; {ram.aShotBlowCnt} Get the blow up timer
04EA: A7              AND     A                   ; Zero means shot is done
04EB: 21 45 20        LD      HL,$2045            ; Plunger shot data
04EE: C2 5B 05        JP      NZ,$055B            ; {code.FromShotStruct} If shot is still running, go copy the updated data and out
;
04F1: 11 40 1B        LD      DE,$1B40            ; Reload ...
04F4: 21 40 20        LD      HL,$2040            ; ... object ...
04F7: 06 10           LD      B,$10               ; ... structure ...
04F9: CD 32 1A        CALL    $1A32               ; {code.BlockCopy} ... from mirror
;
04FC: 3A 82 20        LD      A,($2082)           ; {ram.numAliens} Number of aliens on screen
04FF: 3D              DEC     A                   ; Is there only one left?
0500: C2 08 05        JP      NZ,$0508            ; {} No ... move on
0503: 3E 01           LD      A,$01               ; Disable plunger shot ...
0505: 32 6E 20        LD      ($206E),A           ; {ram.skipPlunger} ... when only one alien remains
0508: 2A 76 20        LD      HL,($2076)          ; {ram.aShotCFirLSB} Set the plunger shot's ...
...
0514: CD 50 05        CALL    $0550               ; {code.ToShotStruct} Copy squiggly shot to
0517: 3A 46 20        LD      A,($2046)           ; {ram.pluShotStepCnt} Get plunger ...
051A: 32 70 20        LD      ($2070),A           ; {ram.otherShot1} ... step count
051D: 3A 36 20        LD      A,($2036)           ; {ram.rolShotStepCnt} Get rolling ...
0520: 32 71 20        LD      ($2071),A           ; {ram.otherShot2} ... step count
0523: CD 63 05        CALL    $0563               ; {code.HandleAlienShot} Handle active shot structure
0526: 3A 76 20        LD      A,($2076)           ; {ram.aShotCFirLSB} LSB of column-firing table pointer
...
0531: 32 76 20        LD      ($2076),A           ; {ram.aShotCFirLSB} ... back to the start of the table
0534: 3A 78 20        LD      A,($2078)           ; {ram.aShotBlowCnt} Check to see if squiggly shot is done
0537: A7              AND     A                   ; 0 means blow-up timer expired
0538: 21 55 20        LD      HL,$2055            ; Squiggly shot data structure
053B: C2 5B 05        JP      NZ,$055B            ; {code.FromShotStruct} If shot is still running, go copy the updated data and out
...
0546: CD 32 1A        CALL    $1A32               ; {code.BlockCopy} ... from mirror
0549: 2A 76 20        LD      HL,($2076)          ; {ram.aShotCFirLSB} Copy pointer to column-firing table ...
054C: 22 58 20        LD      ($2058),HL          ; {ram.squShotCFirLSB} ... back to data structure (for next shot)
054F: C9              RET                         ; Done

ToShotStruct:
0550: 32 7F 20        LD      ($207F),A           ; {ram.shotPicEnd} LSB of last byte of last picture in sprite
0553: 21 73 20        LD      HL,$2073            ; Destination is the shot-structure
0556: 06 0B           LD      B,$0B               ; 11 bytes
0558: C3 32 1A        JP      $1A32               ; {code.BlockCopy} Block copy and out

FromShotStruct:
055B: 11 73 20        LD      DE,$2073            ; Source is the shot-structure
055E: 06 0B           LD      B,$0B               ; 11 bytes
0560: C3 32 1A        JP      $1A32               ; {code.BlockCopy} Block copy and out
...
056C: 3A C1 20        LD      A,($20C1)           ; {ram.isrSplashTask} ISR splash task
056F: FE 04           CP      $04                 ; Shooting the "C" ?
0571: 3A 69 20        LD      A,($2069)           ; {ram.enableAlienFire} Alien fire enabled flag
...
057C: 3A 70 20        LD      A,($2070)           ; {ram.otherShot1} Get the step count of the 1st "other shot"
057F: A7              AND     A                   ; Any steps made?
0580: CA 89 05        JP      Z,$0589             ; {} No ... ignore this count
0583: 47              LD      B,A                 ; Shuffle off step count
0584: 3A CF 20        LD      A,($20CF)           ; {ram.aShotReloadRate} Get the reload rate (based on MSB of score)
0587: B8              CP      B                   ; Too soon to fire again?
0588: D0              RET     NC                  ; Yes ... don't fire
0589: 3A 71 20        LD      A,($2071)           ; {ram.otherShot2} Get the step count of the 2nd "other shot"
...
0591: 3A CF 20        LD      A,($20CF)           ; {ram.aShotReloadRate} Get the reload rate (based on MSB of score)
...
059C: 2A 76 20        LD      HL,($2076)          ; {ram.aShotCFirLSB} Column-firing table
059F: 4E              LD      C,(HL)              ; Get next column to fire from
05A0: 23              INC     HL                  ; Bump the ...
05A1: 00              NOP                         ; % WHY?
05A2: 22 76 20        LD      ($2076),HL          ; {ram.aShotCFirLSB} ... pointer into column table
05A5: CD 2F 06        CALL    $062F               ; {code.FindInColumn} Find alien in target column
05A8: D0              RET     NC                  ; No alien is alive in target column ... out
;
05A9: CD 7A 01        CALL    $017A               ; {code.GetAlienCoords} Get coordinates of alien (lowest alien in firing column)
...
05B3: 6F              LD      L,A                 ; To L
05B4: 22 7B 20        LD      ($207B),HL          ; {ram.alienShotYr} Set shot coordinates below alien
;
...
05C4: CD 06 1A        CALL    $1A06               ; {code.CompYToBeam} Compare to beam position
...
05CC: C2 44 06        JP      NZ,$0644            ; {code.ShotBlowingUp} Go do shot-is-blowing-up sequence
05CF: 23              INC     HL                  ; 2074 step count
05D0: 34              INC     (HL)                ; Count the steps (used for fire rate)
05D1: CD 75 06        CALL    $0675               ; {} Erase shot
05D4: 3A 79 20        LD      A,($2079)           ; {ram.aShotImageLSB} Get LSB of the image pointer
...
05E2: 32 79 20        LD      ($2079),A           ; {ram.aShotImageLSB} New LSB image pointer
05E5: 3A 7B 20        LD      A,($207B)           ; {ram.alienShotYr} Get shot's Y coordinate
05E8: 47              LD      B,A                 ; Hold it
05E9: 3A 7E 20        LD      A,($207E)           ; {ram.alienShotDelta} Get alien shot delta
05EC: 80              ADD     A,B                 ; Add to shots coordinate
05ED: 32 7B 20        LD      ($207B),A           ; {ram.alienShotYr} New shot Y coordinate
05F0: CD 6C 06        CALL    $066C               ; {} Draw the alien shot
05F3: 3A 7B 20        LD      A,($207B)           ; {ram.alienShotYr} Shot's Y coordinate
05F6: FE 15           CP      $15                 ; Still in the active playfield?
05F8: DA 12 06        JP      C,$0612             ; {} No ... end it
05FB: 3A 61 20        LD      A,($2061)           ; {ram.collision} Did shot collide ...
05FE: A7              AND     A                   ; ... with something?
05FF: C8              RET     Z                   ; No ... we are done here
0600: 3A 7B 20        LD      A,($207B)           ; {ram.alienShotYr} Shot's Y coordinate
...
060F: 32 15 20        LD      ($2015),A           ; {ram.playerAlive} ... has been struck
;
0612: 3A 73 20        LD      A,($2073)           ; {ram.aShotStatus} Flag to ...
0615: F6 01           OR      $01                 ; ... start shot ...
0617: 32 73 20        LD      ($2073),A           ; {ram.aShotStatus} ... blowing up
061A: C9              RET                         ; Out
;
; Start a shot right over the player
061B: 3A 1B 20        LD      A,($201B)           ; {ram.playerXr} Player's X coordinate
061E: C6 08           ADD     A,$08               ; Center of player
0620: 67              LD      H,A                 ; To H for routine
0621: CD 6F 15        CALL    $156F               ; {code.FindColumn} Find the column
...
0630: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Player's MSB (21xx or 22xx)
...
0654: 22 79 20        LD      ($2079),HL          ; {ram.aShotImageLSB} ... explosion sprite
...
0661: 32 7D 20        LD      ($207D),A           ; {ram.alienShotSize} ... size 6
...
066F: CD 3B 1A        CALL    $1A3B               ; {code.ReadDesc} Read 5 byte structure
0672: C3 91 14        JP      $1491               ; {code.DrawSprCollision} Draw shot and out
;
0675: 21 79 20        LD      HL,$2079            ; Alien shot descriptor
0678: CD 3B 1A        CALL    $1A3B               ; {code.ReadDesc} Read 5 byte structure
067B: C3 52 14        JP      $1452               ; {code.EraseShifted} Erase the shot and out

067E: 22 48 20        LD      ($2048),HL          ; {ram.pluShotCFirLSB} From 50B, update ...
...
0683: 3A 80 20        LD      A,($2080)           ; {ram.shotSync} Sync flag (copied from GO-2's timer value)
...
0691: 3A 56 20        LD      A,($2056)           ; {ram.squShotStepCnt} Is there a ...
...
069E: 3A 82 20        LD      A,($2082)           ; {ram.numAliens} Number of aliens remaining
...
06AE: CD 06 1A        CALL    $1A06               ; {code.CompYToBeam} Compare to beam position
...
06C1: 32 8A 20        LD      ($208A),A           ; {ram.saucerPriPicMSB} New coordinate
...
06D8: CD DC 19        CALL    $19DC               ; {code.SoundBits3Off} ... flying saucer sound
...
06F4: D3 05           OUT     ($05),A             ; {hard.SOUND2} ... cocktail cabinet bit
06F6: 00              NOP                         ; ** Why
06F7: 00              NOP                         ; **
06F8: 00              NOP                         ; **
;
06F9: CD 42 07        CALL    $0742               ; {} Covert pixel pos from descriptor to HL screen and shift
06FC: CD CB 14        CALL    $14CB               ; {code.ClearSmallSprite} Clear a one byte sprite at HL
...
0709: C3 DC 19        JP      $19DC               ; {code.SoundBits3Off} ... sound and out

070C: 3E 01           LD      A,$01               ; Flag the score ...
070E: 32 F1 20        LD      ($20F1),A           ; {ram.adjustScore} ... needs updating
0711: 2A 8D 20        LD      HL,($208D)          ; {ram.sauScoreLSB} Saucer score table
...
0729: 32 87 20        LD      ($2087),A           ; {ram.saucerPriLocLSB} Message's LSB (_50=1D94 100=1D97 150=1D9A 300=1D9D)
...
0733: 22 F2 20        LD      ($20F2),HL          ; {ram.scoreDeltaLSB} Add score for hitting saucer (015 becomes 150 in BCD).
...
073F: C3 39 14        JP      $1439               ; {code.DrawSimpSprite} ... flying saucer

0742: 21 87 20        LD      HL,$2087            ; Read flying saucer ...
0745: CD 3B 1A        CALL    $1A3B               ; {code.ReadDesc} ... structure
0748: C3 47 1A        JP      $1A47               ; {code.ConvToScr} Convert pixel number to screen and shift and out
;
...
078E: CD F3 08        CALL    $08F3               ; {code.PrintMessage} Print message
0791: DB 01           IN      A,($01)             ; {hard.INP1} Read player controls
...
079B: 32 CE 20        LD      ($20CE),A           ; {ram.twoPlayers} Set flag for 1 or 2 players
079E: 3A EB 20        LD      A,($20EB)           ; {ram.numCoins} Number of credits
07A1: 80              ADD     A,B                 ; Take away credits
07A2: 27              DAA                         ; Convert back to DAA
07A3: 32 EB 20        LD      ($20EB),A           ; {ram.numCoins} New credit count
07A6: CD 47 19        CALL    $1947               ; {code.DrawNumCredits} Display number of credits
07A9: 21 00 00        LD      HL,$0000            ; Score of 0000
07AC: 22 F8 20        LD      ($20F8),HL          ; {ram.P1ScorL} Clear player-1 score
07AF: 22 FC 20        LD      ($20FC),HL          ; {ram.P2ScorL} Clear player-2 score
07B2: CD 25 19        CALL    $1925               ; {} Print player-1 score
07B5: CD 2B 19        CALL    $192B               ; {} Print player-2 score
07B8: CD D7 19        CALL    $19D7               ; {code.DsableGameTasks} Disable game tasks
07BB: 21 01 01        LD      HL,$0101            ; Two bytes 1, 1
07BE: 7C              LD      A,H                 ; 1 to A
07BF: 32 EF 20        LD      ($20EF),A           ; {ram.gameMode} 20EF=1 ... game mode
07C2: 22 E7 20        LD      ($20E7),HL          ; {ram.player1Alive} 20E7 and 20E8 both one ... players 1 and 2 are alive
07C5: 22 E5 20        LD      ($20E5),HL          ; {ram.player1Ex} Extra-ship is available for player-1 and player-2
07C8: CD 56 19        CALL    $1956               ; {code.DrawStatus} Print scores and credits
07CB: CD EF 01        CALL    $01EF               ; {code.DrawShieldPl1} Draw shields for player-1
07CE: CD F5 01        CALL    $01F5               ; {code.DrawShieldPl2} Draw shields for player-2
07D1: CD D1 08        CALL    $08D1               ; {code.GetShipsPerCred} Get number of ships from DIP settings
07D4: 32 FF 21        LD      ($21FF),A           ; {ram.p1ShipsRem} Player-1 ships
07D7: 32 FF 22        LD      ($22FF),A           ; {ram.p2ShipsRem} Player-2 ships
07DA: CD D7 00        CALL    $00D7               ; {} Set player-1 and player-2 alien racks going right
07DD: AF              XOR     A                   ; Make a 0
07DE: 32 FE 21        LD      ($21FE),A           ; {ram.p1RackCnt} Player 1 is on first rack of aliens
07E1: 32 FE 22        LD      ($22FE),A           ; {ram.p2RackCnt} Player 2 is on first rack of aliens
07E4: CD C0 01        CALL    $01C0               ; {code.InitAliens} Initialize 55 aliens for player 1
07E7: CD 04 19        CALL    $1904               ; {code.InitAliensP2} Initialize 55 aliens for player 2
07EA: 21 78 38        LD      HL,$3878            ; Screen coordinates for lower-left alien
07ED: 22 FC 21        LD      ($21FC),HL          ; {ram.p1RefAlienY} Initialize reference alien for player 1
07F0: 22 FC 22        LD      ($22FC),HL          ; {ram.p2RefAlienYr} Initialize reference alien for player 2
07F3: CD E4 01        CALL    $01E4               ; {code.CopyRAMMirror} Copy ROM mirror to RAM (2000 - 20C0)
07F6: CD 7F 1A        CALL    $1A7F               ; {code.RemoveShip} Initialize ship hold indicator
;
07F9: CD 8D 08        CALL    $088D               ; {code.PromptPlayer} Prompt with "PLAY PLAYER "
07FC: CD D6 09        CALL    $09D6               ; {code.ClearPlayField} Clear the playfield
07FF: 00              NOP                         ; % Why?
0800: AF              XOR     A                   ; Make a 0
0801: 32 C1 20        LD      ($20C1),A           ; {ram.isrSplashTask} Disable isr splash-task animation
0804: CD CF 01        CALL    $01CF               ; {code.DrawBottomLine} Draw line across screen under player
0807: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Current player
080A: 0F              RRCA                        ; Right bit tells all
080B: DA 72 08        JP      C,$0872             ; {} Go do player 1
;
080E: CD 13 02        CALL    $0213               ; {code.RestoreShields2} Restore shields for player 2
0811: CD CF 01        CALL    $01CF               ; {code.DrawBottomLine} Draw line across screen under player
0814: CD B1 00        CALL    $00B1               ; {code.InitRack} Initialize alien rack for current player
0817: CD D1 19        CALL    $19D1               ; {code.EnableGameTasks} Enable game tasks in ISR
081A: 06 20           LD      B,$20               ; Enable ...
081C: CD FA 18        CALL    $18FA               ; {code.SoundBits3On} ... sound amplifier
;
; GAME LOOP
;
081F: CD 18 16        CALL    $1618               ; {code.PlrFireOrDemo} Initiate player shot if button pressed
0822: CD 0A 19        CALL    $190A               ; {code.PlyrShotAndBump} Collision detect player's shot and rack-bump
0825: CD F3 15        CALL    $15F3               ; {code.CountAliens} Count aliens (count to 2082)
0828: CD 88 09        CALL    $0988               ; {code.AdjustScore} Adjust score (and print) if there is an adjustment
082B: 3A 82 20        LD      A,($2082)           ; {ram.numAliens} Number of live aliens
082E: A7              AND     A                   ; All aliens gone?
082F: CA EF 09        JP      Z,$09EF             ; {} Yes ... end of turn
0832: CD 0E 17        CALL    $170E               ; {code.AShotReloadRate} Update alien-shot-rate based on player's score
0835: CD 35 09        CALL    $0935               ; {} Check (and handle) extra ship award
0838: CD D8 08        CALL    $08D8               ; {code.SpeedShots} Adjust alien shot speed
083B: CD 2C 17        CALL    $172C               ; {code.ShotSound} Shot sound on or off with 2025
...
0846: CD FA 18        CALL    $18FA               ; {code.SoundBits3On} Make explosion sound
0849: CD 75 17        CALL    $1775               ; {code.FleetDelayExShip} Extra-ship sound timer, set fleet-delay, play fleet movement sound
084C: D3 06           OUT     ($06),A             ; {hard.WATCHDOG} Feed the watchdog
084E: CD 04 18        CALL    $1804               ; {code.CtrlSaucerSound} Control saucer sound
...
085A: CD F3 08        CALL    $08F3               ; {code.PrintMessage} Print message
085D: 06 98           LD      B,$98               ; -2 (take away 2 credits)
085F: DB 01           IN      A,($01)             ; {hard.INP1} Read player controls
0861: 0F              RRCA                        ; Test ...
0862: 0F              RRCA                        ; ... bit 2
0863: DA 6D 08        JP      C,$086D             ; {} 2 player button pressed ... do it
0866: 0F              RRCA                        ; Test bit 3
0867: DA 98 07        JP      C,$0798             ; {code.NewGame} One player start ... do it
...
0872: CD 1A 02        CALL    $021A               ; {code.RestoreShields1} Restore shields for player 1
0875: C3 14 08        JP      $0814               ; {} Continue in game loop

0878: 3A 08 20        LD      A,($2008)           ; {ram.refAlienDXr} Alien deltaY
087B: 47              LD      B,A                 ; Hold it
087C: 2A 09 20        LD      HL,($2009)          ; {ram.refAlienYr} Alien coordinates
087F: EB              EX      DE,HL               ; Coordinates to DE
0880: C3 86 08        JP      $0886               ; {code.GetAlRefPtr} HL is 21FC or 22FC and out
            
0883: 00 00 00                                    ; ** Why?

GetAlRefPtr:
; Get pointer to player's alien ref coordiantes
0886: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Player data MSB (21 or 22)
0889: 67              LD      H,A                 ; To H
...
0895: CD F3 08        CALL    $08F3               ; {code.PrintMessage} Print the message
0898: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Get the player number
089B: 0F              RRCA                        ; C will be set for player 1
089C: 3E 1C           LD      A,$1C               ; The "2" character
089E: 21 11 37        LD      HL,$3711            ; Replace the "<1>" with "<2">
08A1: D4 FF 08        CALL    NC,$08FF            ; {code.DrawChar} If player 2 ... change the message
08A4: 3E B0           LD      A,$B0               ; Delay of 176 (roughly 2 seconds)
08A6: 32 C0 20        LD      ($20C0),A           ; {ram.isrDelay} Set the ISR delay value
;
08A9: 3A C0 20        LD      A,($20C0)           ; {ram.isrDelay} Get the ISR delay value
...
08B6: CD 31 19        CALL    $1931               ; {code.DrawScore} Draw the score
...
08C1: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Get the player number
...
08CB: CD CB 14        CALL    $14CB               ; {code.ClearSmallSprite} Clear a one byte sprite at HL
08CE: C3 A9 08        JP      $08A9               ; {} Back to the top of the wait loop


GetShipsPerCred:
; Get number of ships from DIP settings
08D1: DB 02           IN      A,($02)             ; {hard.INP2} DIP settings
...
;
08D8: 3A 82 20        LD      A,($2082)           ; {ram.numAliens} Number of aliens on screen
08DB: FE 09           CP      $09                 ; More than 8?
08DD: D0              RET     NC                  ; Yes ... leave shot speed alone
08DE: 3E FB           LD      A,$FB               ; Normally FF (-4) ... now FB (-5)
08E0: 32 7E 20        LD      ($207E),A           ; {ram.alienShotDelta} Speed up alien shots
08E3: C9              RET                         ; Done

08E4: 3A CE 20        LD      A,($20CE)           ; {ram.twoPlayers} Number of players
...
08EE: C3 CB 14        JP      $14CB               ; {code.ClearSmallSprite} Clear a one byte sprite (32 rows long) at HL
...
08F5: CD FF 08        CALL    $08FF               ; {code.DrawChar} Print character
08F8: D1              POP     DE                  ; Restore
08F9: 13              INC     DE                  ; Next character
08FA: 0D              DEC     C                   ; All done?
08FB: C2 F3 08        JP      NZ,$08F3            ; {code.PrintMessage} Print all of message
08FE: C9              RET                         ; Out
...
090E: D3 06           OUT     ($06),A             ; {hard.WATCHDOG} Feed watchdog
0910: C3 39 14        JP      $1439               ; {code.DrawSimpSprite} To screen

TimeToSaucer:
0913: 3A 09 20        LD      A,($2009)           ; {ram.refAlienYr} Reference alien's X coordinate
0916: FE 78           CP      $78                 ; Don't process saucer timer ... ($78 is 1st rack Yr)
0918: D0              RET     NC                  ; ... unless aliens are closer to bottom
0919: 2A 91 20        LD      HL,($2091)          ; {ram.tillSaucerLSB} Time to saucer
...
0926: 32 83 20        LD      ($2083),A           ; {ram.saucerStart} ... saucer sequence
0929: 2B              DEC     HL                  ; Decrement the ...
092A: 22 91 20        LD      ($2091),HL          ; {ram.tillSaucerLSB} ... time-to-saucer
092D: C9              RET                         ; Done

;=============================================================
; Get number of ships for acive player
092E: CD 11 16        CALL    $1611               ; {code.GetPlayerDataPtr} HL points to player data
...
0935: CD 10 19        CALL    $1910               ; {code.CurPlyAlive} Get descriptor of sorts
...
093F: DB 02           IN      A,($02)             ; {hard.INP2} Read DIP settings
...
0963: CD 39 14        CALL    $1439               ; {code.DrawSimpSprite} Draw the sprite
0966: F1              POP     AF                  ; Restore the count
0967: 3C              INC     A                   ; +1
0968: CD 8B 1A        CALL    $1A8B               ; {} Print the number of ships
096B: CD 10 19        CALL    $1910               ; {code.CurPlyAlive} Get descriptor for active player of some sort
...
0974: 32 99 20        LD      ($2099),A           ; {ram.extraHold} ... for extra-ship sound
0977: 06 10           LD      B,$10               ; Make sound ...
0979: C3 FA 18        JP      $18FA               ; {code.SoundBits3On} ... for extra man
...
098B: 3A F1 20        LD      A,($20F1)           ; {ram.adjustScore} Does the score ...
098E: A7              AND     A                   ; ... need increasing?
098F: C8              RET     Z                   ; No ... done
0990: AF              XOR     A                   ; Mark score ...
0991: 32 F1 20        LD      ($20F1),A           ; {ram.adjustScore} ... as adjusted
0994: E5              PUSH    HL                  ; Hold the pointer to the structure
0995: 2A F2 20        LD      HL,($20F2)          ; {ram.scoreDeltaLSB} Get requested adjustment
...
09AA: C3 AD 09        JP      $09AD               ; {code.Print4Digits} ** Usually a good idea, but wasted here

Print4Digits:
; Print 4 digits in DE
09AD: 7A              LD      A,D                 ; Get first 2 digits of BCD or hex
09AE: CD B2 09        CALL    $09B2               ; {code.DrawHexByte} Print them
...
09C7: C3 FF 08        JP      $08FF               ; {code.DrawChar} Continue ...

; Get score descriptor for active player
09CA: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Get active player
...
09F3: 32 E9 20        LD      ($20E9),A           ; {ram.suspendPlay} ... ISR game tasks
09F6: CD D6 09        CALL    $09D6               ; {code.ClearPlayField} Clear playfield
09F9: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Hold current player number ...
09FC: F5              PUSH    AF                  ; ... on stack
09FD: CD E4 01        CALL    $01E4               ; {code.CopyRAMMirror} Block copy RAM mirror from ROM
0A00: F1              POP     AF                  ; Restore ...
0A01: 32 67 20        LD      ($2067),A           ; {ram.playerDataMSB} ... current player number
0A04: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} ** Why load this again? Nobody ever jumps to 0A04?
0A07: 67              LD      H,A                 ; To H
...
0A27: 32 98 20        LD      ($2098),A           ; {ram.soundPort5} ... first sound
0A2A: CD F5 01        CALL    $01F5               ; {code.DrawShieldPl2} Draw shields for player 2
0A2D: CD 04 19        CALL    $1904               ; {code.InitAliensP2} Initalize aliens for player 2
0A30: C3 04 08        JP      $0804               ; {} Continue at top of game loop
;
0A33: CD EF 01        CALL    $01EF               ; {code.DrawShieldPl1} Draw shields for player 1
0A36: CD C0 01        CALL    $01C0               ; {code.InitAliens} Initialize aliens for player 1
...
0A44: 32 C0 20        LD      ($20C0),A           ; {ram.isrDelay} Set ISR timer
0A47: 3A C0 20        LD      A,($20C0)           ; {ram.isrDelay} Has timer expired?
...
0A59: 3A 15 20        LD      A,($2015)           ; {ram.playerAlive} Active player hit flag
...
0A5F: 3A EF 20        LD      A,($20EF)           ; {ram.gameMode} Are we in ...
...
0A69: CD FA 18        CALL    $18FA               ; {code.SoundBits3On} Enable sound
0A6C: 41              LD      B,C                 ; Restore row number
0A6D: 78              LD      A,B                 ; Into A
0A6E: CD 7C 09        CALL    $097C               ; {code.AlienScoreValue} Look up the score for the alien
...
0A82: 32 C1 20        LD      ($20C1),A           ; {ram.isrSplashTask} ... sprite animation (splash)
0A85: D3 06           OUT     ($06),A             ; {hard.WATCHDOG} Feed watchdog
0A87: 3A CB 20        LD      A,($20CB)           ; {ram.splashReached} Has the ...
0A8A: A7              AND     A                   ; ... sprite reached target?
0A8B: CA 85 0A        JP      Z,$0A85             ; {} No ... wait
0A8E: AF              XOR     A                   ; Stop ...
0A8F: 32 C1 20        LD      ($20C1),A           ; {ram.isrSplashTask} ... ISR animation
0A92: C9              RET                         ; Done
...
0A95: CD FF 08        CALL    $08FF               ; {code.DrawChar} Draw character on screen
0A98: D1              POP     DE                  ; Preserve
0A99: 3E 07           LD      A,$07               ; Delay between letters
0A9B: 32 C0 20        LD      ($20C0),A           ; {ram.isrDelay} Set counter
0A9E: 3A C0 20        LD      A,($20C0)           ; {ram.isrDelay} Get counter
0AA1: 3D              DEC     A                   ; Is it 1?
0AA2: C2 9E 0A        JP      NZ,$0A9E            ; {} No ... wait on it
0AA5: 13              INC     DE                  ; Next in message
0AA6: 0D              DEC     C                   ; All done?
0AA7: C2 93 0A        JP      NZ,$0A93            ; {code.PrintMessageDel} No ... do all
0AAA: C9              RET                         ; Out
...
0AB3: C3 D7 0A        JP      $0AD7               ; {code.WaitOnDelay} Do delay

TwoSecDelay:
; Delay 128 interrupts
0AB6: 3E 80           LD      A,$80               ; Delay of 80 (tad over 2 sec)
0AB8: C3 D7 0A        JP      $0AD7               ; {code.WaitOnDelay} Do delay
...
;
0ABF: 3A C1 20        LD      A,($20C1)           ; {ram.isrSplashTask} Get the ISR task number
0AC2: 0F              RRCA                        ; In demo play mode?
0AC3: DA BB 0A        JP      C,$0ABB             ; {code.SplashDemo} 1: Yes ... go do game play (without sound)
0AC6: 0F              RRCA                        ; Moving little alien from point A to B?
0AC7: DA 68 18        JP      C,$1868             ; {code.SplashSprite} 2: Yes ... go move little alien from point A to B
0ACA: 0F              RRCA                        ; Shooting extra "C" with squiggly shot?
0ACB: DA AB 0A        JP      C,$0AAB             ; {code.SplashSquiggly} 4: Yes ... go shoot extra "C" in splash
...
0AD4: C3 93 0A        JP      $0A93               ; {code.PrintMessageDel} Print and out

WaitOnDelay:
; Wait on ISR counter to reach 0
0AD7: 32 C0 20        LD      ($20C0),A           ; {ram.isrDelay} Delay counter
0ADA: 3A C0 20        LD      A,($20C0)           ; {ram.isrDelay} Get current delay
...
0AE7: C3 32 1A        JP      $1A32               ; {code.BlockCopy} Block copy DE to descriptor

;=============================================================
; After initialization ... splash screens
0AEA: AF              XOR     A                   ; Make a 0
0AEB: D3 03           OUT     ($03),A             ; {hard.SOUND1} Turn off sound
0AED: D3 05           OUT     ($05),A             ; {hard.SOUND2} Turn off sound
0AEF: CD 82 19        CALL    $1982               ; {} Turn off ISR splash-task
0AF2: FB              EI                          ; Enable interrupts (using them for delays)
0AF3: CD B1 0A        CALL    $0AB1               ; {code.OneSecDelay} One second delay
0AF6: 3A EC 20        LD      A,($20EC)           ; {ram.splashAnimate} Splash screen type
...
0B05: CD 93 0A        CALL    $0A93               ; {code.PrintMessageDel} Print the "PLAy"
0B08: 11 AF 1D        LD      DE,$1DAF            ; "SPACE  INVADERS" message
0B0B: CD CF 0A        CALL    $0ACF               ; {} Print to middle-ish of screen
0B0E: CD B1 0A        CALL    $0AB1               ; {code.OneSecDelay} One second delay
0B11: CD 15 18        CALL    $1815               ; {code.DrawAdvTable} Draw "SCORE ADVANCE TABLE" with print delay
0B14: CD B6 0A        CALL    $0AB6               ; {code.TwoSecDelay} Two second delay
0B17: 3A EC 20        LD      A,($20EC)           ; {ram.splashAnimate} Do splash ...
...
0B21: CD E2 0A        CALL    $0AE2               ; {code.IniSplashAni} Copy to splash-animate structure
0B24: CD 80 0A        CALL    $0A80               ; {code.Animate} Wait for ISR to move sprite (small alien)
0B27: 11 B0 1B        LD      DE,$1BB0            ; Animate sprite from Y=98 to Y=FF step 1
0B2A: CD E2 0A        CALL    $0AE2               ; {code.IniSplashAni} Copy to splash-animate structure
0B2D: CD 80 0A        CALL    $0A80               ; {code.Animate} Wait for ISR to move sprite (alien pulling upside down Y)
0B30: CD B1 0A        CALL    $0AB1               ; {code.OneSecDelay} One second delay
0B33: 11 C9 1F        LD      DE,$1FC9            ; Animate sprite from Y=FF to Y=97 step 1
0B36: CD E2 0A        CALL    $0AE2               ; {code.IniSplashAni} Copy to splash-animate structure
0B39: CD 80 0A        CALL    $0A80               ; {code.Animate} Wait for ISR to move sprite (alien pushing Y)
0B3C: CD B1 0A        CALL    $0AB1               ; {code.OneSecDelay} One second delay
0B3F: 21 B7 33        LD      HL,$33B7            ; Where the splash alien ends up
0B42: 06 0A           LD      B,$0A               ; 10 rows
0B44: CD CB 14        CALL    $14CB               ; {code.ClearSmallSprite} Clear a one byte sprite at HL
0B47: CD B6 0A        CALL    $0AB6               ; {code.TwoSecDelay} Two second delay
;
; Play demo
0B4A: CD D6 09        CALL    $09D6               ; {code.ClearPlayField} Clear playfield
0B4D: 3A FF 21        LD      A,($21FF)           ; {ram.p1ShipsRem} Number of ships for player-1
0B50: A7              AND     A                   ; If non zero ...
0B51: C2 5D 0B        JP      NZ,$0B5D            ; {} ... keep it (counts down between demos)
0B54: CD D1 08        CALL    $08D1               ; {code.GetShipsPerCred} Get number of ships from DIP settings
0B57: 32 FF 21        LD      ($21FF),A           ; {ram.p1ShipsRem} Reset number of ships for player-1
0B5A: CD 7F 1A        CALL    $1A7F               ; {code.RemoveShip} Remove a ship from stash and update indicators
;
0B5D: CD E4 01        CALL    $01E4               ; {code.CopyRAMMirror} Block copy ROM mirror to initialize RAM
0B60: CD C0 01        CALL    $01C0               ; {code.InitAliens} Initialize all player 1 aliens
0B63: CD EF 01        CALL    $01EF               ; {code.DrawShieldPl1} Draw shields for player 1 (to buffer)
0B66: CD 1A 02        CALL    $021A               ; {code.RestoreShields1} Restore shields for player 1 (to screen)
0B69: 3E 01           LD      A,$01               ; ISR splash-task ...
0B6B: 32 C1 20        LD      ($20C1),A           ; {ram.isrSplashTask} ... playing demo
0B6E: CD CF 01        CALL    $01CF               ; {code.DrawBottomLine} Draw playfield line
;
0B71: CD 18 16        CALL    $1618               ; {code.PlrFireOrDemo} In demo ... process demo movement and always fire
0B74: CD F1 0B        CALL    $0BF1               ; {} Check player shot and aliens bumping edges of screen and hidden message
0B77: D3 06           OUT     ($06),A             ; {hard.WATCHDOG} Feed watchdog
0B79: CD 59 0A        CALL    $0A59               ; {} Has demo player been hit?
0B7C: CA 71 0B        JP      Z,$0B71             ; {} No ... continue game
0B7F: AF              XOR     A                   ; Remove player shot ...
0B80: 32 25 20        LD      ($2025),A           ; {ram.plyrShotStatus} ... from activity
0B83: CD 59 0A        CALL    $0A59               ; {} Wait for demo player ...
0B86: C2 83 0B        JP      NZ,$0B83            ; {} ... to stop exploding
;
; Credit information
0B89: AF              XOR     A                   ; Turn off ...
0B8A: 32 C1 20        LD      ($20C1),A           ; {ram.isrSplashTask} ... splash animation
0B8D: CD B1 0A        CALL    $0AB1               ; {code.OneSecDelay} One second delay
...
0B9B: CD F3 08        CALL    $08F3               ; {code.PrintMessage} Print message
0B9E: 3A EC 20        LD      A,($20EC)           ; {ram.splashAnimate} Do splash ...
...
0BAB: CD FF 08        CALL    $08FF               ; {code.DrawChar} Put an extra "C" for "CCOIN" on the screen
0BAE: 01 9C 1F        LD      BC,$1F9C            ; "<1 OR 2 PLAYERS>  "
0BB1: CD 56 18        CALL    $1856               ; {code.ReadPriStruct} Load the screen,pointer
0BB4: CD 4C 18        CALL    $184C               ; {} Print the message
0BB7: DB 02           IN      A,($02)             ; {hard.INP2} Display coin info (bit 7) ...
...
0BC3: CD B6 0A        CALL    $0AB6               ; {code.TwoSecDelay} Print TWO descriptors worth
0BC6: 3A EC 20        LD      A,($20EC)           ; {ram.splashAnimate} Doing splash ...
...
0BD1: CD E2 0A        CALL    $0AE2               ; {code.IniSplashAni} Copy the animation block
0BD4: CD 80 0A        CALL    $0A80               ; {code.Animate} Wait for the animation to complete
...
0BE2: CD D6 09        CALL    $09D6               ; {code.ClearPlayField} Clear play field
0BE5: C3 DF 18        JP      $18DF               ; {} Keep splashing

0BE8: 11 AB 1D        LD      DE,$1DAB            ; "PLAY" with normal 'Y'
0BEB: CD 93 0A        CALL    $0A93               ; {code.PrintMessageDel} Print it
0BEE: C3 0B 0B        JP      $0B0B               ; {} Continue with splash (HL will be pointing to next message)

0BF1: CD 0A 19        CALL    $190A               ; {code.PlyrShotAndBump} Check if player is shot and aliens bumping the edge of screen
0BF4: C3 9A 19        JP      $199A               ; {code.CheckHiddenMes} Check for hidden-message display sequence
...
1004: D3 03           OUT     ($03),A             ; {hard.SOUND1} ... sound effects (AMP ENABLE bit 5)
1006: 0E 03           LD      C,$03               ; 
1008: 06 55           LD      B,$55               ; 
...
10CA: 22 00 20        LD      ($2000),HL          ; {ram.waitOnDraw}
10CD: 3E 01           LD      A,$01               ; 
10CF: F5              PUSH    AF                  ; 
10D0: F6 20           OR      $20                 ; 
10D2: D3 03           OUT     ($03),A             ; {hard.SOUND1}
10D4: CD 05 11        CALL    $1105               ; {}
10D7: 3E 20           LD      A,$20               ; 
10D9: D3 03           OUT     ($03),A             ; {hard.SOUND1}
...
10E5: 3E 01           LD      A,$01               ; 
10E7: F5              PUSH    AF                  ; 
10E8: D3 05           OUT     ($05),A             ; {hard.SOUND2}
10EA: CD 05 11        CALL    $1105               ; {}
10ED: AF              XOR     A                   ; 
10EE: D3 05           OUT     ($05),A             ; {hard.SOUND2}
...
1105: 2A 00 20        LD      HL,($2000)          ; {ram.waitOnDraw}
1108: 3E 0F           LD      A,$0F               ; 
110A: CD 66 11        CALL    $1166               ; {}
110D: C3 1F 11        JP      $111F               ; {}
1110: CD 1F 11        CALL    $111F               ; {}
1113: 2A 00 20        LD      HL,($2000)          ; {ram.waitOnDraw}
1116: 3E 28           LD      A,$28               ; 
1118: CD 66 11        CALL    $1166               ; {}
111B: 22 00 20        LD      ($2000),HL          ; {ram.waitOnDraw}
111E: C9              RET                         ; 
111F: 3E 60           LD      A,$60               ; 
...
112D: DB 01           IN      A,($01)             ; {hard.INP1}
112F: CD 3C 11        CALL    $113C               ; {}
1132: 21 0E 2F        LD      HL,$2F0E            ; 
1135: DB 02           IN      A,($02)             ; {hard.INP2}
...
11C9: D3 06           OUT     ($06),A             ; {hard.WATCHDOG}
11CB: C3 B4 11        JP      $11B4               ; {}
11CE: CD 29 11        CALL    $1129               ; {}
11D1: D3 06           OUT     ($06),A             ; {hard.WATCHDOG}
11D3: C9              RET                         ; 
11D4: FF              RST     0X38                ; 
11D5: FF              RST     0X38                ; 
11D6: FF              RST     0X38                ; 
...
13F7: 00              NOP                         ; 
13F8: 22 22 22        LD      ($2222),HL          ; {ram.+22}
13FB: 3E 22           LD      A,$22               ; 
...
1401: CD 74 14        CALL    $1474               ; {code.CnvtPixNumber} Convert pixel number to coord and shift
...
1408: D3 04           OUT     ($04),A             ; {hard.SHFT_DATA} Store in shift register
140A: DB 03           IN      A,($03)             ; {hard.SHFT_IN} Read the shifted pixels
...
1411: D3 04           OUT     ($04),A             ; {hard.SHFT_DATA} ... to next byte in register (shift in 0)
1413: DB 03           IN      A,($03)             ; {hard.SHFT_IN} Read the shifted pixels
...
1424: CD 74 14        CALL    $1474               ; {code.CnvtPixNumber} Convert pixel number in HL
...
1443: C2 39 14        JP      NZ,$1439            ; {code.DrawSimpSprite} Do all
1446: C9              RET                         ; Out
...
1452: CD 74 14        CALL    $1474               ; {code.CnvtPixNumber} Convert pixel number in HL to coorinates with shift
1455: C5              PUSH    BC                  ; Hold BC
1456: E5              PUSH    HL                  ; Hold coordinate
1457: 1A              LD      A,(DE)              ; Get picture value
1458: D3 04           OUT     ($04),A             ; {hard.SHFT_DATA} Value into shift register
145A: DB 03           IN      A,($03)             ; {hard.SHFT_IN} Read shifted sprite picture
...
1462: D3 04           OUT     ($04),A             ; {hard.SHFT_DATA} ... 8 bits (shift in 0)
1464: DB 03           IN      A,($03)             ; {hard.SHFT_IN} Read 2nd byte of image
...
1477: D3 02           OUT     ($02),A             ; {hard.SHFTAMNT} Write shift amount to hardware
1479: C3 47 1A        JP      $1A47               ; {code.ConvToScr} HL = HL/8 + 2000 (screen coordinate)
...
148D: C2 7C 14        JP      NZ,$147C            ; {code.RememberShields} No ... do multi rows
1490: C9              RET                         ; Done

DrawSprCollision:
1491: CD 74 14        CALL    $1474               ; {code.CnvtPixNumber} Convert pixel number to coord and shift
1494: AF              XOR     A                   ; Clear the ...
1495: 32 61 20        LD      ($2061),A           ; {ram.collision} ... collision-detection flag
1498: C5              PUSH    BC                  ; Hold count
1499: E5              PUSH    HL                  ; Hold screen
149A: 1A              LD      A,(DE)              ; Get byte
149B: D3 04           OUT     ($04),A             ; {hard.SHFT_DATA} Write first byte to shift register
149D: DB 03           IN      A,($03)             ; {hard.SHFT_IN} Read shifted pattern
...
14A6: 32 61 20        LD      ($2061),A           ; {ram.collision} ... collision flag
...
14AF: D3 04           OUT     ($04),A             ; {hard.SHFT_DATA} ... to shift register
14B1: DB 03           IN      A,($03)             ; {hard.SHFT_IN} Read 2nd half of shifted sprite
...
14BA: 32 61 20        LD      ($2061),A           ; {ram.collision} ... collision flag
...
;
14D8: 3A 25 20        LD      A,($2025)           ; {ram.plyrShotStatus} Player shot flag
...
;
14E1: 3A 29 20        LD      A,($2029)           ; {ram.obj1CoorYr} Get Yr coordinate of player shot
...
14EA: 3A 02 20        LD      A,($2002)           ; {ram.alienIsExploding} Is an alien ...
...
14F8: 3A 09 20        LD      A,($2009)           ; {ram.refAlienYr} Ref alien Y coordianate
...
14FD: D2 04 15        JP      NC,$1504            ; {code.CodeBug1} ... aliens are down in the shields
...
1505: CD 62 15        CALL    $1562               ; {code.FindRow} Look up row number to B
1508: 3A 2A 20        LD      A,($202A)           ; {ram.obj1CoorXr} Player's shot's Xr coordinate ...
150B: 67              LD      H,A                 ; ... to H
150C: CD 6F 15        CALL    $156F               ; {code.FindColumn} Get alien's coordinate
150F: 22 64 20        LD      ($2064),HL          ; {ram.expAlienYr} Put it in the exploding-alien descriptor
1512: 3E 05           LD      A,$05               ; Flag alien explosion ...
1514: 32 25 20        LD      ($2025),A           ; {ram.plyrShotStatus} ... in progress
1517: CD 81 15        CALL    $1581               ; {code.GetAlienStatPtr} Get descriptor for alien
...
1521: CD 5F 0A        CALL    $0A5F               ; {code.ScoreForAlien} Makes alien explosion sound and adjust score
1524: CD 3B 1A        CALL    $1A3B               ; {code.ReadDesc} Load 5 byte sprite descriptor
1527: CD D3 15        CALL    $15D3               ; {code.DrawSprite} Draw explosion sprite on screen
152A: 3E 10           LD      A,$10               ; Initiate alien-explosion
152C: 32 03 20        LD      ($2003),A           ; {ram.expAlienTimer} ... timer to 16
152F: C9              RET                         ; Out
;
; Player shot leaving playfield, hitting shield, or hitting an alien shot
1530: 3E 03           LD      A,$03               ; Mark ...
1532: 32 25 20        LD      ($2025),A           ; {ram.plyrShotStatus} ... player shot hit something other than alien
...
153D: 2A 64 20        LD      HL,($2064)          ; {ram.expAlienYr} Pixel pointer for exploding alien
1540: 06 10           LD      B,$10               ; 16 row pixel
1542: CD 24 14        CALL    $1424               ; {code.EraseSimpleSprite} Clear the explosion sprite from the screen
1545: 3E 04           LD      A,$04               ; 4 means that ...
1547: 32 25 20        LD      ($2025),A           ; {ram.plyrShotStatus} ... alien has exploded (remove from active duty)
;
154A: AF              XOR     A                   ; Turn off ...
154B: 32 02 20        LD      ($2002),A           ; {ram.alienIsExploding} ... alien-is-blowing-up flag
154E: 06 F7           LD      B,$F7               ; Turn off ...
1550: C3 DC 19        JP      $19DC               ; {code.SoundBits3Off} ... alien exploding sound
...
1557: D4 90 15        CALL    NC,$1590            ; {code.WrapRef} If reference is greater or equal then do something questionable ... see below
...
;
1562: 3A 09 20        LD      A,($2009)           ; {ram.refAlienYr} Reference alien Yr coordinate
1565: 65              LD      H,L                 ; Target Yr coordinate to H
1566: CD 54 15        CALL    $1554               ; {code.Cnt16s} Count 16s needed to bring ref alien to target
...
;
156F: 3A 0A 20        LD      A,($200A)           ; {ram.refAlienXr} Reference alien Yn coordinate
1572: CD 54 15        CALL    $1554               ; {code.Cnt16s} Count 16s to bring Y to target Y
1575: DE 10           SBC     A,$10               ; Subtract off extra 16
1577: 67              LD      H,A                 ; To H
1578: C9              RET                         ; Done

1579: 3E 01           LD      A,$01               ; Mark flying ...
157B: 32 85 20        LD      ($2085),A           ; {ram.saucerHit} ... saucer has been hit
...
158B: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Set ...
...
1593: FA 90 15        JP      M,$1590             ; {code.WrapRef} Keep going till result is positive
1596: C9              RET                         ; Out
...
1597: 3A 0D 20        LD      A,($200D)           ; {ram.rackDirection} Get rack direction
...
;
15A9: 32 0D 20        LD      ($200D),A           ; {ram.rackDirection} Set new rack direction
15AC: 78              LD      A,B                 ; B has delta X
15AD: 32 08 20        LD      ($2008),A           ; {ram.refAlienDXr} Set new delta X
15B0: 3A 0E 20        LD      A,($200E)           ; {ram.rackDownDelta} Set delta Y ...
15B3: 32 07 20        LD      ($2007),A           ; {ram.refAlienDYr} ... to drop rack by 8
15B6: C9              RET                         ; Done
;
...
15D3: CD 74 14        CALL    $1474               ; {code.CnvtPixNumber} Convert pixel number to screen/shift
...
15DA: D3 04           OUT     ($04),A             ; {hard.SHFT_DATA} Write data to shift register
15DC: DB 03           IN      A,($03)             ; {hard.SHFT_IN} Read back shifted amount
15DE: 77              LD      (HL),A              ; Shifted sprite to screen
15DF: 23              INC     HL                  ; Adjacent cell
15E0: 13              INC     DE                  ; Next in sprite data
15E1: AF              XOR     A                   ; 0
15E2: D3 04           OUT     ($04),A             ; {hard.SHFT_DATA} Write 0 to shift register
15E4: DB 03           IN      A,($03)             ; {hard.SHFT_IN} Read back remainder of previous
...
15F3: CD 11 16        CALL    $1611               ; {code.GetPlayerDataPtr} Get active player descriptor
...
1605: 32 82 20        LD      ($2082),A           ; {ram.numAliens} Hold it
...
1613: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Active player number
...
1618: 3A 15 20        LD      A,($2015)           ; {ram.playerAlive} Is there an active player?
...
1626: 3A 25 20        LD      A,($2025)           ; {ram.plyrShotStatus} Does the player have ...
1629: A7              AND     A                   ; ... a shot on the screen?
162A: C0              RET     NZ                  ; Yes ... ignore
162B: 3A EF 20        LD      A,($20EF)           ; {ram.gameMode} Are we in ...
162E: A7              AND     A                   ; ... game mode?
162F: CA 52 16        JP      Z,$1652             ; {} No ... in demo mode ... constant firing in demo
1632: 3A 2D 20        LD      A,($202D)           ; {ram.fireBounce} Is fire button ...
1635: A7              AND     A                   ; ... being held down?
1636: C2 48 16        JP      NZ,$1648            ; {} Yes ... wait for bounce
1639: CD C0 17        CALL    $17C0               ; {code.ReadInputs} Read active player controls
163C: E6 10           AND     $10                 ; Fire-button pressed?
163E: C8              RET     Z                   ; No ... out
163F: 3E 01           LD      A,$01               ; Flag
1641: 32 25 20        LD      ($2025),A           ; {ram.plyrShotStatus} Flag shot active
1644: 32 2D 20        LD      ($202D),A           ; {ram.fireBounce} Flag that fire button is down
1647: C9              RET                         ; Out
1648: CD C0 17        CALL    $17C0               ; {code.ReadInputs} Read active player controls
164B: E6 10           AND     $10                 ; Fire-button pressed?
164D: C0              RET     NZ                  ; Yes ... ignore
164E: 32 2D 20        LD      ($202D),A           ; {ram.fireBounce} Else ... clear flag
1651: C9              RET                         ; Out
; Handle demo (constant fire, parse demo commands)
1652: 21 25 20        LD      HL,$2025            ; Demo fires ...
1655: 36 01           LD      (HL),$01            ; ... constantly
1657: 2A ED 20        LD      HL,($20ED)          ; {ram.demoCmdPtrLSB} Demo command bufer
...
1663: 22 ED 20        LD      ($20ED),HL          ; {ram.demoCmdPtrLSB} Next demo command
1666: 7E              LD      A,(HL)              ; Get next command
1667: 32 1D 20        LD      ($201D),A           ; {ram.nextDemoCmd} Set command for movement
166A: C9              RET                         ; Done
...
1671: CD 10 19        CALL    $1910               ; {code.CurPlyAlive} Get active-flag ptr for current player
...
1695: CD 50 19        CALL    $1950               ; {code.PrintHiScore} Draw the new high score
1698: 3A CE 20        LD      A,($20CE)           ; {ram.twoPlayers} Number of players
...
16A7: CD 93 0A        CALL    $0A93               ; {code.PrintMessageDel} Print message
16AA: 25              DEC     H                   ; Back up ...
16AB: 25              DEC     H                   ; ... to player indicator
16AC: 06 1B           LD      B,$1B               ; "1"
16AE: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Player number
16B1: 0F              RRCA                        ; Is this player 1?
16B2: DA B7 16        JP      C,$16B7             ; {} Yes ... keep the digit
16B5: 06 1C           LD      B,$1C               ; Else ... set digit 2
16B7: 78              LD      A,B                 ; To A
16B8: CD FF 08        CALL    $08FF               ; {code.DrawChar} Print player number
16BB: CD B1 0A        CALL    $0AB1               ; {code.OneSecDelay} Short delay
...
16D1: CD 93 0A        CALL    $0A93               ; {code.PrintMessageDel} Print message
16D4: CD B6 0A        CALL    $0AB6               ; {code.TwoSecDelay} Long delay
16D7: CD D6 09        CALL    $09D6               ; {code.ClearPlayField} Clear center window
16DA: AF              XOR     A                   ; Now in ...
16DB: 32 EF 20        LD      ($20EF),A           ; {ram.gameMode} ... demo mode
16DE: D3 05           OUT     ($05),A             ; {hard.SOUND2} All sound off
16E0: CD D1 19        CALL    $19D1               ; {code.EnableGameTasks} Enable ISR game tasks
...
16EA: AF              XOR     A                   ; Flag ...
16EB: 32 15 20        LD      ($2015),A           ; {ram.playerAlive} ... player is shot
16EE: CD D8 14        CALL    $14D8               ; {code.PlayerShotHit} Player's shot collision detection
16F1: 06 04           LD      B,$04               ; Player has been hit ...
16F3: CD FA 18        CALL    $18FA               ; {code.SoundBits3On} ... sound
16F6: CD 59 0A        CALL    $0A59               ; {} Has flag been set?
16F9: C2 EE 16        JP      NZ,$16EE            ; {} No ... wait for the flag
16FC: CD D7 19        CALL    $19D7               ; {code.DsableGameTasks} Disable ISR game tasks
...
1728: 32 CF 20        LD      ($20CF),A           ; {ram.aShotReloadRate} Save the value for use in shot routine
172B: C9              RET                         ; Done

; Shot sound on or off depending on 2025
ShotSound:
172C: 3A 25 20        LD      A,($2025)           ; {ram.plyrShotStatus} Player shot flag
172F: FE 00           CP      $00                 ; Active shot?
1731: C2 39 17        JP      NZ,$1739            ; {} Yes ... go
1734: 06 FD           LD      B,$FD               ; Sound mask
1736: C3 DC 19        JP      $19DC               ; {code.SoundBits3Off} Mask off sound
;
1739: 06 02           LD      B,$02               ; Sound bit
173B: C3 FA 18        JP      $18FA               ; {code.SoundBits3On} OR on sound
...
1747: 3A 68 20        LD      A,($2068)           ; {ram.playerOK} Is player OK?
...
1757: D3 05           OUT     ($05),A             ; {hard.SOUND2} Set sounds
1759: 3A 82 20        LD      A,($2082)           ; {ram.numAliens} Number of aliens on active screen
...
1769: 32 9B 20        LD      ($209B),A           ; {ram.fleetSndHold} ... time for fleet sound
176C: C9              RET                         ; Done

176D: 3A 98 20        LD      A,($2098)           ; {ram.soundPort5} Current sound port 3 value
1770: E6 30           AND     $30                 ; Mask off fleet movement sounds
1772: D3 05           OUT     ($05),A             ; {hard.SOUND2} Set sounds
1774: C9              RET                         ; Out
...
;
1775: 3A 95 20        LD      A,($2095)           ; {ram.changeFleetSnd} Time for new ...
...
1782: 3A 82 20        LD      A,($2082)           ; {ram.numAliens} Get the number of aliens on the screen
...
178F: 32 97 20        LD      ($2097),A           ; {ram.fleetSndReload} Store the new alien sound delay
...
17A7: 32 95 20        LD      ($2095),A           ; {ram.changeFleetSnd} ... waiting on fleet time
;
...
17B1: C3 DC 19        JP      $19DC               ; {code.SoundBits3Off} Stop sound and out
...
17BC: D3 05           OUT     ($05),A             ; {hard.SOUND2} Turn off extended play
17BE: C9              RET                         ; Out
...
17C0: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Get active player
17C3: 0F              RRCA                        ; Test player
17C4: D2 CA 17        JP      NC,$17CA            ; {} Player 2 ... read port 2
17C7: DB 01           IN      A,($01)             ; {hard.INP1} Player 1 ... read port 1
17C9: C9              RET                         ; Out
17CA: DB 02           IN      A,($02)             ; {hard.INP2} Get controls for player 2
17CC: C9              RET                         ; Out

; Check and handle TILT
CheckHandleTilt:
17CD: DB 02           IN      A,($02)             ; {hard.INP2} Read input port
17CF: E6 04           AND     $04                 ; Tilt?
17D1: C8              RET     Z                   ; No tilt ... return
17D2: 3A 9A 20        LD      A,($209A)           ; {ram.tilt} Already in TILT handle?
17D5: A7              AND     A                   ; 1 = yes
17D6: C0              RET     NZ                  ; Yes ... ignore it now
17D7: 31 00 24        LD      SP,$2400            ; Reset stack
17DA: 06 04           LD      B,$04               ; Do this 4 times
17DC: CD D6 09        CALL    $09D6               ; {code.ClearPlayField} Clear center window
17DF: 05              DEC     B                   ; All done?
17E0: C2 DC 17        JP      NZ,$17DC            ; {} No ... do again
17E3: 3E 01           LD      A,$01               ; Flag ...
17E5: 32 9A 20        LD      ($209A),A           ; {ram.tilt} ... handling TILT
17E8: CD D7 19        CALL    $19D7               ; {code.DsableGameTasks} Disable game tasks
...
17F4: CD 93 0A        CALL    $0A93               ; {code.PrintMessageDel} Print "TILT"
17F7: CD B1 0A        CALL    $0AB1               ; {code.OneSecDelay} Short delay
17FA: AF              XOR     A                   ; Zero
17FB: 32 9A 20        LD      ($209A),A           ; {ram.tilt} TILT handle over
17FE: 32 93 20        LD      ($2093),A           ; {ram.waitStartLoop} Back into splash screens
...
1812: C3 FA 18        JP      $18FA               ; {code.SoundBits3On} ... sound (retrigger makes it warble?)
...
181D: CD F3 08        CALL    $08F3               ; {code.PrintMessage} Print message
1820: 3E 0A           LD      A,$0A               ; 10 bytes in every "=xx POINTS" string
1822: 32 6C 20        LD      ($206C),A           ; {ram.temp206C} Hold the count
1825: 01 BE 1D        LD      BC,$1DBE            ; Coordinate/sprite for drawing table
1828: CD 56 18        CALL    $1856               ; {code.ReadPriStruct} Get HL=coordinate, DE=image
182B: DA 37 18        JP      C,$1837             ; {} Move on if done
182E: CD 44 18        CALL    $1844               ; {} Draw 16-byte sprite
1831: C3 28 18        JP      $1828               ; {} Do all in table
;
1834: CD B1 0A        CALL    $0AB1               ; {code.OneSecDelay} One second delay
1837: 01 CF 1D        LD      BC,$1DCF            ; Coordinate/message for drawing table
183A: CD 56 18        CALL    $1856               ; {code.ReadPriStruct} Get HL=coordinate, DE=message
...
1847: CD 39 14        CALL    $1439               ; {code.DrawSimpSprite} Draw simple
184A: C1              POP     BC                  ; Restore BC
184B: C9              RET                         ; Out
;
184C: C5              PUSH    BC                  ; Hold BC
184D: 3A 6C 20        LD      A,($206C)           ; {ram.temp206C} Count of 10 ...
1850: 4F              LD      C,A                 ; ... to C
1851: CD 93 0A        CALL    $0A93               ; {code.PrintMessageDel} Print the message with delay between letters
...
186E: CD D9 01        CALL    $01D9               ; {code.AddDelta} Add delta-X and delta-Y to X and Y
1871: 47              LD      B,A                 ; Current y coordinate
1872: 3A CA 20        LD      A,($20CA)           ; {ram.splashTargetY} Has sprite reached ...
1875: B8              CP      B                   ; ... target coordinate?
1876: CA 98 18        JP      Z,$1898             ; {} Yes ... flag and out
1879: 3A C2 20        LD      A,($20C2)           ; {ram.splashAnForm} Image number
187C: E6 04           AND     $04                 ; Watching bit 3 for flip delay
187E: 2A CC 20        LD      HL,($20CC)          ; {ram.splashImRestLSB} Image
...
1888: 22 C7 20        LD      ($20C7),HL          ; {ram.splashImageLSB} Image to descriptor structure
188B: 21 C5 20        LD      HL,$20C5            ; X,Y,Image descriptor
188E: CD 3B 1A        CALL    $1A3B               ; {code.ReadDesc} Read sprite descriptor
1891: EB              EX      DE,HL               ; Image to DE, position to HL
1892: C3 D3 15        JP      $15D3               ; {code.DrawSprite} Draw the sprite

1895: 00 00 00                          

1898: 3E 01           LD      A,$01               ; Flag that sprite ...
189A: 32 CB 20        LD      ($20CB),A           ; {ram.splashReached} ... reached location
189D: C9              RET                         ; Out
...
18A6: CD 32 1A        CALL    $1A32               ; {code.BlockCopy} ... 16 bytes
18A9: 3E 02           LD      A,$02               ; Set shot sync ...
18AB: 32 80 20        LD      ($2080),A           ; {ram.shotSync} ... to run the squiggly shot
18AE: 3E FF           LD      A,$FF               ; Shot direction (-1)
18B0: 32 7E 20        LD      ($207E),A           ; {ram.alienShotDelta} Alien shot delta
18B3: 3E 04           LD      A,$04               ; Animate ...
18B5: 32 C1 20        LD      ($20C1),A           ; {ram.isrSplashTask} ... shot
18B8: 3A 55 20        LD      A,($2055)           ; {ram.squShotStatus} Has shot ...
18BB: E6 01           AND     $01                 ; ... collided?
18BD: CA B8 18        JP      Z,$18B8             ; {} No ... keep waiting
18C0: 3A 55 20        LD      A,($2055)           ; {ram.squShotStatus} Wait ...
...
18CD: 00              NOP                         ; ** Why?
18CE: CD FF 08        CALL    $08FF               ; {code.DrawChar} Draw character
18D1: C3 B6 0A        JP      $0AB6               ; {code.TwoSecDelay} Two second delay and out
...
18DC: CD 56 19        CALL    $1956               ; {code.DrawStatus} Print scores and credits
;
18DF: 3E 08           LD      A,$08               ; Set alien ...
18E1: 32 CF 20        LD      ($20CF),A           ; {ram.aShotReloadRate} ... shot reload rate
18E4: C3 EA 0A        JP      $0AEA               ; {} Top of splash screen loop

; Get player-alive flag for OTHER player
18E7: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Player data MSB
...
18F3: 3A 82 20        LD      A,($2082)           ; {ram.numAliens} Number of aliens on screen
...
18FA: 3A 94 20        LD      A,($2094)           ; {ram.soundPort3} Current value of sound port
18FD: B0              OR      B                   ; Add in new sounds
18FE: 32 94 20        LD      ($2094),A           ; {ram.soundPort3} New value of sound port
1901: D3 03           OUT     ($03),A             ; {hard.SOUND1} Write new value to sound hardware
1903: C9              RET                         
...
PlyrShotAndBump:
190A: CD D8 14        CALL    $14D8               ; {code.PlayerShotHit} Player's shot collision detection
190D: C3 97 15        JP      $1597               ; {code.RackBump} Change alien deltaX and deltaY when rack bumps edges

CurPlyAlive:
; Get the current player's alive status
1910: 21 E7 20        LD      HL,$20E7            ; Alive flags
1913: 3A 67 20        LD      A,($2067)           ; {ram.playerDataMSB} Player 1 or 2
...
1922: C3 F3 08        JP      $08F3               ; {code.PrintMessage} Print score header

1925: 21 F8 20        LD      HL,$20F8            ; Player 1 score descriptor
1928: C3 31 19        JP      $1931               ; {code.DrawScore} Print score

192B: 21 FC 20        LD      HL,$20FC            ; Player 2 score descriptor
192E: C3 31 19        JP      $1931               ; {code.DrawScore} Print score
...
1939: C3 AD 09        JP      $09AD               ; {code.Print4Digits} Print 4 digits in DE
...
1944: C3 F3 08        JP      $08F3               ; {code.PrintMessage} Print message

DrawNumCredits:
; Display number of credits on screen
1947: 3A EB 20        LD      A,($20EB)           ; {ram.numCoins} Number of credits
194A: 21 01 3C        LD      HL,$3C01            ; Screen coordinates
194D: C3 B2 09        JP      $09B2               ; {code.DrawHexByte} Character to screen

PrintHiScore:
1950: 21 F4 20        LD      HL,$20F4            ; Hi Score descriptor
1953: C3 31 19        JP      $1931               ; {code.DrawScore} Print Hi-Score

DrawStatus:
; Print scores (with header) and credits (with label)
1956: CD 5C 1A        CALL    $1A5C               ; {code.ClearScreen} Clear the screen
1959: CD 1A 19        CALL    $191A               ; {code.DrawScoreHead} Print score header
195C: CD 25 19        CALL    $1925               ; {} Print player 1 score
195F: CD 2B 19        CALL    $192B               ; {} Print player 2 score
1962: CD 50 19        CALL    $1950               ; {code.PrintHiScore} Print hi score
1965: CD 3C 19        CALL    $193C               ; {} Print credit lable
1968: C3 47 19        JP      $1947               ; {code.DrawNumCredits} Number of credits

196B: CD DC 19        CALL    $19DC               ; {code.SoundBits3Off} From 170B with B=FB. Turn off player shot sound
196E: C3 71 16        JP      $1671               ; {} Update high-score if player's score is greater

1971: 3E 01           LD      A,$01               ; Set flag that ...
1973: 32 6D 20        LD      ($206D),A           ; {ram.invaded} ... aliens reached bottom of screen
1976: C3 E6 16        JP      $16E6               ; {} End of round

1979: CD D7 19        CALL    $19D7               ; {code.DsableGameTasks} Disable ISR game tasks
197C: CD 47 19        CALL    $1947               ; {code.DrawNumCredits} Display number of credits on screen
197F: C3 3C 19        JP      $193C               ; {} Print message "CREDIT"

1982: 32 C1 20        LD      ($20C1),A           ; {ram.isrSplashTask} Set ISR splash task
1985: C9              RET                         ; Done
...
1988: C3 D6 09        JP      $09D6               ; {code.ClearPlayField} Clear playfield and out
...
1993: C3 F3 08        JP      $08F3               ; {code.PrintMessage} Print message
...
;
199A: 3A 1E 20        LD      A,($201E)           ; {ram.hidMessSeq} Has the 1st "hidden-message" sequence ...
199D: A7              AND     A                   ; ... been registered?
199E: C2 AC 19        JP      NZ,$19AC            ; {} Yes ... go look for the 2nd sequence
19A1: DB 01           IN      A,($01)             ; {hard.INP1} Get player inputs
...
19A9: 32 1E 20        LD      ($201E),A           ; {ram.hidMessSeq} ... has been entered
19AC: DB 01           IN      A,($01)             ; {hard.INP1} Check inputs for 2nd sequence
...
19BB: C3 F3 08        JP      $08F3               ; {code.PrintMessage} Print message and out
...
19D3: 32 E9 20        LD      ($20E9),A           ; {ram.suspendPlay} ... game tasks enabled
19D6: C9              RET                         ; Done
...
19DC: 3A 94 20        LD      A,($2094)           ; {ram.soundPort3} Current sound effects value
19DF: A0              AND     B                   ; Mask bits off
19E0: 32 94 20        LD      ($2094),A           ; {ram.soundPort3} Store new hold value
19E3: D3 03           OUT     ($03),A             ; {hard.SOUND1} Change sounds
19E5: C9              RET                         ; Done
...
19F2: CD 39 14        CALL    $1439               ; {code.DrawSimpSprite} Display 1byte sprite to screen
...
19FC: CD CB 14        CALL    $14CB               ; {code.ClearSmallSprite} Clear 1byte sprite at HL
...
1A37: C2 32 1A        JP      NZ,$1A32            ; {code.BlockCopy} Do all
1A3A: C9              RET                         ; Done
...
1A7B: C2 69 1A        JP      NZ,$1A69            ; {code.RestoreShields} Do all rows
1A7E: C9              RET                         
...
1A87: CD E6 19        CALL    $19E6               ; {code.DrawNumShips} Draw the line of ships
