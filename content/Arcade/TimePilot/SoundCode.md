...
0008: 32 00 50        LD      ($5000),A           ; {hard.AY0_ADDR} Store the register number
000B: 3A 00 40        LD      A,($4000)           ; {hard.AY0_DATA} Read the value
000E: C9              RET                         ; Done
000F: FF              RST     $38                 ; Filler to align RST routines

RST10:
; Read CHIP1 register. A=register, retun in A
0010: 32 00 70        LD      ($7000),A           ; {hard.AY1_ADDR} Store the register number
0013: 3A 00 60        LD      A,($6000)           ; {hard.AY1_DATA} Read the value
0016: C9              RET                         ; Done
0017: FF              RST     $38                 ; Filler to align RST routines

RST18:
; Write CHIP0 register. A=register, C=value
0018: 32 00 50        LD      ($5000),A           ; {hard.AY0_ADDR} Store register number
001B: 79              LD      A,C                 ; Write ...
001C: 32 00 40        LD      ($4000),A           ; {hard.AY0_DATA} ... value
001F: C9              RET                         ; Done

RST20:
; Write CHIP1 register. A=register, C=value
0020: 32 00 70        LD      ($7000),A           ; {hard.AY1_ADDR} Store register number
0023: 79              LD      A,C                 ; Write ...
0024: 32 00 60        LD      ($6000),A           ; {hard.AY1_DATA} ... value
0027: C9              RET                         ; Done
...
003A: CD 40 00        CALL    $0040               ; {code.HandleCommand} Make the call
...
0044: 28 2F           JR      Z,$75               ; {code.StopAllCommands} ... clear the entire buffer and out
...
0050: CD 80 00        CALL    $0080               ; {code.FindCommand} Is the command already in the buffer?
0053: 20 1C           JR      NZ,$71              ; {} Found ... restart it.
0055: CD 80 00        CALL    $0080               ; {code.FindCommand} Find A=0 (empty slot) in the buffer
...
006D: CD 80 00        CALL    $0080               ; {code.FindCommand} Find entry
...
0092: CD 80 00        CALL    $0080               ; {code.FindCommand} Find entry
...
00A8: 22 0C 30        LD      ($300C),HL          ; {ram.currentCaps} Keep up with write-only value
...
00CD: 3A 01 30        LD      A,($3001)           ; {ram.commandBuffer+1} First voice, 2nd byte
00D0: B7              OR      A                   ; Set flags about 2nd byte
00D1: 3A 00 30        LD      A,($3000)           ; {ram.commandBuffer} Get command (no flags)
00D4: 28 06           JR      Z,$DC               ; {} 2nd byte == 0
00D6: CD 7F 01        CALL    $017F               ; {code.ContinueCommand} Continue command
00D9: C3 DF 00        JP      $00DF               ; {} Skip over
00DC: CD 69 01        CALL    $0169               ; {code.InitCommand} Initialize command
...
00E8: 3A 03 30        LD      A,($3003)           ; {ram.commandBuffer+3}
00EB: B7              OR      A                   ; 
00EC: 3A 02 30        LD      A,($3002)           ; {ram.commandBuffer+2}
00EF: 28 06           JR      Z,$F7               ; {} Same as index 1
00F1: CD 7F 01        CALL    $017F               ; {code.ContinueCommand}
00F4: C3 FA 00        JP      $00FA               ; {}
00F7: CD 69 01        CALL    $0169               ; {code.InitCommand}
00FA: FB              EI                          ; 
00FB: 00              NOP                         ; 
00FC: 00              NOP                         ; 
...
0103: 3A 05 30        LD      A,($3005)           ; {ram.commandBuffer+5}
0106: B7              OR      A                   ; 
0107: 3A 04 30        LD      A,($3004)           ; {ram.commandBuffer+4}
010A: 28 06           JR      Z,$112              ; {} Same as index 1
010C: CD 7F 01        CALL    $017F               ; {code.ContinueCommand}
010F: C3 15 01        JP      $0115               ; {}
0112: CD 69 01        CALL    $0169               ; {code.InitCommand}
0115: FB              EI                          ; 
0116: 00              NOP                         ; 
0117: 00              NOP                         ; 
...
011E: 3A 07 30        LD      A,($3007)           ; {ram.commandBuffer+7}
0121: B7              OR      A                   ; 
0122: 3A 06 30        LD      A,($3006)           ; {ram.commandBuffer+6}
0125: 28 06           JR      Z,$12D              ; {}
0127: CD 7F 01        CALL    $017F               ; {code.ContinueCommand} Same as index 1
012A: C3 30 01        JP      $0130               ; {}
012D: CD 69 01        CALL    $0169               ; {code.InitCommand}
0130: FB              EI                          ; 
0131: 00              NOP                         ; 
0132: 00              NOP                         ; 
...
0139: 3A 09 30        LD      A,($3009)           ; {ram.commandBuffer+9}
013C: B7              OR      A                   ; 
013D: 3A 08 30        LD      A,($3008)           ; {ram.commandBuffer+8}
0140: 28 06           JR      Z,$148              ; {} Same as index 1
0142: CD 7F 01        CALL    $017F               ; {code.ContinueCommand}
0145: C3 4B 01        JP      $014B               ; {}
0148: CD 69 01        CALL    $0169               ; {code.InitCommand}
014B: FB              EI                          ; 
014C: 00              NOP                         ; 
014D: 00              NOP                         ; 
...
0154: 3A 0B 30        LD      A,($300B)           ; {ram.commandBuffer+B}
0157: B7              OR      A                   ; 
0158: 3A 0A 30        LD      A,($300A)           ; {ram.commandBuffer+A}
015B: 28 06           JR      Z,$163              ; {} Same as index 1
015D: CD 7F 01        CALL    $017F               ; {code.ContinueCommand}
0160: C3 BF 00        JP      $00BF               ; {code.MainLoop} Back to the main loop
0163: CD 69 01        CALL    $0169               ; {code.InitCommand}

; The upper nibble in B should be non-zero by now.
0166: C3 BF 00        JP      $00BF               ; {code.MainLoop} Continue main loop
...
01A2: C3 18 00        JP      $0018               ; {code.RST18} Write C to reg A chip 0
01A5: C6 04           ADD     A,$04               ; Add 4 (8,9, or 10)
01A7: C3 20 00        JP      $0020               ; {code.RST20} Write C to reg A chip 1
...
01B9: C3 18 00        JP      $0018               ; {code.RST18} Write Chip 0
...
01C3: 3C              INC     A                   ; ...
01C4: 4C              LD      C,H                 ; ...
01C5: C3 20 00        JP      $0020               ; {code.RST20} Done
```
...
01EC: 2A 0C 30        LD      HL,($300C)          ; {ram.currentCaps} Old value
01EF: 7D              LD      A,L                 ; LSB
01F0: A1              AND     C                   ; Mask
...
01F7: 22 0C 30        LD      ($300C),HL          ; {ram.currentCaps} New configuration value
...
03E7: AF              XOR     A                   ; No ...
03E8: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps} ... capacitors
...
0432: AF              XOR     A                   ; A=0
0433: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps} No filter caps on channel
0436: 0E 0A           LD      C,$0A               ; Set volume ...
0438: CD 99 01        CALL    $0199               ; {} ... to 10
043B: 3E 80           LD      A,$80               ; Loop count ...
043D: 32 44 30        LD      ($3044),A           ; {ram.?command02?} ... is 128
0440: 21 04 00        LD      HL,$0004            ; Initial frequency ...
0443: 22 45 30        LD      ($3045),HL          ; {ram.?command02?+1} ... is 4
...
0456: 2A 45 30        LD      HL,($3045)          ; {ram.?command02?+1} Current tone value
...
0463: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps} ... caps on channel
0466: 21 C0 00        LD      HL,$00C0            ; Inital ...
0469: 22 47 30        LD      ($3047),HL          ; {ram.?command03?} ... tone
...
049A: 3E 01           LD      A,$01               ; 
049C: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
04AE: 22 4C 30        LD      ($304C),HL          ; {ram.?command04?+2}
04B1: AF              XOR     A                   ; 
04B2: C9              RET                         ; 
...
04B8: E6 0F           AND     $0F                 ; 
04BA: 28 16           JR      Z,$4D2              ; {}
04BC: 2A 4C 30        LD      HL,($304C)          ; {ram.?command04?+2}
04BF: FE 0C           CP      $0C                 ; 
04C1: 01 10 00        LD      BC,$0010            ; 
04C4: 30 03           JR      NC,$4C9             ; {}
04C6: 01 D0 FF        LD      BC,$FFD0            ; 
04C9: 09              ADD     HL,BC               ; 
04CA: 22 4C 30        LD      ($304C),HL          ; {ram.?command04?+2}
...
04D9: AF              XOR     A                   ; 
04DA: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
050A: AF              XOR     A                   ; 
050B: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
054D: AF              XOR     A                   ; 
054E: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
0564: 22 57 30        LD      ($3057),HL          ; {ram.?command07?+2}
...
0573: FE 16           CP      $16                 ; 
0575: 28 EA           JR      Z,$561              ; {}
0577: 2A 57 30        LD      HL,($3057)          ; {ram.?command07?+2}
057A: 01 10 00        LD      BC,$0010            ; 
057D: 09              ADD     HL,BC               ; 
057E: 22 57 30        LD      ($3057),HL          ; {ram.?command07?+2}
...
0588: 3E 01           LD      A,$01               ; 
058A: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
059C: 22 59 30        LD      ($3059),HL          ; {ram.?command08?}
...
05AD: 4F              LD      C,A                 ; 
05AE: 06 00           LD      B,$00               ; 
05B0: 2A 59 30        LD      HL,($3059)          ; {ram.?command08?}
05B3: 09              ADD     HL,BC               ; 
05B4: 22 59 30        LD      ($3059),HL          ; {ram.?command08?}
...
05C8: 3E 01           LD      A,$01               ; 
05CA: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
061B: AF              XOR     A                   ; 
061C: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
0632: 22 65 30        LD      ($3065),HL          ; {ram.?command0A?+3}
...
0649: E6 0F           AND     $0F                 ; 
064B: 28 DD           JR      Z,$62A              ; {}
064D: 2A 65 30        LD      HL,($3065)          ; {ram.?command0A?+3}
0650: 01 40 00        LD      BC,$0040            ; 
0653: 09              ADD     HL,BC               ; 
0654: 22 65 30        LD      ($3065),HL          ; {ram.?command0A?+3}
...
065F: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps} ... caps on channel
...
0695: 3E 01           LD      A,$01               ; 
0697: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
06D9: 3E 01           LD      A,$01               ; 
06DB: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
0715: 3E 01           LD      A,$01               ; 
0717: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
075C: 3E 01           LD      A,$01               ; 
075E: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
078A: 3E 01           LD      A,$01               ; 
078C: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
078F: 0E 0A           LD      C,$0A               ; 
...
07C6: 3E 01           LD      A,$01               ; 
07C8: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
080B: 3E 01           LD      A,$01               ; 
080D: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
0810: 21 00 00        LD      HL,$0000            ; 
0813: 22 81 30        LD      ($3081),HL          ; {ram.?command12?+3}
...
083D: C6 C5           ADD     A,$C5               ; 
083F: 77              LD      (HL),A              ; 
0840: 2A 81 30        LD      HL,($3081)          ; {ram.?command12?+3}
0843: AA              XOR     D                   ; 
0844: AD              XOR     L                   ; 
0845: 6F              LD      L,A                 ; 
0846: 67              LD      H,A                 ; 
...
084B: 26 01           LD      H,$01               ; 
084D: 22 81 30        LD      ($3081),HL          ; {ram.?command12?+3}
...
0857: 3E 02           LD      A,$02               ; 
0859: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
0877: 22 87 30        LD      ($3087),HL          ; {ram.?command13?+4}
087A: 22 89 30        LD      ($3089),HL          ; {ram.?command13?+6}
087D: AF              XOR     A                   ; 
087E: C9              RET                         ; 
...
089F: 4F              LD      C,A                 ; 
08A0: 06 00           LD      B,$00               ; 
08A2: 2A 89 30        LD      HL,($3089)          ; {ram.?command13?+6}
08A5: 09              ADD     HL,BC               ; 
08A6: 22 89 30        LD      ($3089),HL          ; {ram.?command13?+6}
...
08B6: 2C              INC     L                   ; 
08B7: 3A 85 30        LD      A,($3085)           ; {ram.?command13?+2}
08BA: E6 3F           AND     $3F                 ; 
08BC: C6 30           ADD     A,$30               ; 
08BE: 77              LD      (HL),A              ; 
08BF: 2A 87 30        LD      HL,($3087)          ; {ram.?command13?+4}
08C2: 01 60 00        LD      BC,$0060            ; 
08C5: 09              ADD     HL,BC               ; 
08C6: 22 87 30        LD      ($3087),HL          ; {ram.?command13?+4}
08C9: 22 89 30        LD      ($3089),HL          ; {ram.?command13?+6}
08CC: AF              XOR     A                   ; 
08CD: C9              RET                         ; 
08CE: 3D              DEC     A                   ; 
08CF: C9              RET                         ; 
...
08D0: 3E 02           LD      A,$02               ; 
08D2: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
08F0: 22 8F 30        LD      ($308F),HL          ; {ram.?command14?+4}
08F3: 22 91 30        LD      ($3091),HL          ; {ram.?command14?+6}
08F6: AF              XOR     A                   ; 
08F7: C9              RET                         ; 
...
0911: AF              XOR     A                   ; 
0912: C9              RET                         ; 
0913: 2A 91 30        LD      HL,($3091)          ; {ram.?command14?+6}
0916: 01 30 00        LD      BC,$0030            ; 
0919: 09              ADD     HL,BC               ; 
091A: 22 91 30        LD      ($3091),HL          ; {ram.?command14?+6}
...
092A: 2C              INC     L                   ; 
092B: 3A 8D 30        LD      A,($308D)           ; {ram.?command14?+2}
092E: E6 3F           AND     $3F                 ; 
0930: C6 30           ADD     A,$30               ; 
0932: 77              LD      (HL),A              ; 
0933: 2A 8F 30        LD      HL,($308F)          ; {ram.?command14?+4}
0936: 01 06 00        LD      BC,$0006            ; 
0939: 09              ADD     HL,BC               ; 
093A: 22 8F 30        LD      ($308F),HL          ; {ram.?command14?+4}
093D: 22 91 30        LD      ($3091),HL          ; {ram.?command14?+6}
0940: AF              XOR     A                   ; 
0941: C9              RET                         ; 
0942: 3D              DEC     A                   ; 
0943: C9              RET                         ; 
...
0944: 3E 01           LD      A,$01               ; 
0946: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
099C: AF              XOR     A                   ; 
099D: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
09A0: 0E 09           LD      C,$09               ; 
...
09D1: 3E 01           LD      A,$01               ; 
09D3: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
09D6: 0E 0F           LD      C,$0F               ; 
...
0A02: AF              XOR     A                   ; 
0A03: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
0A06: 0E 0A           LD      C,$0A               ; 
0A08: CD 99 01        CALL    $0199               ; {} Set volume
0A0B: 3E 80           LD      A,$80               ; 
0A0D: 32 9E 30        LD      ($309E),A           ; {ram.?command18?+2}
...
0A30: 2C              INC     L                   ; 
0A31: BE              CP      (HL)                ; 
0A32: 3A 9D 30        LD      A,($309D)           ; {ram.?command18?+1}
...
0A4B: AF              XOR     A                   ; 
0A4C: CD C8 01        CALL    $01C8               ; {code.SetFilterCaps}
...
0B1C: CD 7E 03        CALL    $037E               ; {code.InitSong} Initialize music descriptors
...
0B2C: C3 BF 00        JP      $00BF               ; {code.MainLoop} Goto main loop
