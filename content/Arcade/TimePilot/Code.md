...
0069: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
...
0076: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
...
0083: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
...
0093: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
...
00A8: 32 00 C3        LD      ($C300),A           ; {hard.in0} raise the interrupt-enable line from the low bit the caller carries
00AB: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
...
00EC: AF              XOR     A                   
00ED: 32 00 C3        LD      ($C300),A           ; {hard.in0} disarm this interrupt's own enable line
00F0: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
00F3: 3C              INC     A                   
...
010C: 3A 00 C2        LD      A,($C200)           ; {hard.dsw1} read the dip-switch bank
...
0113: 3A 00 C3        LD      A,($C300)           ; {hard.in0} read the player-one controls
0116: 2F              CPL                         
0117: 32 AE A9        LD      ($A9AE),A           ; {hard.workRam+1AE} store the player-one-control mirror
011A: 3A 20 C3        LD      A,($C320)           ; {hard.in1} read the next input port
011D: 2F              CPL                         
011E: 32 AF A9        LD      ($A9AF),A           ; {hard.workRam+1AF} store its mirror
0121: 3A 40 C3        LD      A,($C340)           ; {hard.in2} read the next input port
0124: 2F              CPL                         
0125: 32 B0 A9        LD      ($A9B0),A           ; {hard.workRam+1B0} store its mirror
0128: 3A 60 C3        LD      A,($C360)           ; {hard.dsw0} read the last input port
012B: 2F              CPL                         
...
0187: 32 00 C3        LD      ($C300),A           ; {hard.in0} re-arm this interrupt for the next frame
018A: F1              POP     AF                  
018B: C9              RET                         
...
04CD: 3A 11 B4        LD      A,($B411)           ; {hard.spriteRam1+11} read this sprite's second bank byte
...
04D4: 32 11 B4        LD      ($B411),A           ; {hard.spriteRam1+11} raise the second byte's top bit
...
loc_04de:
04DE: 3A 13 B4        LD      A,($B413)           ; {hard.spriteRam1+13} same top-bit raise for the next of the eight sprites
04E1: C6 80           ADD     A,$80               
04E3: 38 0A           JR      C,$04EF             ; {code.loc_04ef}
04E5: 32 13 B4        LD      ($B413),A           ; {hard.spriteRam1+13}
...
loc_04ef:
04EF: 3A 15 B4        LD      A,($B415)           ; {hard.spriteRam1+15} same raise for the next sprite
04F2: C6 80           ADD     A,$80               
04F4: 38 0A           JR      C,$0500             ; {code.loc_0500}
04F6: 32 15 B4        LD      ($B415),A           ; {hard.spriteRam1+15}
...
loc_0500:
0500: 3A 37 B4        LD      A,($B437)           ; {hard.spriteRam1+37} same raise for the next sprite
0503: C6 80           ADD     A,$80               
0505: 38 0A           JR      C,$0511             ; {code.loc_0511}
0507: 32 37 B4        LD      ($B437),A           ; {hard.spriteRam1+37}
...
loc_0511:
0511: 3A 39 B4        LD      A,($B439)           ; {hard.spriteRam1+39} same raise for the next sprite
0514: C6 80           ADD     A,$80               
0516: 38 0A           JR      C,$0522             ; {code.loc_0522}
0518: 32 39 B4        LD      ($B439),A           ; {hard.spriteRam1+39}
...
loc_0522:
0522: 3A 3B B4        LD      A,($B43B)           ; {hard.spriteRam1+3B} same raise for the next sprite
0525: C6 80           ADD     A,$80               
0527: 38 0A           JR      C,$0533             ; {code.loc_0533}
0529: 32 3B B4        LD      ($B43B),A           ; {hard.spriteRam1+3B}
...
loc_0533:
0533: 3A 3D B4        LD      A,($B43D)           ; {hard.spriteRam1+3D} same raise for the next sprite
0536: C6 80           ADD     A,$80               
0538: 38 0A           JR      C,$0544             ; {code.loc_0544}
053A: 32 3D B4        LD      ($B43D),A           ; {hard.spriteRam1+3D}
...
loc_0544:
0544: 3A 3F B4        LD      A,($B43F)           ; {hard.spriteRam1+3F} same raise for the last of the eight sprites
0547: C6 80           ADD     A,$80               
0549: 38 0A           JR      C,$0555             ; {code.loc_0555}
054B: 32 3F B4        LD      ($B43F),A           ; {hard.spriteRam1+3F}
...
07BC: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
...
0910: 38 01           JR      C,$0913             ; {hard.rom+913}
...
0F97: 3A 11 B4        LD      A,($B411)           ; {hard.spriteRam1+11} read the first scenery slot's Y byte -- bit 7 marks a slot wanting a second image this frame
0F9A: CB 7F           BIT     7,A                 ; test that request bit
0F9C: 28 19           JR      Z,$0FB7             ; {code.loc_0fb7} clear -- move on to the next slot
0F9E: 4F              LD      C,A                 
0F9F: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the live scanline counter
...
0FAC: 32 11 B4        LD      ($B411),A           ; {hard.spriteRam1+11} store the moved Y byte
0FAF: 3A 10 B0        LD      A,($B010)           ; {hard.spriteRam0+10} read the slot's paired X byte
0FB2: C6 80           ADD     A,$80               ; add half a screen across
0FB4: 32 10 B0        LD      ($B010),A           ; {hard.spriteRam0+10} store it -- the same sprite draws again half a screen away, with no extra hardware slot

loc_0fb7:
0FB7: 3A 13 B4        LD      A,($B413)           ; {hard.spriteRam1+13} read the next slot's Y byte
0FBA: CB 7F           BIT     7,A                 ; test its request bit
0FBC: 28 19           JR      Z,$0FD7             ; {code.loc_0fd7} clear -- next slot
0FBE: 4F              LD      C,A                 
0FBF: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
0FCC: 32 13 B4        LD      ($B413),A           ; {hard.spriteRam1+13} store the moved Y byte
0FCF: 3A 12 B0        LD      A,($B012)           ; {hard.spriteRam0+12} read the paired X byte
0FD2: C6 80           ADD     A,$80               ; add half a screen across
0FD4: 32 12 B0        LD      ($B012),A           ; {hard.spriteRam0+12} store it -- second image placed

loc_0fd7:
0FD7: 3A 15 B4        LD      A,($B415)           ; {hard.spriteRam1+15} read the next slot's Y byte
0FDA: CB 7F           BIT     7,A                 ; test its request bit
0FDC: 28 19           JR      Z,$0FF7             ; {code.loc_0ff7} clear -- next slot
0FDE: 4F              LD      C,A                 
0FDF: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
0FEC: 32 15 B4        LD      ($B415),A           ; {hard.spriteRam1+15} store the moved Y byte
0FEF: 3A 14 B0        LD      A,($B014)           ; {hard.spriteRam0+14} read the paired X byte
0FF2: C6 80           ADD     A,$80               ; add half a screen across
0FF4: 32 14 B0        LD      ($B014),A           ; {hard.spriteRam0+14} store it -- second image placed

loc_0ff7:
0FF7: 3A 37 B4        LD      A,($B437)           ; {hard.spriteRam1+37} read the next slot's Y byte
0FFA: CB 7F           BIT     7,A                 ; test its request bit
0FFC: 28 19           JR      Z,$1017             ; {code.loc_1017} clear -- next slot
0FFE: 4F              LD      C,A                 
0FFF: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
100C: 32 37 B4        LD      ($B437),A           ; {hard.spriteRam1+37} store the moved Y byte
100F: 3A 36 B0        LD      A,($B036)           ; {hard.spriteRam0+36} read the paired X byte
1012: C6 80           ADD     A,$80               ; add half a screen across
1014: 32 36 B0        LD      ($B036),A           ; {hard.spriteRam0+36} store it -- second image placed

loc_1017:
1017: 3A 39 B4        LD      A,($B439)           ; {hard.spriteRam1+39} read the next slot's Y byte
101A: CB 7F           BIT     7,A                 ; test its request bit
101C: 28 19           JR      Z,$1037             ; {code.loc_1037} clear -- next slot
101E: 4F              LD      C,A                 
101F: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
102C: 32 39 B4        LD      ($B439),A           ; {hard.spriteRam1+39} store the moved Y byte
102F: 3A 38 B0        LD      A,($B038)           ; {hard.spriteRam0+38} read the paired X byte
1032: C6 80           ADD     A,$80               ; add half a screen across
1034: 32 38 B0        LD      ($B038),A           ; {hard.spriteRam0+38} store it -- second image placed

loc_1037:
1037: 3A 3B B4        LD      A,($B43B)           ; {hard.spriteRam1+3B} read the next slot's Y byte
103A: CB 7F           BIT     7,A                 ; test its request bit
103C: 28 19           JR      Z,$1057             ; {code.loc_1057} clear -- next slot
103E: 4F              LD      C,A                 
103F: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
104C: 32 3B B4        LD      ($B43B),A           ; {hard.spriteRam1+3B} store the moved Y byte
104F: 3A 3A B0        LD      A,($B03A)           ; {hard.spriteRam0+3A} read the paired X byte
1052: C6 80           ADD     A,$80               ; add half a screen across
1054: 32 3A B0        LD      ($B03A),A           ; {hard.spriteRam0+3A} store it -- second image placed

loc_1057:
1057: 3A 3D B4        LD      A,($B43D)           ; {hard.spriteRam1+3D} read the next slot's Y byte
105A: CB 7F           BIT     7,A                 ; test its request bit
105C: 28 19           JR      Z,$1077             ; {code.loc_1077} clear -- next slot
105E: 4F              LD      C,A                 
105F: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
106C: 32 3D B4        LD      ($B43D),A           ; {hard.spriteRam1+3D} store the moved Y byte
106F: 3A 3C B0        LD      A,($B03C)           ; {hard.spriteRam0+3C} read the paired X byte
1072: C6 80           ADD     A,$80               ; add half a screen across
1074: 32 3C B0        LD      ($B03C),A           ; {hard.spriteRam0+3C} store it -- second image placed

loc_1077:
1077: 3A 3F B4        LD      A,($B43F)           ; {hard.spriteRam1+3F} read the last slot's Y byte
107A: CB 7F           BIT     7,A                 ; test its request bit
107C: 28 19           JR      Z,$1097             ; {code.loc_1097} clear -- done
107E: 4F              LD      C,A                 
107F: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
108C: 32 3F B4        LD      ($B43F),A           ; {hard.spriteRam1+3F} store the moved Y byte
108F: 3A 3E B0        LD      A,($B03E)           ; {hard.spriteRam0+3E} read the paired X byte
1092: C6 80           ADD     A,$80               ; add half a screen across
1094: 32 3E B0        LD      ($B03E),A           ; {hard.spriteRam0+3E} store it -- second image placed
...
1098: 3A 11 B4        LD      A,($B411)           ; {hard.spriteRam1+11} read the first scenery slot's Y byte -- this pass waits on the beam rather than skipping
109B: CB 7F           BIT     7,A                 ; test its request bit
109D: 28 19           JR      Z,$10B8             ; {code.loc_10b8} clear -- next slot
109F: 4F              LD      C,A                 
10A0: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
10AD: 32 11 B4        LD      ($B411),A           ; {hard.spriteRam1+11} store the moved Y byte
10B0: 3A 10 B0        LD      A,($B010)           ; {hard.spriteRam0+10} read the paired X byte
10B3: C6 80           ADD     A,$80               ; add half a screen across
10B5: 32 10 B0        LD      ($B010),A           ; {hard.spriteRam0+10} store it -- second image placed

loc_10b8:
10B8: 3A 13 B4        LD      A,($B413)           ; {hard.spriteRam1+13} read the next slot's Y byte
10BB: CB 7F           BIT     7,A                 ; test its request bit
10BD: 28 19           JR      Z,$10D8             ; {code.loc_10d8} clear -- next slot
10BF: 4F              LD      C,A                 
10C0: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
10CD: 32 13 B4        LD      ($B413),A           ; {hard.spriteRam1+13} store the moved Y byte
10D0: 3A 12 B0        LD      A,($B012)           ; {hard.spriteRam0+12} read the paired X byte
10D3: C6 80           ADD     A,$80               ; add half a screen across
10D5: 32 12 B0        LD      ($B012),A           ; {hard.spriteRam0+12} store it -- second image placed

loc_10d8:
10D8: 3A 15 B4        LD      A,($B415)           ; {hard.spriteRam1+15} read the next slot's Y byte
10DB: CB 7F           BIT     7,A                 ; test its request bit
10DD: 28 19           JR      Z,$10F8             ; {code.loc_10f8} clear -- next slot
10DF: 4F              LD      C,A                 
10E0: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
10ED: 32 15 B4        LD      ($B415),A           ; {hard.spriteRam1+15} store the moved Y byte
10F0: 3A 14 B0        LD      A,($B014)           ; {hard.spriteRam0+14} read the paired X byte
10F3: C6 80           ADD     A,$80               ; add half a screen across
10F5: 32 14 B0        LD      ($B014),A           ; {hard.spriteRam0+14} store it -- second image placed

loc_10f8:
10F8: 3A 37 B4        LD      A,($B437)           ; {hard.spriteRam1+37} read the next slot's Y byte
...
10FF: 4F              LD      C,A                 
1100: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
110D: 32 37 B4        LD      ($B437),A           ; {hard.spriteRam1+37} store the moved Y byte
1110: 3A 36 B0        LD      A,($B036)           ; {hard.spriteRam0+36} read the paired X byte
1113: C6 80           ADD     A,$80               ; add half a screen across
1115: 32 36 B0        LD      ($B036),A           ; {hard.spriteRam0+36} store it -- second image placed

loc_1118:
1118: 3A 39 B4        LD      A,($B439)           ; {hard.spriteRam1+39} read the next slot's Y byte
111B: CB 7F           BIT     7,A                 ; test its request bit
111D: 28 19           JR      Z,$1138             ; {code.loc_1138} clear -- next slot
111F: 4F              LD      C,A                 
1120: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
112D: 32 39 B4        LD      ($B439),A           ; {hard.spriteRam1+39} store the moved Y byte
1130: 3A 38 B0        LD      A,($B038)           ; {hard.spriteRam0+38} read the paired X byte
1133: C6 80           ADD     A,$80               ; add half a screen across
1135: 32 38 B0        LD      ($B038),A           ; {hard.spriteRam0+38} store it -- second image placed

loc_1138:
1138: 3A 3B B4        LD      A,($B43B)           ; {hard.spriteRam1+3B} read the next slot's Y byte
113B: CB 7F           BIT     7,A                 ; test its request bit
113D: 28 19           JR      Z,$1158             ; {code.loc_1158} clear -- next slot
113F: 4F              LD      C,A                 
1140: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
114D: 32 3B B4        LD      ($B43B),A           ; {hard.spriteRam1+3B} store the moved Y byte
1150: 3A 3A B0        LD      A,($B03A)           ; {hard.spriteRam0+3A} read the paired X byte
1153: C6 80           ADD     A,$80               ; add half a screen across
1155: 32 3A B0        LD      ($B03A),A           ; {hard.spriteRam0+3A} store it -- second image placed

loc_1158:
1158: 3A 3D B4        LD      A,($B43D)           ; {hard.spriteRam1+3D} read the next slot's Y byte
115B: CB 7F           BIT     7,A                 ; test its request bit
115D: 28 19           JR      Z,$1178             ; {code.loc_1178} clear -- next slot
115F: 4F              LD      C,A                 
1160: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
116D: 32 3D B4        LD      ($B43D),A           ; {hard.spriteRam1+3D} store the moved Y byte
1170: 3A 3C B0        LD      A,($B03C)           ; {hard.spriteRam0+3C} read the paired X byte
1173: C6 80           ADD     A,$80               ; add half a screen across
1175: 32 3C B0        LD      ($B03C),A           ; {hard.spriteRam0+3C} store it -- second image placed

loc_1178:
1178: 3A 3F B4        LD      A,($B43F)           ; {hard.spriteRam1+3F} read the last slot's Y byte
117B: CB 7F           BIT     7,A                 ; test its request bit
117D: 28 19           JR      Z,$1198             ; {code.loc_1198} clear -- done
117F: 4F              LD      C,A                 
1180: 3A 00 C0        LD      A,($C000)           ; {hard.scanline} read the scanline counter
...
118D: 32 3F B4        LD      ($B43F),A           ; {hard.spriteRam1+3F} store the moved Y byte
1190: 3A 3E B0        LD      A,($B03E)           ; {hard.spriteRam0+3E} read the paired X byte
1193: C6 80           ADD     A,$80               ; add half a screen across
1195: 32 3E B0        LD      ($B03E),A           ; {hard.spriteRam0+3E} store it -- second image placed
...
1610: 32 01 A7        LD      ($A701),A           ; {hard.videoRam+301} seed it into one cell
1613: 32 E1 A6        LD      ($A6E1),A           ; {hard.videoRam+2E1} and into another
...
176D: 3A 7C A6        LD      A,($A67C)           ; {hard.videoRam+27C} read a caption cell
1770: FE 7C           CP      $7C                 ; is its glyph the expected one?
1772: C2 9B 45        JP      NZ,$459B            ; {hard.rom+459B} wrong glyph: derail into the mother-ship warp-flash handler
...
19E6: C2 FA 49        JP      NZ,$49FA            ; {hard.rom+49FA} anything else: the line has been tampered -- transfer away and never return
...
256E: C7              RST     $00                 
256F: 00              NOP                         
2570: C3 00 C0        JP      $C000               ; {hard.scanline}
...
2E4C: 28 01           JR      Z,$2E4F             ; {hard.rom+2E4F}
2E4E: 26 01           LD      H,$01               
2E50: 24              INC     H                   
...
32EB: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
...
32F8: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
...
loc_4660:
4660: C3 9B 45        JP      $459B               ; {hard.rom+459B} run the warp/flash stepper again
...
49B8: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog
...
52B6: 3A 60 C3        LD      A,($C360)           ; {hard.dsw0} read the first DIP-switch bank
...
52C0: 3A 00 C2        LD      A,($C200)           ; {hard.dsw1} read the second DIP-switch bank
...
sendSoundCommand:
55F8: 32 00 C0        LD      ($C000),A           ; {hard.scanline} drop the byte into the audio processor's command latch
...
5875: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog -- colour fill done
...
5896: 32 00 C2        LD      ($C200),A           ; {hard.dsw1} kick the watchdog -- once per summed byte
...
59E6: 00              NOP                         
59E7: C6 00           ADD     A,$00               
59E9: C4 00 C2        CALL    NZ,$C200            ; {hard.dsw1}
59EC: 00              NOP                         
59ED: C0              RET     NZ                  
59EE: 00              NOP                         
59EF: BF              CP      A                   
...
5A59: F8              RET     M                   
5A5A: FF              RST     $38                 
5A5B: F2 FF 00        JP      P,$00FF             ; {hard.rom+FF}
5A5E: 00              NOP                         
5A5F: E6 FF           AND     $FF                 
5A61: E0              RET     PO                  
5A62: FF              RST     $38                 
...
5AC8: FF              RST     $38                 
5AC9: 38 FF           JR      C,$5ACA             ; {hard.rom+5ACA}
5ACB: 37              SCF                         
5ACC: FF              RST     $38                 
5ACD: 36 FF           LD      (HL),$FF            
5ACF: 35              DEC     (HL)                
...
5AE1: 37              SCF                         
5AE2: FF              RST     $38                 
5AE3: 38 FF           JR      C,$5AE4             ; {hard.rom+5AE4}
5AE5: 38 FF           JR      C,$5AE6             ; {hard.rom+5AE6}
