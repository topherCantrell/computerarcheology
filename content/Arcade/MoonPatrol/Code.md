...
0006: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} Service mode ...
0009: A7              AND     A                   ; ... switch set?
000A: F2 00 33        JP      P,$3300             ; {code.ServiceMode} 0=Yes ... go do service mode (not in the same ROM chip but should be)
000D: CD F4 05        CALL    $05F4               ; {code.ClearRAM} Clear RAM (E000..E6FF)
0010: CD F2 06        CALL    $06F2               ; {code.ReadSettings} Read the dip-switch settings
0013: CD 29 0D        CALL    $0D29               ; {} ?? Clear screen, screen flip, clear ram mirror, clear sound queue
0016: C3 68 00        JP      $0068               ; {code.StartupCont} Continue startup
...
003A: CD 6D 05        CALL    $056D               ; {code.RegMirror} Copy RAM mirror to hardware at start of VBLANK
;
...
004B: 35              DEC     (HL)                ; -1
;
004C: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} Are we ...
...
007D: AF              XOR     A                   ; * J0091
007E: 32 4D E0        LD      ($E04D),A           ; {ram.mE04D}
...
0093: CD CB 05        CALL    $05CB               ; {code.NextSound} Play next waiting sound (if any) * J0050
0096: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Coins and start
0099: 2F              CPL                         ; Active high
009A: 32 53 E0        LD      ($E053),A           ; {ram.coinStart} Hold for whoever wants it
009D: CB 4F           BIT     1,A                 ; Start-2 pressed?
009F: 28 15           JR      Z,$B6               ; {} No ... move on
00A1: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} DSW2
...
00AF: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Start buttons * J00B4
00B2: E6 01           AND     $01                 ; Is start-1 pressed?
00B4: 20 F9           JR      NZ,$AF              ; {} No ... wait for it
;
00B6: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} DSW2 again * J009F,J00A6,J00AD
...
00C7: CD 0E 04        CALL    $040E               ; {code.CoinsAndCredits} Handle coins and credits
00CA: 3A 41 E0        LD      A,($E041)           ; {ram.slotModeA} Coin mode slot A
00CD: A7              AND     A                   ; Free play?
00CE: 20 05           JR      NZ,$D5              ; {} No ... keep credits
00D0: 3E 02           LD      A,$02               ; Free play ...
00D2: 32 48 E0        LD      ($E048),A           ; {ram.Credits} ... always 2 credits
...
00DD: CB 48           BIT     1,B                 ; 
00DF: 20 1B           JR      NZ,$FC              ; {}
00E1: 3A 48 E0        LD      A,($E048)           ; {ram.Credits}
00E4: A7              AND     A                   ; 
00E5: 20 1D           JR      NZ,$104             ; {}
00E7: CB 50           BIT     2,B                 ; 
00E9: 20 11           JR      NZ,$FC              ; {}
00EB: 3A 47 E0        LD      A,($E047)           ; {ram.cntTillCred}
00EE: A7              AND     A                   ; 
00EF: C2 8F 00        JP      NZ,$008F            ; {}
00F2: CB 40           BIT     0,B                 ; * J00DB
00F4: 28 06           JR      Z,$FC               ; {} Skip game objects
00F6: CD 7A 04        CALL    $047A               ; {code.GetInputs} Get inputs (or demo mode ??)
00F9: CD EC 01        CALL    $01EC               ; {code.RunObjects} Run the game objects
...
011E: 35              DEC     (HL)                ; 
011F: FB              EI                          ; 
0120: F2 68 00        JP      P,$0068             ; {code.StartupCont}
0123: CD 96 0B        CALL    $0B96               ; {}
0126: AF              XOR     A                   ; 
0127: 32 A6 E1        LD      ($E1A6),A           ; {ram.mE1A6}
...
0163: E6 01           AND     $01                 ; 
0165: 3C              INC     A                   ; 
0166: CD BD 03        CALL    $03BD               ; {code.PrintDigit}
0169: CD 03 06        CALL    $0603               ; {code.SwapPlayers}
...
0184: CD E8 05        CALL    $05E8               ; {code.Delay3Sec} * J017F
0187: CD 29 0D        CALL    $0D29               ; {}
018A: 21 2A 2A        LD      HL,$2A2A            ; "TO COUNTINUE GAME" script
018D: CD 00 03        CALL    $0300               ; {code.RunTextScript} Print it
0190: 3E 11           LD      A,$11               ; 
0192: A7              AND     A                   ; * J01AE
0193: 28 3D           JR      Z,$1D2              ; {}
0195: 3D              DEC     A                   ; 
0196: 27              DAA                         ; 
0197: 32 54 E0        LD      ($E054),A           ; {ram.mE054}
019A: 11 33 82        LD      DE,$8233            ; 
019D: 0E 02           LD      C,$02               ; 
019F: CD AE 03        CALL    $03AE               ; {code.PrintBCD}
01A2: 3E 40           LD      A,$40               ; 
01A4: 32 50 E0        LD      ($E050),A           ; {ram.isrCount3}
01A7: 3A 50 E0        LD      A,($E050)           ; {ram.isrCount3} * J01BF,J01C4
01AA: A7              AND     A                   ; 
01AB: 3A 54 E0        LD      A,($E054)           ; {ram.mE054}
01AE: 28 E2           JR      Z,$192              ; {}
01B0: 3A 48 E0        LD      A,($E048)           ; {ram.Credits}
01B3: A7              AND     A                   ; 
...
01D7: C3 68 00        JP      $0068               ; {code.StartupCont}
01DA: 3A 40 E0        LD      A,($E040)           ; {ram.PerCredit} * C01C6,C01C9
01DD: 32 15 E5        LD      ($E515),A           ; {ram.mE515}
01E0: 21 00 00        LD      HL,$0000            ; 
01E3: 22 00 E5        LD      ($E500),HL          ; {ram.curScore}
01E6: 22 02 E5        LD      ($E502),HL          ; {ram.curScore2}
01E9: C3 03 06        JP      $0603               ; {code.SwapPlayers}
```
...
01F2: 32 E8 E0        LD      ($E0E8),A           ; {ram.mE0E8} Hold the count
...
;
0283: 3A CF E1        LD      A,($E1CF)           ; {ram.comListHead} LSB of head of text command list
;
...
02D1: 32 D0 E1        LD      ($E1D0),A           ; {ram.comListTail} ... of next available text command
02D4: C9              RET                         ; Done
...
TxtCmd_03:
;
02ED: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} Get current ISR value
...
0312: 32 50 E0        LD      ($E050),A           ; {ram.isrCount3} count down
0315: 3A 50 E0        LD      A,($E050)           ; {ram.isrCount3} ISR counted ... * J0319
0318: A7              AND     A                   ; ... to 0 ?
0319: 20 FA           JR      NZ,$315             ; {} No ... keep waiting
031B: 18 E6           JR      $303                ; {} Back to top of loop
;
; Try other special commands
031D: CD C9 03        CALL    $03C9               ; {code.RunTextScript2} Process normal character or command sequence * J030E
0320: 18 E1           JR      $303                ; {} Back to top of loop

TxtCmd_04:
; Run the text-script if it is time
;
0322: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} Count from ISR
0325: DD BE 01        CP      (IX+$01)            ; Target count value
0328: 28 05           JR      Z,$32F              ; {code.TxtCmd_05} It is time to run script
...
0335: 18 44           JR      $37B                ; {code.EraseScript} Erase the script and done
...
;
0337: 3A 50 E0        LD      A,($E050)           ; {ram.isrCount3} Count-down ...
033A: A7              AND     A                   ; ... reached 0?
033B: 20 0C           JR      NZ,$349             ; {} No ... go to state 7
033D: CD 2F 03        CALL    $032F               ; {code.TxtCmd_05} Erase the script
...
0346: 32 50 E0        LD      ($E050),A           ; {ram.isrCount3} ... timer to 4 interrupts
...
0359: 3A 50 E0        LD      A,($E050)           ; {ram.isrCount3} Has ISR counted ... * J035D
...
;
0361: 3A 50 E0        LD      A,($E050)           ; {ram.isrCount3} Count-down timer ...
...
0369: 32 50 E0        LD      ($E050),A           ; {ram.isrCount3} ... to 4 interrupts
036C: CD FA 02        CALL    $02FA               ; {code.TxtCmd_02} Run object script
...
;
0374: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} ISR up counter * C01B9,C0536
0377: CB 67           BIT     4,A                 ; Every 16 interrupts
0379: 20 85           JR      NZ,$300             ; {code.RunTextScript} Bit set ... print text (bit cleared ... fall into erase)
...
0389: 28 F0           JR      Z,$37B              ; {code.EraseScript} ... and continue script
;
038B: AF              XOR     A                   ; Clear ...
...
03B7: 1F              RRA                         ; ... BCD ...
03B8: 1F              RRA                         ; ... digit
03B9: CD BD 03        CALL    $03BD               ; {code.PrintDigit} Print digit
...
;
03E7: 3A F9 E0        LD      A,($E0F9)           ; {ram.champColors} Get champion-color flag * C0678,C06A9,C0CD7,C2135,C2993
03EA: A7              AND     A                   ; If 0 ...
...
0414: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Current switch values
...
0421: 3A 02 D0        LD      A,($D002)           ; {hard.IN2} Current switch values
...
;
042C: 3A 4C E0        LD      A,($E04C)           ; {ram.flipValue} Current flip status
042F: B0              OR      B                   ; OR in coin-counter triggers
0430: 32 01 D0        LD      ($D001),A           ; {hard.FLIP} Trigger hardware coin counters
0433: C9              RET                         ; Done
;
...
0456: CD 7D 0D        CALL    $0D7D               ; {code.AddSound2} ... coin sound
...
0483: CB 67           BIT     4,A                 ; 
0485: 28 07           JR      Z,$48E              ; {}
0487: 3A 43 E0        LD      A,($E043)           ; {ram.cabMode} Cabinet type
...
04A9: 5D              LD      E,L                 ; 
04AA: 23              INC     HL                  ; 
04AB: 22 F7 E0        LD      ($E0F7),HL          ; {ram.mE0F7}
04AE: A7              AND     A                   ; 
04AF: 28 04           JR      Z,$4B5              ; {}
04B1: FE FF           CP      $FF                 ; 
04B3: 20 D9           JR      NZ,$48E             ; {}
04B5: 32 E0 E1        LD      ($E1E0),A           ; {ram.mE1E0} * J04AF
04B8: C9              RET                         ; 
;
04B9: 21 4A E0        LD      HL,$E04A            ; * J04A4
04BC: 3A E0 E1        LD      A,($E1E0)           ; {ram.mE1E0}
04BF: A7              AND     A                   ; 
04C0: 28 13           JR      Z,$4D5              ; {}
04C2: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1}
04C5: 07              RLCA                        ; 
04C6: 47              LD      B,A                 ; 
04C7: E6 01           AND     $01                 ; 
04C9: 3C              INC     A                   ; 
04CA: 32 49 E0        LD      ($E049),A           ; {ram.mE049}
04CD: 78              LD      A,B                 ; 
04CE: E6 1E           AND     $1E                 ; 
...
050F: 3A 09 E3        LD      A,($E309)           ; {ram.mE309}
0512: 17              RLA                         ; 
...
0518: C3 C2 02        JP      $02C2               ; {code.NewTxtCmd} Add the text-command and out
...
052A: CD 00 03        CALL    $0300               ; {code.RunTextScript} Print it
...
053F: 3A 48 E0        LD      A,($E048)           ; {ram.Credits} Number of credits
0542: 3D              DEC     A                   ; Only one player?
0543: 28 03           JR      Z,$548              ; {} Yes ... leave this message
0545: 21 97 2A        LD      HL,$2A97            ; "1 OR 2 PLAYERS" script
0548: CD 00 03        CALL    $0300               ; {code.RunTextScript} Print it * J0543
054B: 3A 53 E0        LD      A,($E053)           ; {ram.coinStart}
054E: E6 03           AND     $03                 ; 
0550: C8              RET     Z                   ; 
0551: 1F              RRA                         ; 
0552: 3A 48 E0        LD      A,($E048)           ; {ram.Credits}
0555: 06 80           LD      B,$80               ; 
0557: 38 06           JR      C,$55F              ; {}
0559: D6 01           SUB     $01                 ; 
055B: 27              DAA                         ; 
...
0561: 27              DAA                         ; 
0562: F3              DI                          ; 
0563: 32 48 E0        LD      ($E048),A           ; {ram.Credits}
0566: 78              LD      A,B                 ; 
0567: 32 46 E0        LD      ($E046),A           ; {ram.mE046}
056A: 3C              INC     A                   ; 
056B: FB              EI                          ; 
056C: C9              RET                         ; 
...
0596: 3A C3 E1        LD      A,($E1C3)           ; {ram.mE1C3} Value destined for BKG1Y
...
05B1: 3A 00 88        LD      A,($8800)           ; {hard.PROTECT} Get mangled bkg1X from protection
...
05C8: D3 C0           OUT     ($C0),A             ; {hard.BKGCtrl} Turn selected backgrounds on
05CA: C9              RET                         ; Done
...
; memory $9xxx.
; 
05CB: 2A DD E1        LD      HL,($E1DD)          ; {ram.SndQFront} Get front (L) and back (H) of sound effect queue pointer * C0093
...
05D4: 32 00 D0        LD      ($D000),A           ; {hard.SND_CMD} Write sound command with IRQ triggered (D7=0)
05D7: CB FF           SET     7,A                 ; Release the IRQ ...
05D9: 32 00 D0        LD      ($D000),A           ; {hard.SND_CMD} ... trigger D7=1
05DC: 7D              LD      A,L                 ; Bump to the ...
05DD: 3C              INC     A                   ; ... next effect in the queue
05DE: E6 07           AND     $07                 ; Only 8 bytes in queue ... then we wrap
05E0: 32 DD E1        LD      ($E1DD),A           ; {ram.SndQFront} Store the updated front
05E3: C9              RET                         ; Done
...
;
061C: 3A 46 E0        LD      A,($E046)           ; {ram.mE046} Get game-mode * C017C,C066B
...
062C: 3A 46 E0        LD      A,($E046)           ; {ram.mE046} Game mode
...
063F: CD AF 06        CALL    $06AF               ; {code.CheckExtPlay} Check and handle extended play
;
...
0659: 3A 02 E5        LD      A,($E502)           ; {ram.curScore2} ... to ...
065C: 32 0A E0        LD      ($E00A),A           ; {ram.highScore2} ... high score
;
065F: 3A 0E E5        LD      A,($E50E)           ; {ram.curPoint} Current passed-point
...
0678: CD E7 03        CALL    $03E7               ; {code.TransColor} Translate colors
;
067B: 06 03           LD      B,$03               ; Three bytes to print * J06AD
067D: 7E              LD      A,(HL)              ; Get BCD double digit * J0682
067E: CD AE 03        CALL    $03AE               ; {code.PrintBCD} Print two digits to screen at DE
...
0698: 3A F9 E0        LD      A,($E0F9)           ; {ram.champColors} Champion ...
069B: 1F              RRA                         ; ... course?
...
06A9: CD E7 03        CALL    $03E7               ; {code.TransColor} Do color translation
...
;
06AF: 2A 01 E5        LD      HL,($E501)          ; {ram.curScore1} * C063F
06B2: 3A 45 E0        LD      A,($E045)           ; {ram.extPoints} Extra points setting
06B5: 3D              DEC     A                   ; 
06B6: F8              RET     M                   ; 
06B7: 3D              DEC     A                   ; 
06B8: 28 01           JR      Z,$6BB              ; {}
...
06F5: 3A 03 D0        LD      A,($D003)           ; {hard.DSW1} Read the number of lives-per-credit
...
0708: 32 45 E0        LD      ($E045),A           ; {ram.extPoints} ... extended points field
;
070B: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} Get ...
...
;
0721: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} Various settings
...
076E: 3E 53           LD      A,$53               ; 
0770: 32 5F E0        LD      ($E05F),A           ; {ram.mE05F}
0773: 32 68 E0        LD      ($E068),A           ; {ram.mE068}
...
0794: C3 E8 05        JP      $05E8               ; {code.Delay3Sec}
...
07A7: 3A 41 E0        LD      A,($E041)           ; {ram.slotModeA} * J07DB
...
07F4: 19              ADD     HL,DE               ; 
07F5: 22 54 E0        LD      ($E054),HL          ; {ram.mE054}
...
084A: 0D              DEC     C                   ; 
084B: 20 6B           JR      NZ,$8B8             ; {code.DrawObject}
084D: FE E8           CP      $E8                 ; 
084F: 30 67           JR      NC,$8B8             ; {code.DrawObject}
0851: E1              POP     HL                  ; 
...
08EE: 47              LD      B,A                 ; 
08EF: 3A 3C E0        LD      A,($E03C)           ; {ram.mE03C}
08F2: 80              ADD     A,B                 ; 
08F3: 47              LD      B,A                 ; 
08F4: C9              RET                         ; 
...
092B: E6 1F           AND     $1F                 ; 
092D: FE 0B           CP      $0B                 ; 
092F: 38 38           JR      C,$969              ; {code.ObjDraw_00}
0931: 1C              INC     E                   ; Next color set
0932: 18 35           JR      $969                ; {code.ObjDraw_00}

ObjDraw_13: ; Hover craft full boost
0934: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1}
0937: E6 03           AND     $03                 ; 
0939: 28 01           JR      Z,$93C              ; {}
093B: 14              INC     D                   ; 
093C: CD 69 09        CALL    $0969               ; {code.ObjDraw_00} * J0939
093F: 16 3B           LD      D,$3B               ; 
0941: 18 1B           JR      $95E                ; {}
...
0953: CD 95 0A        CALL    $0A95               ; {code.TrnsSprColor}
0956: 3E F8           LD      A,$F8               ; 
0958: 80              ADD     A,B                 ; 
0959: 47              LD      B,A                 ; 
;
ObjDraw_01: ; Hover craft
095A: CD 69 09        CALL    $0969               ; {code.ObjDraw_00}
095D: 14              INC     D                   ; 
...
0992: D2 25 0B        JP      NC,$0B25            ; {code.ObjDraw_0D}
0995: 18 DF           JR      $976                ; {code.ObjDraw_0E} Store the image
...
09A0: FE F8           CP      $F8                 ; 
09A2: D0              RET     NC                  ; 
09A3: 18 D1           JR      $976                ; {code.ObjDraw_0E}
;
09A5: C6 08           ADD     $08                 ; * J096E
09A7: FE F0           CP      $F0                 ; 
09A9: 38 CB           JR      C,$976              ; {code.ObjDraw_0E}
...
09B0: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} ISR1 up counter
09B3: CB 57           BIT     2,A                 ; Chance ...
09B5: 28 BF           JR      Z,$976              ; {code.ObjDraw_0E} ... to use ...
09B7: 14              INC     D                   ; ... next image
09B8: 18 BC           JR      $976                ; {code.ObjDraw_0E} Store sprite details
...
09C9: 3E 0A           LD      A,$0A               ; 
09CB: 18 02           JR      $9CF                ; {code.ObjDraw_02}
...
09E1: EB              EX      DE,HL               ; 
09E2: 2E 00           LD      L,$00               ; 
09E4: CD 69 09        CALL    $0969               ; {code.ObjDraw_00} * C09C1,C09D4,J0A3C
...
0A2D: CD 83 09        CALL    $0983               ; {code.ObjDraw_17}
...
0A44: 32 A6 E1        LD      ($E1A6),A           ; {ram.mE1A6}
0A47: 32 AA E1        LD      ($E1AA),A           ; {ram.mE1AA}
0A4A: 32 AE E1        LD      ($E1AE),A           ; {ram.mE1AE}
0A4D: CD 95 0A        CALL    $0A95               ; {code.TrnsSprColor}
0A50: 18 10           JR      $A62                ; {}
...
0A63: CD 6F 0A        CALL    $0A6F               ; {code.Draw1x2Sprites} Do top and bottom tiles
...
0A90: CD 95 0A        CALL    $0A95               ; {code.TrnsSprColor}
0A93: 18 CD           JR      $A62                ; {}
...
;
0A95: 3A F9 E0        LD      A,($E0F9)           ; {ram.champColors} Are we on ... * C0953,C0A4D,C0A90,C0AE9,C0AFD
...
0AAE: 47              LD      B,A                 ; 
0AAF: AF              XOR     A                   ; 
0AB0: CD 6F 0A        CALL    $0A6F               ; {code.Draw1x2Sprites}
...
0ACA: EB              EX      DE,HL               ; 
0ACB: 3E 08           LD      A,$08               ; 
0ACD: CD 6F 0A        CALL    $0A6F               ; {code.Draw1x2Sprites} * C0AC1
...
0AE9: CD 95 0A        CALL    $0A95               ; {code.TrnsSprColor}
0AEC: 21 08 F8        LD      HL,$F808            ; 
0AEF: 09              ADD     HL,BC               ; 
0AF0: 44              LD      B,H                 ; 
0AF1: 4D              LD      C,L                 ; 
0AF2: C3 76 09        JP      $0976               ; {code.ObjDraw_0E}

ObjDraw_04: ; Moon buggy explosion 3x2
0AF5: FD 21 A4 E1     LD      IY,$E1A4            ; 
0AF9: FD 36 1A 00     LD      (IY+$1A),$00        ; 
0AFD: CD 95 0A        CALL    $0A95               ; {code.TrnsSprColor}
0B00: 3E F8           LD      A,$F8               ; 
...
0B12: EB              EX      DE,HL               ; 
0B13: CD 6F 0A        CALL    $0A6F               ; {code.Draw1x2Sprites} * C0B02
0B16: 7A              LD      A,D                 ; 
0B17: C6 02           ADD     $02                 ; 
...
0B1D: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1}
0B20: CB 4F           BIT     1,A                 ; 
0B22: 28 01           JR      Z,$B25              ; {code.ObjDraw_0D}
0B24: 1C              INC     E                   ; 
;
ObjDraw_0D: ; Alien ships and shots
0B25: CD 76 09        CALL    $0976               ; {code.ObjDraw_0E} * J0992,J0B22
...
0B35: C3 76 09        JP      $0976               ; {code.ObjDraw_0E} ?? Initialize "other" object to same


0B38: CD 8E 0C        CALL    $0C8E               ; {} * C0112
0B3B: CD BF 0C        CALL    $0CBF               ; {}
0B3E: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2}
0B41: CB 6F           BIT     5,A                 ; 
0B43: C0              RET     NZ                  ; 
0B44: 21 A9 2A        LD      HL,$2AA9            ; "PICTURE NUMBER SET" message
0B47: CD 00 03        CALL    $0300               ; {code.RunTextScript}
0B4A: 3E FF           LD      A,$FF               ; * J0B68,J0B88
0B4C: 32 51 E0        LD      ($E051),A           ; {ram.isrCount4}
...
0B56: 1F              RRA                         ; 
0B57: 38 08           JR      C,$B61              ; {}
0B59: 3A 51 E0        LD      A,($E051)           ; {ram.isrCount4}
0B5C: 47              LD      B,A                 ; 
0B5D: 79              LD      A,C                 ; 
0B5E: 10 F3           DJNZ    $B53                ; {}
0B60: C9              RET                         ; 
...
0B6B: 2A 16 E5        LD      HL,($E516)          ; {ram.mE516}
...
0B78: 78              LD      A,B                 ; 
0B79: 32 0B E5        LD      ($E50B),A           ; {ram.mE50B}
0B7C: 22 16 E5        LD      ($E516),HL          ; {ram.mE516}
0B7F: 32 23 E5        LD      ($E523),A           ; {ram.mE523}
...
0B96: 2A 16 E5        LD      HL,($E516)          ; {ram.mE516} * C0123,C0BE8
...
0BAD: 7E              LD      A,(HL)              ; 
0BAE: 32 0B E5        LD      ($E50B),A           ; {ram.mE50B}
0BB1: 23              INC     HL                  ; 
0BB2: 23              INC     HL                  ; 
...
0BF6: 3E 07           LD      A,$07               ; 
0BF8: 26 A8           LD      H,$A8               ; 
0BFA: 32 14 E5        LD      ($E514),A           ; {ram.mE514} * J0BF3
...
0C31: CD 00 03        CALL    $0300               ; {code.RunTextScript} Print the banner
0C34: 3A 10 E5        LD      A,($E510)           ; {ram.courseNum} Get the champion course number
...
0CAD: 22 16 E5        LD      ($E516),HL          ; {ram.mE516}
0CB0: C9              RET                         ; 

0CB1: 21 DE 26        LD      HL,$26DE            ; * C0B8A
0CB4: 22 F7 E0        LD      ($E0F7),HL          ; {ram.mE0F7}
...
0CC5: 0E 06           LD      C,$06               ; 
0CC7: 3A 0E E5        LD      A,($E50E)           ; {ram.curPoint} Current passed point
...
0CD1: 32 F9 E0        LD      ($E0F9),A           ; {ram.champColors} Store champion colors flag * J0CCE
0CD4: C5              PUSH    BC                  ; 
0CD5: 0E 01           LD      C,$01               ; 
0CD7: CD E7 03        CALL    $03E7               ; {code.TransColor}
0CDA: 79              LD      A,C                 ; 
0CDB: C1              POP     BC                  ; 
...
0D47: 28 01           JR      Z,$D4A              ; {}
0D49: 3C              INC     A                   ; 
0D4A: 32 4C E0        LD      ($E04C),A           ; {ram.flipValue} * J0D3F,J0D47
;
...
0D69: F3              DI                          ; 
0D6A: CD 7D 0D        CALL    $0D7D               ; {code.AddSound2} Queue up a stop-sound command
0D6D: FB              EI                          ; 
0D6E: C9              RET                         ; 
...
0D7E: 2A DE E1        LD      HL,($E1DE)          ; {ram.SndQBack} Get the back (next to store) of the sound queue
...
0D88: 32 DE E1        LD      ($E1DE),A           ; {ram.SndQBack} Store the new back
...
0DBB: 19              ADD     HL,DE               ; 
0DBC: 18 F9           JR      $DB7                ; {}
0DBE: 22 54 E0        LD      ($E054),HL          ; {ram.mE054} * J0DB9,J0DD4,J0DDA
...
0DCA: 19              ADD     HL,DE               ; 
0DCB: 18 F4           JR      $DC1                ; {}
0DCD: 3A 54 E0        LD      A,($E054)           ; {ram.mE054} * J0DC5
0DD0: 3C              INC     A                   ; 
0DD1: 6F              LD      L,A                 ; 
0DD2: E6 1F           AND     $1F                 ; 
...
0DF1: 3A 4D E0        LD      A,($E04D)           ; {ram.mE04D}
0DF4: A7              AND     A                   ; 
0DF5: C0              RET     NZ                  ; 
...
0E07: 2A 16 E5        LD      HL,($E516)          ; {ram.mE516}
0E0A: BE              CP      (HL)                ; 
0E0B: 20 0B           JR      NZ,$E18             ; {}
0E0D: 23              INC     HL                  ; 
0E0E: 7E              LD      A,(HL)              ; 
0E0F: E6 7F           AND     $7F                 ; 
0E11: 32 D7 E1        LD      ($E1D7),A           ; {ram.mE1D7}
0E14: 23              INC     HL                  ; 
0E15: 22 16 E5        LD      ($E516),HL          ; {ram.mE516}
...
0E76: C6 41           ADD     $41                 ; 
0E78: 77              LD      (HL),A              ; 
0E79: 22 DA E1        LD      ($E1DA),HL          ; {ram.mE1DA}
0E7C: 09              ADD     HL,BC               ; 
0E7D: 36 F2           LD      (HL),$F2            ; 
0E7F: 01 E0 03        LD      BC,$03E0            ; 
0E82: 09              ADD     HL,BC               ; 
0E83: 3A F9 E0        LD      A,($E0F9)           ; {ram.champColors}
0E86: C6 05           ADD     $05                 ; 
0E88: 77              LD      (HL),A              ; 
0E89: C9              RET                         ; 
0E8A: 3C              INC     A                   ; * J0E2B
...
0ECF: 3A 09 E5        LD      A,($E509)           ; {ram.mE509}
0ED2: D6 02           SUB     $02                 ; 
...
0F0B: 6F              LD      L,A                 ; 
0F0C: 6E              LD      L,(HL)              ; 
0F0D: FD 2A E4 E0     LD      IY,($E0E4)          ; {ram.mE0E4} * J0F43
...
0F20: 18 F2           JR      $F14                ; {}
0F22: C8              RET     Z                   ; * J0F17
0F23: 22 E6 E0        LD      ($E0E6),HL          ; {ram.mE0E6}
0F26: 3D              DEC     A                   ; 
0F27: 28 11           JR      Z,$F3A              ; {}
...
0F3C: 32 D7 E1        LD      ($E1D7),A           ; {ram.mE1D7}
0F3F: C9              RET                         ; 
0F40: 2A E6 E0        LD      HL,($E0E6)          ; {ram.mE0E6} * J0EF1
0F43: 18 C8           JR      $F0D                ; {}
...
0FC3: 23              INC     HL                  ; 
0FC4: 77              LD      (HL),A              ; 
0FC5: 3A D5 E1        LD      A,($E1D5)           ; {ram.mE1D5} * J0FBB
0FC8: A7              AND     A                   ; 
0FC9: C8              RET     Z                   ; 
0FCA: 5F              LD      E,A                 ; 
0FCB: 16 00           LD      D,$00               ; 
...
1092: E6 7F           AND     $7F                 ; 
1094: 47              LD      B,A                 ; 
1095: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX}
1098: 80              ADD     A,B                 ; 
1099: D6 2F           SUB     $2F                 ; 
109B: 4F              LD      C,A                 ; 
...
10C8: CB 3A           SRL     D                   ; 
10CA: CB 3A           SRL     D                   ; 
10CC: 3A 14 E5        LD      A,($E514)           ; {ram.mE514}
10CF: EE 1F           XOR     $1F                 ; 
10D1: 92              SUB     D                   ; 
...
110E: 3A 08 E5        LD      A,($E508)           ; {ram.mE508}
1111: FE 03           CP      $03                 ; 
1113: 30 79           JR      NC,$118E            ; {}
1115: 47              LD      B,A                 ; 
1116: 05              DEC     B                   ; 
1117: 21 B4 2C        LD      HL,$2CB4            ; 
111A: 3A E2 E1        LD      A,($E1E2)           ; {ram.mE1E2}
111D: 1F              RRA                         ; 
111E: 1F              RRA                         ; 
111F: 1F              RRA                         ; 
1120: E6 1F           AND     $1F                 ; 
...
1128: 4E              LD      C,(HL)              ; 
1129: 3A E2 E1        LD      A,($E1E2)           ; {ram.mE1E2} * J11CE,J11D5,J11E7
112C: 1F              RRA                         ; 
112D: 1F              RRA                         ; 
112E: 1F              RRA                         ; 
112F: E6 1F           AND     $1F                 ; 
...
113E: 06 07           LD      B,$07               ; 
1140: 3A 14 E5        LD      A,($E514)           ; {ram.mE514}
1143: B8              CP      B                   ; * J114B
...
114D: 22 E4 E0        LD      ($E0E4),HL          ; {ram.mE0E4} * J1144
1150: 71              LD      (HL),C              ; 
...
1160: 3A E2 E1        LD      A,($E1E2)           ; {ram.mE1E2}
1163: 1F              RRA                         ; 
1164: 1F              RRA                         ; 
1165: E6 3E           AND     $3E                 ; 
1167: C6 00           ADD     $00                 ; 
1169: 6F              LD      L,A                 ; 
116A: 26 E2           LD      H,$E2               ; 
116C: 3A 14 E5        LD      A,($E514)           ; {ram.mE514}
116F: EE 1F           XOR     $1F                 ; 
1171: 57              LD      D,A                 ; 
...
1195: 3E 0A           LD      A,$0A               ; 
1197: 32 D8 E1        LD      ($E1D8),A           ; {ram.mE1D8} * J1193
...
11A7: 36 0B           LD      (HL),$0B            ; 
11A9: 3A 14 E5        LD      A,($E514)           ; {ram.mE514}
...
11F3: 34              INC     (HL)                ; 
11F4: 2A E1 E1        LD      HL,($E1E1)          ; {ram.mE1E1}
11F7: 22 F1 E0        LD      ($E0F1),HL          ; {ram.mE0F1}
11FA: 21 00 00        LD      HL,$0000            ; 
11FD: 22 EF E0        LD      ($E0EF),HL          ; {ram.mE0EF}
1200: C9              RET                         ; 
...
1204: ED 53 EB E0     LD      ($E0EB),DE          ; {ram.splshObjLSB} Will be pointer to screen, but this isn't a pointer to screen memory yet * J1253
1208: 22 E9 E0        LD      ($E0E9),HL          ; {ram.splashLSB} Hold pointer to splash text data * J124D
120B: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} Current ISR
...
;
1218: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} Current ISR
121B: DD BE 01        CP      (IX+$01)            ; Time to take action?
121E: 20 F3           JR      NZ,$1213            ; {} No ... stay in state 0B and out
;
1220: 2A EB E0        LD      HL,($E0EB)          ; {ram.splshObjLSB} Pointer to screen
1223: 23              INC     HL                  ; Next column ...
1224: 22 EB E0        LD      ($E0EB),HL          ; {ram.splshObjLSB} ... over
...
122F: 2A E9 E0        LD      HL,($E0E9)          ; {ram.splashLSB} Get data cursor
...
1258: C3 00 03        JP      $0300               ; {code.RunTextScript} Print script and return



125B: 3A 0D E5        LD      A,($E50D)           ; {ram.mE50D} * C0DDC
125E: 3D              DEC     A                   ; 
125F: F8              RET     M                   ; No indicator to print ... out
;
1260: 21 AB 2C        LD      HL,$2CAB            ; "CAUTION" script
1263: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1}
1266: E6 3F           AND     $3F                 ; 
1268: 28 1B           JR      Z,$1285             ; {code.PrintCaution} Print "CAUTION", show indicators, out
126A: E6 1F           AND     $1F                 ; 
126C: C0              RET     NZ                  ; 
126D: CD 7B 03        CALL    $037B               ; {code.EraseScript} Erase "CAUTION"
...
;
1285: CD 00 03        CALL    $0300               ; {code.RunTextScript} Print "CAUTION" * J1268
1288: 3A 0D E5        LD      A,($E50D)           ; {ram.mE50D} * C1270
...
12C5: 3A DF E1        LD      A,($E1DF)           ; {ram.mE1DF} * J12BB
12C8: 1F              RRA                         ; 
12C9: 1F              RRA                         ; 
12CA: D8              RET     C                   ; 
...
12D7: 3A DF E1        LD      A,($E1DF)           ; {ram.mE1DF}
12DA: A7              AND     A                   ; 
12DB: C8              RET     Z                   ; 
12DC: AF              XOR     A                   ; 
12DD: 18 ED           JR      $12CC               ; {}
12DF: 3A DF E1        LD      A,($E1DF)           ; {ram.mE1DF} * J12B3
12E2: 1F              RRA                         ; 
12E3: 0D              DEC     C                   ; 
12E4: 30 E5           JR      NC,$12CB            ; {}
12E6: C9              RET                         ; 
12E7: 3A DC E1        LD      A,($E1DC)           ; {ram.mE1DC} * C027D
12EA: A7              AND     A                   ; 
12EB: C8              RET     Z                   ; 
12EC: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1}
12EF: E6 0F           AND     $0F                 ; 
...
12F7: 3C              INC     A                   ; * J12F1
12F8: 4F              LD      C,A                 ; 
12F9: 2A DA E1        LD      HL,($E1DA)          ; {ram.mE1DA}
12FC: 7E              LD      A,(HL)              ; 
12FD: D6 5A           SUB     $5A                 ; 
...
1314: 22 02 E3        LD      ($E302),HL          ; {ram.mE302} Store ?? and initial X
1317: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} Decrement counter ...
...
1327: CD 7B 03        CALL    $037B               ; {code.EraseScript} Erase the text
132A: 3E 18           LD      A,$18               ; Start ...
132C: CD 75 0D        CALL    $0D75               ; {code.AddSound} ... background music
132F: 18 29           JR      $135A               ; {}

ISROBJRun_02: ; Buggy running normally
;
1331: CD 48 15        CALL    $1548               ; {code.PlayerShot} Check starting player shot
1334: 07              RLCA                        ; 
1335: 07              RLCA                        ; 
1336: 30 2B           JR      NC,$1363            ; {}
1338: 2A 1A E3        LD      HL,($E31A)          ; {ram.mE31A}
...
1375: CD 75 0D        CALL    $0D75               ; {code.AddSound} ... car jump
1378: CD 48 15        CALL    $1548               ; {code.PlayerShot} Check starting player shot * J13C0
137B: 2A 0E E3        LD      HL,($E30E)          ; {ram.mE30E}
...
;
1388: CD 48 15        CALL    $1548               ; {code.PlayerShot} Check starting player shot
138B: 2A 0E E3        LD      HL,($E30E)          ; {ram.mE30E}
138E: CD A4 14        CALL    $14A4               ; {}
1391: 2A 06 E3        LD      HL,($E306)          ; {ram.mE306}
1394: ED 5B 08 E3     LD      DE,($E308)          ; {ram.mE308}
1398: 19              ADD     HL,DE               ; 
1399: 22 06 E3        LD      ($E306),HL          ; {ram.mE306}
139C: 21 0C 00        LD      HL,$000C            ; 
139F: 19              ADD     HL,DE               ; 
13A0: 22 08 E3        LD      ($E308),HL          ; {ram.mE308}
13A3: CB 14           RL      H                   ; 
13A5: 38 0F           JR      C,$13B6             ; {}
13A7: CD 33 15        CALL    $1533               ; {}
13AA: D6 1D           SUB     $1D                 ; 
13AC: 47              LD      B,A                 ; 
13AD: 3A 07 E3        LD      A,($E307)           ; {ram.buggyY}
13B0: B8              CP      B                   ; 
13B1: 38 03           JR      C,$13B6             ; {}
13B3: DD 34 00        INC     (IX+$00)            ; Player is now in state "landing"
13B6: CD AC 15        CALL    $15AC               ; {} * J13A5,J13B1
13B9: C3 B8 08        JP      $08B8               ; {code.DrawObject}
...
;
13C2: 2A 02 E3        LD      HL,($E302)          ; {ram.mE302}
13C5: ED 5B 04 E3     LD      DE,($E304)          ; {ram.mE304}
13C9: 19              ADD     HL,DE               ; 
13CA: 22 02 E3        LD      ($E302),HL          ; {ram.mE302}
13CD: 2A 06 E3        LD      HL,($E306)          ; {ram.mE306}
13D0: ED 5B 08 E3     LD      DE,($E308)          ; {ram.mE308}
13D4: 19              ADD     HL,DE               ; 
13D5: 22 06 E3        LD      ($E306),HL          ; {ram.mE306}
13D8: CD B8 08        CALL    $08B8               ; {code.DrawObject}
13DB: DD 35 0A        DEC     (IX+$0A)            ; 
13DE: C0              RET     NZ                  ; 
13DF: DD 34 00        INC     (IX+$00)            ; 
13E2: DD 36 0D 03     LD      (IX+$0D),$03        ; 
13E6: 3E 1F           LD      A,$1F               ; Play ...
13E8: C3 75 0D        JP      $0D75               ; {code.AddSound} ... car explosion then return


ISROBJRun_07: ; ?? Handle player crash ?? wheels?
;
13EB: AF              XOR     A                   ; 
13EC: 32 A2 E1        LD      ($E1A2),A           ; {ram.mE1A2}
...
13FF: CD B8 08        CALL    $08B8               ; {code.DrawObject}
...
1417: FE C0           CP      $C0                 ; 
1419: D2 B8 08        JP      NC,$08B8            ; {code.DrawObject}
...
1436: 22 D6 E0        LD      ($E0D6),HL          ; {ram.mE0D6}
1439: 7C              LD      A,H                 ; 
143A: D6 08           SUB     $08                 ; 
143C: FE F0           CP      $F0                 ; 
143E: D2 52 08        JP      NC,$0852            ; {}
1441: 2A D8 E0        LD      HL,($E0D8)          ; {ram.mE0D8}
1444: 23              INC     HL                  ; 
1445: CB 7C           BIT     7,H                 ; 
1447: 20 02           JR      NZ,$144B            ; {}
1449: 2B              DEC     HL                  ; 
144A: 2B              DEC     HL                  ; 
144B: 22 D8 E0        LD      ($E0D8),HL          ; {ram.mE0D8} * J1447
144E: 2A DA E0        LD      HL,($E0DA)          ; {ram.mE0DA}
1451: ED 5B DC E0     LD      DE,($E0DC)          ; {ram.mE0DC}
1455: 19              ADD     HL,DE               ; 
1456: 4C              LD      C,H                 ; 
1457: 22 DA E0        LD      ($E0DA),HL          ; {ram.mE0DA}
145A: 21 24 00        LD      HL,$0024            ; 
145D: 19              ADD     HL,DE               ; 
145E: 22 DC E0        LD      ($E0DC),HL          ; {ram.mE0DC}
1461: CB 14           RL      H                   ; 
...
146F: ED 5B DC E0     LD      DE,($E0DC)          ; {ram.mE0DC}
...
147E: CB 1B           RR      E                   ; 
1480: 19              ADD     HL,DE               ; 
1481: 22 DC E0        LD      ($E0DC),HL          ; {ram.mE0DC}
1484: CD B8 08        CALL    $08B8               ; {code.DrawObject} * J1463,J146D
1487: C3 EF 20        JP      $20EF               ; {}
148A: 3A 49 E0        LD      A,($E049)           ; {ram.mE049} * C1363
148D: 87              ADD     A,A                 ; 
...
1495: 66              LD      H,(HL)              ; 
1496: 2E 00           LD      L,$00               ; 
1498: 22 0E E3        LD      ($E30E),HL          ; {ram.mE30E}
149B: AF              XOR     A                   ; 
149C: CB 13           RL      E                   ; 
149E: 17              RLA                         ; 
149F: 57              LD      D,A                 ; 
14A0: ED 53 1C E3     LD      ($E31C),DE          ; {ram.mE31C}
14A4: ED 5B 02 E3     LD      DE,($E302)          ; {ram.mE302} * C137E,C138E
14A8: AF              XOR     A                   ; 
...
14BD: 5E              LD      E,(HL)              ; 
14BE: 16 00           LD      D,$00               ; 
14C0: 2A 04 E3        LD      HL,($E304)          ; {ram.mE304}
14C3: 7C              LD      A,H                 ; 
14C4: A7              AND     A                   ; 
...
14DF: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} * J1531
14E2: 1F              RRA                         ; 
14E3: 2A 04 E3        LD      HL,($E304)          ; {ram.mE304}
14E6: ED 5A           ADC     HL,DE               ; 
14E8: 22 04 E3        LD      ($E304),HL          ; {ram.mE304}
14EB: ED 5B 02 E3     LD      DE,($E302)          ; {ram.mE302}
14EF: 19              ADD     HL,DE               ; 
14F0: 22 02 E3        LD      ($E302),HL          ; {ram.mE302}
14F3: 54              LD      D,H                 ; 
14F4: 21 DC E1        LD      HL,$E1DC            ; 
14F7: 7E              LD      A,(HL)              ; 
14F8: 3D              DEC     A                   ; 
14F9: C0              RET     NZ                  ; 
14FA: 3A 0B E5        LD      A,($E50B)           ; {ram.mE50B}
14FD: 87              ADD     A,A                 ; 
14FE: 87              ADD     A,A                 ; 
14FF: 87              ADD     A,A                 ; 
1500: C6 08           ADD     $08                 ; 
...
1522: CD 75 0D        CALL    $0D75               ; {code.AddSound} ... passing point
1525: AF              XOR     A                   ; 
1526: 0E 01           LD      C,$01               ; 
1528: CD C2 02        CALL    $02C2               ; {code.NewTxtCmd}
152B: C3 12 0D        JP      $0D12               ; {}
152E: 11 FD FF        LD      DE,$FFFD            ; * J14D5,J14DA
1531: 18 AC           JR      $14DF               ; {}

1533: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX} * C1366,C1381,C13A7,C18C3
1536: C6 20           ADD     $20                 ; 
1538: 47              LD      B,A                 ; * C10B1,C1467,C157C,C1A83,C1AE5,C1CFA
1539: 3A E2 E1        LD      A,($E1E2)           ; {ram.mE1E2}
153C: 80              ADD     A,B                 ; 
...
; a time).
;
1548: 2A 4A E0        LD      HL,($E04A)          ; {ram.mE04A} Current inputs to L, last inputs to H * C1331,C1378,C1388
...
1571: CD 75 0D        CALL    $0D75               ; {code.AddSound} ... missile from car
1574: 79              LD      A,C                 ; Restore A
1575: C9              RET                         ; Done

1576: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX} * C135D
1579: 4F              LD      C,A                 ; 
157A: C6 04           ADD     $04                 ; 
...
15A5: FD 23           INC     IY                  ; 
15A7: FD 23           INC     IY                  ; 
15A9: C3 76 09        JP      $0976               ; {code.ObjDraw_0E}
15AC: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX} * C13B6
15AF: 4F              LD      C,A                 ; 
15B0: 3A 07 E3        LD      A,($E307)           ; {ram.buggyY}
15B3: C6 11           ADD     $11                 ; 
...
15DD: C3 76 09        JP      $0976               ; {code.ObjDraw_0E}

ISROBJRun_0A: ; Init player forward shot
;
15E0: DD 36 0A 0C     LD      (IX+$0A),$0C        ; 12 movements before ending
15E4: 3A 07 E3        LD      A,($E307)           ; {ram.buggyY} Moon buggy Y coordinate
15E7: C6 0A           ADD     $0A                 ; Start shot in front of gun
15E9: 32 27 E3        LD      ($E327),A           ; {ram.mE327} Set the forward shot's Y coordinate
15EC: 2A 02 E3        LD      HL,($E302)          ; {ram.mE302} X coordinate to H
15EF: 11 00 1C        LD      DE,$1C00            ; 1C offset for Y
15F2: 19              ADD     HL,DE               ; Add 1C to H (Y coordinate of shot)
15F3: 22 22 E3        LD      ($E322),HL          ; {ram.mE322} Store Y coordinate for forward shot (set ?? to 0)
...
15FF: 2A 22 E3        LD      HL,($E322)          ; {ram.mE322} Forward shot X coordinate (and ???)
1602: 11 5D 04        LD      DE,$045D            ; +4 X forward rate
1605: 19              ADD     HL,DE               ; 
1606: 22 22 E3        LD      ($E322),HL          ; {ram.mE322}
...
1615: C3 B8 08        JP      $08B8               ; {code.DrawObject}
;
...
;
1623: 3A E2 E1        LD      A,($E1E2)           ; {ram.mE1E2} * C1DF7,C1E18
...
;
1649: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX} Get the player-object X coordinate
164C: C6 0A           ADD     $0A                 ; Offset to where the up-gun is in the image
164E: DD 77 03        LD      (IX+$03),A          ; Set the air-shot's X coordinate
1651: 3A 07 E3        LD      A,($E307)           ; {ram.buggyY} Get the player's Y coordinate
...
;
165D: 3A 00 E3        LD      A,($E300)           ; {ram.buggyHandler} Moon buggy command
1660: FE 06           CP      $06                 ; Is buggy crashing?
1662: 30 2B           JR      NC,$168F            ; {code.ISROBJRun_10} Yes ... command 6 or greater ... remove shot from active duty
1664: DD 7E 07        LD      A,(IX+$07)          ; Get shot Y coordinate
1667: D6 03           SUB     $03                 ; Move it up
1669: FE 3A           CP      $3A                 ; Less than 58 (status area) ?
166B: 38 22           JR      C,$168F             ; {code.ISROBJRun_10} Shot has left the screen ... remove it
166D: DD 77 07        LD      (IX+$07),A          ; New shot Y coordinate
1670: CD 1B 08        CALL    $081B               ; {code.XYToTextPtr} Get text screen pointer for X,Y coordinates
...
;
168F: CD 1B 08        CALL    $081B               ; {code.XYToTextPtr} Get text screen pointer for X,Y coordinates * J1662,J166B
...
16A5: 85              ADD     A,L                 ; 
16A6: 6F              LD      L,A                 ; 
16A7: 3A 20 E3        LD      A,($E320)           ; {ram.mE320}
...
16B7: 3A 23 E3        LD      A,($E323)           ; {ram.mE323} * J16AF
16BA: 82              ADD     A,D                 ; 
16BB: 91              SUB     C                   ; 
16BC: FE E8           CP      $E8                 ; 
16BE: 38 0B           JR      C,$16CB             ; {}
16C0: 3A 27 E3        LD      A,($E327)           ; {ram.mE327}
...
16CF: F8              RET     M                   ; 
16D0: 23              INC     HL                  ; 
16D1: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX}
16D4: 91              SUB     C                   ; 
16D5: 86              ADD     A,(HL)              ; 
16D6: 23              INC     HL                  ; 
16D7: 46              LD      B,(HL)              ; 
...
16EA: 6F              LD      L,A                 ; 
16EB: 24              INC     H                   ; 
16EC: 3A 07 E3        LD      A,($E307)           ; {ram.buggyY}
16EF: 86              ADD     A,(HL)              ; 
16F0: DD 96 07        SUB     (IX+$07)            ; 
16F3: 3D              DEC     A                   ; 
16F4: 80              ADD     A,B                 ; 
16F5: F8              RET     M                   ; 
16F6: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} Get DIP switches DSW2
...
16FF: 32 00 E3        LD      ($E300),A           ; {ram.buggyHandler} ... handle player crash
1702: 3E 03           LD      A,$03               ; 
1704: 32 0D E3        LD      ($E30D),A           ; {ram.mE30D}
1707: AF              XOR     A                   ; 
1708: 32 0A E3        LD      ($E30A),A           ; {ram.mE30A}
170B: 32 B0 E3        LD      ($E3B0),A           ; {ram.??objectB??}
170E: 32 72 E1        LD      ($E172),A           ; {ram.mE172}
1711: CD 52 08        CALL    $0852               ; {}
1714: 06 03           LD      B,$03               ; 
1716: 3A 07 E3        LD      A,($E307)           ; {ram.buggyY}
1719: C6 14           ADD     $14                 ; 
171B: 4F              LD      C,A                 ; 
171C: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX}
...
174B: C3 75 0D        JP      $0D75               ; {code.AddSound} ... car explosion

174E: 3A E2 E1        LD      A,($E1E2)           ; {ram.mE1E2} * J16C8
1751: DD 86 03        ADD     A,(IX+$03)          ; 
1754: 32 2F E3        LD      ($E32F),A           ; {ram.mE32F}
1757: 3E 0D           LD      A,$0D               ; 
1759: 32 20 E3        LD      ($E320),A           ; {ram.mE320}
175C: 3E 26           LD      A,$26               ; 
175E: 32 2D E3        LD      ($E32D),A           ; {ram.mE32D}
1761: 3E 03           LD      A,$03               ; 
1763: 32 2A E3        LD      ($E32A),A           ; {ram.mE32A}
1766: 3E 01           LD      A,$01               ; Play ...
1768: CD 75 0D        CALL    $0D75               ; {code.AddSound} ... shot a rock
...
1778: CD C2 02        CALL    $02C2               ; {code.NewTxtCmd}
177B: E1              POP     HL                  ; 
...
17A2: E5              PUSH    HL                  ; 
17A3: CD C2 02        CALL    $02C2               ; {code.NewTxtCmd}
17A6: E1              POP     HL                  ; 
...
17E3: E6 0F           AND     $0F                 ; 
17E5: 0E 01           LD      C,$01               ; 
17E7: C3 C2 02        JP      $02C2               ; {code.NewTxtCmd}
...
; ?? Moves background
;
1809: 3A 00 E3        LD      A,($E300)           ; {ram.buggyHandler} Buggy command
180C: FE 06           CP      $06                 ; Buggy running normally?
180E: D0              RET     NC                  ; No ... skip
180F: FE 01           CP      $01                 ; 
1811: 26 00           LD      H,$00               ; 
1813: 28 76           JR      Z,$188B             ; {}
1815: 2A 1C E3        LD      HL,($E31C)          ; {ram.mE31C}
1818: ED 5B 1A E3     LD      DE,($E31A)          ; {ram.mE31A}
181C: 7A              LD      A,D                 ; 
181D: A7              AND     A                   ; 
...
182B: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} * J1826
182E: 1F              RRA                         ; 
182F: 38 01           JR      C,$1832             ; {}
1831: 1D              DEC     E                   ; 
1832: 19              ADD     HL,DE               ; * J182F
1833: 22 1A E3        LD      ($E31A),HL          ; {ram.mE31A}
1836: ED 5B 04 E3     LD      DE,($E304)          ; {ram.mE304}
183A: A7              AND     A                   ; 
183B: ED 52           SBC     HL,DE               ; 
183D: 22 14 E3        LD      ($E314),HL          ; {ram.mE314}
1840: EB              EX      DE,HL               ; 
1841: 2A E1 E1        LD      HL,($E1E1)          ; {ram.mE1E1}
1844: 19              ADD     HL,DE               ; 
1845: 22 E1 E1        LD      ($E1E1),HL          ; {ram.mE1E1}
1848: 3A D9 E1        LD      A,($E1D9)           ; {ram.mE1D9}
184B: 1F              RRA                         ; 
184C: 30 3D           JR      NC,$188B            ; {}
184E: 2A EF E0        LD      HL,($E0EF)          ; {ram.mE0EF}
1851: 19              ADD     HL,DE               ; 
1852: 7C              LD      A,H                 ; 
1853: C6 06           ADD     $06                 ; 
1855: FE 0C           CP      $0C                 ; 
...
186D: ED B8           LDDR                        ; 
186F: E1              POP     HL                  ; 
1870: 22 EF E0        LD      ($E0EF),HL          ; {ram.mE0EF} * J1857
1873: ED 5B F1 E0     LD      DE,($E0F1)          ; {ram.mE0F1}
1877: 19              ADD     HL,DE               ; 
1878: 3A 08 E5        LD      A,($E508)           ; {ram.mE508}
187B: FE 03           CP      $03                 ; 
187D: 30 0C           JR      NC,$188B            ; {}
187F: 3A E2 E1        LD      A,($E1E2)           ; {ram.mE1E2}
1882: 94              SUB     H                   ; 
1883: FE F4           CP      $F4                 ; 
1885: 38 04           JR      C,$188B             ; {}
1887: AF              XOR     A                   ; 
1888: 32 D9 E1        LD      ($E1D9),A           ; {ram.mE1D9}
188B: 7C              LD      A,H                 ; * J1813,J184C,J187D,J1885
188C: 2F              CPL                         ; 
188D: 21 3D E0        LD      HL,$E03D            ; 
1890: 86              ADD     A,(HL)              ; 
1891: 32 C0 E1        LD      ($E1C0),A           ; {ram.mE1C0}
1894: 2A 14 E3        LD      HL,($E314)          ; {ram.mE314}
1897: 54              LD      D,H                 ; 
1898: 5D              LD      E,L                 ; 
1899: CB 3C           SRL     H                   ; 
189B: CB 1D           RR      L                   ; 
...
18A4: CB 1D           RR      L                   ; 
18A6: EB              EX      DE,HL               ; 
18A7: 2A 04 E5        LD      HL,($E504)          ; {ram.mE504}
18AA: 19              ADD     HL,DE               ; 
18AB: 22 04 E5        LD      ($E504),HL          ; {ram.mE504}
18AE: 7C              LD      A,H                 ; 
18AF: 2F              CPL                         ; 
18B0: 32 C1 E1        LD      ($E1C1),A           ; {ram.mE1C1}
18B3: CB 3A           SRL     D                   ; 
18B5: CB 1B           RR      E                   ; 
18B7: 2A 06 E5        LD      HL,($E506)          ; {ram.mE506}
18BA: 19              ADD     HL,DE               ; 
18BB: 22 06 E5        LD      ($E506),HL          ; {ram.mE506}
18BE: 7C              LD      A,H                 ; 
18BF: 2F              CPL                         ; 
18C0: 32 C2 E1        LD      ($E1C2),A           ; {ram.mE1C2}
...
18D2: C6 94           ADD     $94                 ; 
18D4: 32 C3 E1        LD      ($E1C3),A           ; {ram.mE1C3}
18D7: CB 28           SRA     B                   ; 
18D9: 78              LD      A,B                 ; 
18DA: C6 72           ADD     $72                 ; 
18DC: 32 C4 E1        LD      ($E1C4),A           ; {ram.mE1C4}
18DF: C9              RET                         ; 
...
18F6: 17              RLA                         ; 
18F7: D8              RET     C                   ; 
18F8: 3A C0 E3        LD      A,($E3C0)           ; {ram.??objectC??}
18FB: A7              AND     A                   ; 
18FC: C0              RET     NZ                  ; 
...
ISROBJRun_11:
;
193D: 3A 00 E3        LD      A,($E300)           ; {ram.buggyHandler}
1940: FE 06           CP      $06                 ; 
1942: D0              RET     NC                  ; 
...
1970: 85              ADD     A,L                 ; 
1971: 6F              LD      L,A                 ; 
1972: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX}
...
197D: 47              LD      B,A                 ; 
197E: 3A 00 E3        LD      A,($E300)           ; {ram.buggyHandler}
1981: FE 04           CP      $04                 ; 
1983: C8              RET     Z                   ; 
1984: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2}
1987: CB 77           BIT     6,A                 ; 
1989: C8              RET     Z                   ; 
198A: 78              LD      A,B                 ; 
198B: 46              LD      B,(HL)              ; 
...
199D: 80              ADD     A,B                 ; 
199E: D6 1C           SUB     $1C                 ; 
19A0: 32 03 E3        LD      ($E303),A           ; {ram.buggyX}
19A3: 79              LD      A,C                 ; 
19A4: 32 0D E3        LD      ($E30D),A           ; {ram.mE30D}
19A7: 3D              DEC     A                   ; 
19A8: 3D              DEC     A                   ; 
19A9: 32 05 E3        LD      ($E305),A           ; {ram.mE305}
19AC: 3E 80           LD      A,$80               ; 
19AE: 32 04 E3        LD      ($E304),A           ; {ram.mE304}
19B1: 23              INC     HL                  ; 
19B2: 7E              LD      A,(HL)              ; 
19B3: 32 0A E3        LD      ($E30A),A           ; {ram.mE30A}
19B6: 21 00 02        LD      HL,$0200            ; 
19B9: 22 08 E3        LD      ($E308),HL          ; {ram.mE308}
19BC: 3E 06           LD      A,$06               ; 
19BE: 32 00 E3        LD      ($E300),A           ; {ram.buggyHandler}
19C1: C9              RET                         ; 
...
19CB: 7E              LD      A,(HL)              ; 
19CC: 0E 01           LD      C,$01               ; 
19CE: C3 C2 02        JP      $02C2               ; {code.NewTxtCmd}
...
19E2: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1}
19E5: E6 30           AND     $30                 ; 
...
1A0D: 3A 20 E3        LD      A,($E320)           ; {ram.mE320} * J19D4
1A10: FE 0F           CP      $0F                 ; 
1A12: 38 0E           JR      C,$1A22             ; {}
1A14: C0              RET     NZ                  ; 
1A15: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX}
...
ISROBJRun_16:
;
1A44: 3A 00 E3        LD      A,($E300)           ; {ram.buggyHandler}
1A47: FE 06           CP      $06                 ; 
1A49: D0              RET     NC                  ; 
...
1A96: 3A 00 E3        LD      A($E300)            ; {ram.buggyHandler}
1A99: FE 06           CP      $06                 ; 
1A9B: D0              RET     NC                  ; 
1A9C: 2A 72 E3        LD      HL,($E372)          ; {ram.mE372}
1A9F: 7C              LD      A,H                 ; 
1AA0: FE 08           CP      $08                 ; 
1AA2: 28 09           JR      Z,$1AAD             ; {}
1AA4: 11 70 00        LD      DE,$0070            ; 
1AA7: 19              ADD     HL,DE               ; 
1AA8: 22 72 E3        LD      ($E372),HL          ; {ram.mE372}
1AAB: 18 2C           JR      $1AD9               ; {}
...
ISROBJRun_18:
;
1AB9: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1}
1ABC: 1F              RRA                         ; 
1ABD: D8              RET     C                   ; 
1ABE: 3A 00 E3        LD      A,($E300)           ; {ram.buggyHandler}
1AC1: FE 06           CP      $06                 ; 
1AC3: D0              RET     NC                  ; 
1AC4: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} * J1AF3,J1B2D
1AC7: 47              LD      B,A                 ; 
1AC8: E6 03           AND     $03                 ; 
...
1AD9: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} * J1AAB,J1ACA,J1AD1,J1B19
1ADC: E6 07           AND     $07                 ; 
...
1AED: C3 B8 08        JP      $08B8               ; {code.DrawObject} * J1ADE
...
1B06: 2A 72 E3        LD      HL,($E372)          ; {ram.mE372}
1B09: 11 80 01        LD      DE,$0180            ; 
1B0C: 19              ADD     HL,DE               ; 
1B0D: 22 72 E3        LD      ($E372),HL          ; {ram.mE372}
1B10: 7C              LD      A,H                 ; 
1B11: FE F0           CP      $F0                 ; 
...
1B3D: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1}
1B40: 96              SUB     (HL)                ; 
1B41: F8              RET     M                   ; 
...
1B64: ED 53 D4 E1     LD      ($E1D4),DE          ; {ram.mE1D4}
1B68: 0D              DEC     C                   ; 
1B69: F8              RET     M                   ; 
1B6A: 3A 10 E5        LD      A,($E510)           ; {ram.courseNum} Course number
1B6D: E6 03           AND     $03                 ; 
1B6F: FE 01           CP      $01                 ; 
1B71: 89              ADC     A,C                 ; 
1B72: 4F              LD      C,A                 ; 
...
1BA5: 86              ADD     A,(HL)              ; 
1BA6: 32 EE E0        LD      ($E0EE),A           ; {ram.mE0EE}
1BA9: 16 00           LD      D,$00               ; 
...
1BC5: 18 C9           JR      $1B90               ; {}
1BC7: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} * C0277
...
1BEE: 3A D6 E1        LD      A,($E1D6)           ; {ram.mE1D6}
1BF1: 3D              DEC     A                   ; 
...
1CE6: 22 D6 E0        LD      ($E0D6),HL          ; {ram.mE0D6} * J1D38
1CE9: 7C              LD      A,H                 ; 
1CEA: 2A DC E0        LD      HL,($E0DC)          ; {ram.mE0DC}
1CED: 09              ADD     HL,BC               ; 
1CEE: 22 DC E0        LD      ($E0DC),HL          ; {ram.mE0DC}
1CF1: ED 5B DA E0     LD      DE,($E0DA)          ; {ram.mE0DA} * J20A9
1CF5: 19              ADD     HL,DE               ; 
1CF6: 22 DA E0        LD      ($E0DA),HL          ; {ram.mE0DA}
1CF9: 54              LD      D,H                 ; 
...
1D1D: CD 75 0D        CALL    $0D75               ; {code.AddSound} Play missile hitting ground or effect played after that * J1D16
1D20: C3 56 08        JP      $0856               ; {}

ISROBJRun_2A:
;
1D23: CD DB 20        CALL    $20DB               ; {}
1D26: ED 5B 14 E3     LD      DE,($E314)          ; {ram.mE314}
1D2A: CB 2A           SRA     D                   ; 
1D2C: CB 1B           RR      E                   ; 
...
1D38: 18 AC           JR      $1CE6               ; {}
1D3A: 3A 00 E3        LD      A,($E300)           ; {ram.buggyHandler} * J1D03
1D3D: FE 06           CP      $06                 ; 
1D3F: D2 B8 08        JP      NC,$08B8            ; {code.DrawObject}
...
;
1D77: 3A 07 E3        LD      A,($E307)           ; {ram.buggyY}
1D7A: DD 96 07        SUB     (IX+$07)            ; 
1D7D: FE F0           CP      $F0                 ; 
1D7F: DA B8 08        JP      C,$08B8             ; {code.DrawObject}
1D82: 57              LD      D,A                 ; 
1D83: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX}
1D86: DD 96 03        SUB     (IX+$03)            ; 
1D89: D6 04           SUB     $04                 ; 
1D8B: FE EA           CP      $EA                 ; 
1D8D: DA B8 08        JP      C,$08B8             ; {code.DrawObject}
1D90: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2}
1D93: CB 77           BIT     6,A                 ; 
1D95: CA B8 08        JP      Z,$08B8             ; {code.DrawObject}
...
1DCC: CD 75 0D        CALL    $0D75               ; {code.AddSound} ... UFO explosion
1DCF: 35              DEC     (HL)                ; 
...
1DE6: 78              LD      A,B                 ; 
1DE7: CD C2 02        CALL    $02C2               ; {code.NewTxtCmd}
1DEA: C9              RET                         ; 
;
...
ISROBJRun_2C:
;
1E51: 3A D3 E1        LD      A,($E1D3)           ; {ram.mE1D3}
1E54: A7              AND     A                   ; 
...
1E76: 0E 09           LD      C,$09               ; 
1E78: 3E 01           LD      A,$01               ; 
1E7A: CD C2 02        CALL    $02C2               ; {code.NewTxtCmd}
...
1EA1: C3 B8 08        JP      $08B8               ; {code.DrawObject}

ISROBJRun_26:
;
1EA4: DD 35 0F        DEC     (IX+$0F)            ; 
1EA7: CA 4D 20        JP      Z,$204D             ; {code.ISROBJRun_28}
...
1F3A: 2A D6 E0        LD      HL,($E0D6)          ; {ram.mE0D6}
1F3D: ED 5B D8 E0     LD      DE,($E0D8)          ; {ram.mE0D8}
1F41: 19              ADD     HL,DE               ; 
1F42: 7A              LD      A,D                 ; 
1F43: ED 5B 04 E3     LD      DE,($E304)          ; {ram.mE304}
1F47: ED 52           SBC     HL,DE               ; 
1F49: 22 D6 E0        LD      ($E0D6),HL          ; {ram.mE0D6}
1F4C: 17              RLA                         ; 
...
1F64: 2A DA E0        LD      HL,($E0DA)          ; {ram.mE0DA} * J1F81
1F67: ED 5B DC E0     LD      DE,($E0DC)          ; {ram.mE0DC}
1F6B: 19              ADD     HL,DE               ; 
1F6C: 22 DA E0        LD      ($E0DA),HL          ; {ram.mE0DA}
1F6F: 7C              LD      A,H                 ; 
1F70: FE 38           CP      $38                 ; 
...
1F86: C9              RET                         ; 
1F87: 2A D8 E0        LD      HL,($E0D8)          ; {ram.mE0D8} * J1F33
1F8A: DD 5E 0B        LD      E,(IX+$0B)          ; 
1F8D: 16 00           LD      D,$00               ; 
1F8F: A7              AND     A                   ; 
1F90: ED 52           SBC     HL,DE               ; 
1F92: 22 D8 E0        LD      ($E0D8),HL          ; {ram.mE0D8}
...
1FAE: 2A D6 E0        LD      HL,($E0D6)          ; {ram.mE0D6}
...
1FB9: 19              ADD     HL,DE               ; 
1FBA: 19              ADD     HL,DE               ; 
1FBB: ED 5B 04 E3     LD      DE,($E304)          ; {ram.mE304} * J1FB7
1FBF: ED 52           SBC     HL,DE               ; 
1FC1: 22 D6 E0        LD      ($E0D6),HL          ; {ram.mE0D6}
1FC4: 2A DC E0        LD      HL,($E0DC)          ; {ram.mE0DC}
...
1FD6: 22 DC E0        LD      ($E0DC),HL          ; {ram.mE0DC} * J1FEC
1FD9: EB              EX      DE,HL               ; 
1FDA: 2A DA E0        LD      HL,($E0DA)          ; {ram.mE0DA}
1FDD: A7              AND     A                   ; 
...
1FE3: 19              ADD     HL,DE               ; 
1FE4: 19              ADD     HL,DE               ; 
1FE5: 22 DA E0        LD      ($E0DA),HL          ; {ram.mE0DA} * J1FE1
...
202B: ED 5B DC E0     LD      DE,($E0DC)          ; {ram.mE0DC}
202F: 2A DA E0        LD      HL,($E0DA)          ; {ram.mE0DA}
2032: 19              ADD     HL,DE               ; 
2033: 22 DA E0        LD      ($E0DA),HL          ; {ram.mE0DA}
2036: 7C              LD      A,H                 ; 
2037: FE 30           CP      $30                 ; 
...
2047: 22 DC E0        LD      ($E0DC),HL          ; {ram.mE0DC}
204A: C3 78 1F        JP      $1F78               ; {}

ISROBJRun_28:
;
204D: 3A 03 E3        LD      A,($E303)           ; {ram.buggyX} * J1EA7
2050: 16 00           LD      D,$00               ; 
2052: C6 10           ADD     $10                 ; 
...
2092: 2A 1A E3        LD      HL,($E31A)          ; {ram.mE31A} * C108D
...
20E4: ED B0           LDIR                        ; 
20E6: ED 5B D6 E0     LD      DE,($E0D6)          ; {ram.mE0D6}
20EA: 2A D8 E0        LD      HL,($E0D8)          ; {ram.mE0D8}
20ED: 19              ADD     HL,DE               ; 
20EE: C9              RET                         ; 
...
2105: ED 4B 1A E3     LD      BC,($E31A)          ; {ram.mE31A}
2109: ED 42           SBC     HL,BC               ; 
210B: 19              ADD     HL,DE               ; 
210C: C9              RET                         ; 
210D: 21 51 E0        LD      HL,$E051            ; 
2110: 3A 00 E3        LD      A,($E300)           ; {ram.buggyHandler}
2113: 3D              DEC     A                   ; 
...
2129: 32 51 E0        LD      ($E051),A           ; {ram.isrCount4}
212C: 11 90 80        LD      DE,$8090            ; * C0D0C
212F: FD 21 90 84     LD      IY,$8490            ; 
2133: 0E 02           LD      C,$02               ; 
2135: CD E7 03        CALL    $03E7               ; {code.TransColor}
2138: 3A 12 E5        LD      A,($E512)           ; {ram.mE512}
213B: CD BD 03        CALL    $03BD               ; {code.PrintDigit}
213E: 3A 11 E5        LD      A,($E511)           ; {ram.mE511}
2141: C3 AE 03        JP      $03AE               ; {code.PrintBCD}
...
27C4: FB              EI                          ; 
27C5: 78              LD      A,B                 ; 
27C6: 32 13 E5        LD      ($E513),A           ; {ram.mE513}
27C9: FE 05           CP      $05                 ; 
...
27E4: CD E8 05        CALL    $05E8               ; {code.Delay3Sec}
27E7: 21 24 2C        LD      HL,$2C24            ; "CONGRATULATIONS" script
27EA: CD 00 03        CALL    $0300               ; {code.RunTextScript}
27ED: CD BA 28        CALL    $28BA               ; {}
27F0: 21 0C 2C        LD      HL,$2C0C            ; "SPECIAL BOUNS POINTS" script
27F3: CD 00 03        CALL    $0300               ; {code.RunTextScript}
27F6: 3A 13 E5        LD      A,($E513)           ; {ram.mE513}
27F9: 17              RLA                         ; * J2880
27FA: 17              RLA                         ; 
27FB: 17              RLA                         ; 
27FC: 17              RLA                         ; 
...
2804: 24              INC     H                   ; 
2805: 22 98 E0        LD      ($E098),HL          ; {ram.mE098} * J2802
2808: CD 71 29        CALL    $2971               ; {}
280B: 2A 96 E0        LD      HL,($E096)          ; {ram.mE096}
280E: 56              LD      D,(HL)              ; 
280F: 2B              DEC     HL                  ; 
2810: 5E              LD      E,(HL)              ; 
2811: 2A 11 E5        LD      HL,($E511)          ; {ram.mE511}
2814: A7              AND     A                   ; 
2815: ED 52           SBC     HL,DE               ; 
2817: 30 0E           JR      NC,$2827            ; {}
2819: 19              ADD     HL,DE               ; 
281A: EB              EX      DE,HL               ; 
281B: 2A 96 E0        LD      HL,($E096)          ; {ram.mE096}
281E: 72              LD      (HL),D              ; 
281F: 2B              DEC     HL                  ; 
2820: 73              LD      (HL),E              ; 
...
2835: 25              DEC     H                   ; 
2836: 22 94 E0        LD      ($E094),HL          ; {ram.mE094} * J2833
2839: A7              AND     A                   ; 
283A: ED 52           SBC     HL,DE               ; 
283C: 38 50           JR      C,$288E             ; {}
283E: CD 68 29        CALL    $2968               ; {}
2841: 2A 98 E0        LD      HL,($E098)          ; {ram.mE098}
2844: 7D              LD      A,L                 ; 
2845: C6 01           ADD     $01                 ; 
2847: 27              DAA                         ; 
2848: 6F              LD      L,A                 ; 
...
284B: 24              INC     H                   ; 
284C: 22 98 E0        LD      ($E098),HL          ; {ram.mE098} * J2849
...
289F: 22 11 E5        LD      ($E511),HL          ; {ram.mE511}
...
28E0: 0E 20           LD      C,$20               ; 
28E2: ED 43 94 E0     LD      ($E094),BC          ; {ram.mE094} * J28D7,J28DE
28E6: 83              ADD     A,E                 ; 
28E7: 87              ADD     A,A                 ; 
28E8: 5F              LD      E,A                 ; 
28E9: FE 0A           CP      $0A                 ; 
28EB: 20 09           JR      NZ,$28F6            ; {}
28ED: 3A 10 E5        LD      A,($E510)           ; {ram.courseNum} Course number
28F0: FE 03           CP      $03                 ; 
...
28FC: 7E              LD      A,(HL)              ; 
28FD: 23              INC     HL                  ; 
28FE: 22 96 E0        LD      ($E096),HL          ; {ram.mE096}
2901: B6              OR      (HL)                ; 
...
297E: 36 30           LD      (HL),$30            ; 
2980: C9              RET                         ; 
2981: 3A 0E E5        LD      A,($E50E)           ; {ram.curPoint} * C0C1A,C0E73,C290D,C29AB
2984: FE 1A           CP      $1A                 ; 
2986: D8              RET     C                   ; 
2987: D6 1A           SUB     $1A                 ; 
2989: C9              RET                         ; 
...
2993: CD E7 03        CALL    $03E7               ; {code.TransColor}
2996: 79              LD      A,C                 ; 
...
29D2: D6 08           SUB     $08                 ; 
29D4: 18 F2           JR      $29C8               ; {}
29D6: 2A F4 E0        LD      HL,($E0F4)          ; {ram.mE0F4} * C0E03,C2863
29D9: 3A F6 E0        LD      A,($E0F6)           ; {ram.mE0F6}
29DC: 3C              INC     A                   ; * J29CA
29DD: 57              LD      D,A                 ; 
29DE: C6 21           ADD     $21                 ; 
29E0: CB 66           BIT     4,(HL)              ; 
...
3302: D3 C0           OUT     ($C0),A             ; {hard.BKGCtrl} ... backgrounds
3304: 32 C5 E1        LD      ($E1C5),A           ; {ram.mE1C5} Now ISR disable all backgrounds
;
...
3321: C2 BB 34        JP      NZ,$34BB            ; {code.BadRAM} No ... RAM error
...
336F: C3 BB 34        JP      $34BB               ; {code.BadRAM} Go show the error
;
...
33E8: C3 B1 35        JP      $35B1               ; {code.ClearVideo} Clear the screen
33EB: 21 66 3A        LD      HL,$3A66            ; "RAM  OK" message
33EE: DD 21 F5 33     LD      IX,$33F5            ; Next line (reuse existing routine)
33F2: C3 65 34        JP      $3465               ; {code.MultiCopy} Print RAM is OK
...
3406: 32 00 E7        LD      ($E700),A           ; {ram.mE700} .. chip 0 * J3420
...
3416: CD 35 34        CALL    $3435               ; {code.ROMReport} Print report
3419: E1              POP     HL                  ; Restore ROM pointer
341A: 3A 00 E7        LD      A,($E700)           ; {ram.mE700} Chip number
341D: 3C              INC     A                   ; Next chip
341E: FE 04           CP      $04                 ; Done all 4 chips?
3420: 20 E4           JR      NZ,$3406            ; {} No ... go back for all 4

3422: AF              XOR     A                   ; ??
3423: D3 1C           OUT     ($1C),A             ; {hard.SCROLLC} ?? Clear Scroll register 0C ??
3425: FB              EI                          ; ??
3426: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Read inputs * J342B
3429: CB 4F           BIT     1,A                 ; START-2 pressed?
342B: 20 F9           JR      NZ,$3426            ; {} No ... wait for it
342D: 3E 03           LD      A,$03               ; 3 means first row of options
342F: 32 01 E7        LD      ($E701),A           ; {ram.menuRow} Start menu on 1st row
3432: C3 CD 35        JP      $35CD               ; {code.ServiceMenu} Continue with interactive menu
...
3441: 3A 00 E7        LD      A,($E700)           ; {ram.mE700} Chip number
...
3459: CD 9A 34        CALL    $349A               ; {code.PrintHex} Print checksum to screen at IY
345C: 3A 00 E7        LD      A,($E700)           ; {ram.mE700} Chip number
...
3474: 18 EF           JR      $3465               ; {code.MultiCopy} Do next copy
...
34C1: C3 B1 35        JP      $35B1               ; {code.ClearVideo} Clear the video and color RAM
;
34C4: 21 72 3A        LD      HL,$3A72            ; "RAM___NG__:____:" Message
34C7: DD 21 CE 34     LD      IX,$34CE            ; Return to next line (don't trust RAM)
34CB: C3 65 34        JP      $3465               ; {code.MultiCopy} Print the message
;
...
34D8: C3 78 34        JP      $3478               ; {code.PrintUpperHex} Print the upper nibble of A
;
34DB: DD 21 E3 34     LD      IX,$34E3            ; Return to next line (don't trust RAM)
34DF: 7C              LD      A,H                 ; H=MSB of RAM failure
34E0: C3 8B 34        JP      $348B               ; {code.PrintLowerHex} Print the lower nibble of A
;
...
34EC: C3 78 34        JP      $3478               ; {code.PrintUpperHex} Print the upper nibble of A
;
34EF: DD 21 F7 34     LD      IX,$34F7            ; Return to next line (don't trust RAM)
34F3: 7D              LD      A,L                 ; L=LSB of RAM failure
34F4: C3 8B 34        JP      $348B               ; {code.PrintLowerHex} Print the lower nibble of A
;
...
3500: C3 78 34        JP      $3478               ; {code.PrintUpperHex} Print the upper nibble of A
;
3503: DD 21 0B 35     LD      IX,$350B            ; Return to next line (don't trust RAM)
3507: 7A              LD      A,D                 ; D=Value written
3508: C3 8B 34        JP      $348B               ; {code.PrintLowerHex} Print the lower nibble of A
;
...
3514: C3 78 34        JP      $3478               ; {code.PrintUpperHex} Print the upper nibble of A
;
3517: 7B              LD      A,E                 ; E=Value read
3518: DD 21 1F 35     LD      IX,$351F            ; Return to next line (don't trust RAM)
351C: C3 8B 34        JP      $348B               ; {code.PrintLowerHex} Print the lower nibble of A
;
351F: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Is START 1 ... * J3529
...
3541: C3 B1 35        JP      $35B1               ; {code.ClearVideo} Clear video
;
...
;
35CD: CD AD 38        CALL    $38AD               ; {code.ClearScreen} Clear screen and color memory * J3432,J36AF,J3777,J37BD,J3993,J39A6,J3A63
35D0: 21 BF 3E        LD      HL,$3EBF            ; Print the six rows ...
35D3: CD 08 39        CALL    $3908               ; {code.MultiCopyToRAM} ... of options
35D6: 3A 01 E7        LD      A,($E701)           ; {ram.menuRow} Current row pointer
35D9: 06 02           LD      B,$02               ; Color value of 2
35DB: CD 1C 39        CALL    $391C               ; {code.MenuColor} Highlight the selected option
35DE: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Is start-1 ... * J360E,J361F,J3630
35E1: CB 47           BIT     0,A                 ; ... pressed?
35E3: 20 14           JR      NZ,$35F9            ; {} No ... skip to Left/Right check
;
35E5: 3A 01 E7        LD      A,($E701)           ; {ram.menuRow} Row number (3 is first)
35E8: 07              RLCA                        ; *2
...
;
364E: 3A 01 E7        LD      A,($E701)           ; {ram.menuRow} Current menu row * C363C,C364A
3651: DD BE 01        CP      (IX+$01)            ; Are we at the end for this direction?
3654: C8              RET     Z                   ; Yes ... don't delta
3655: 06 00           LD      B,$00               ; Un-highlight ...
3657: CD 1C 39        CALL    $391C               ; {code.MenuColor} ... the current selection
365A: DD 86 02        ADD     A,(IX+$02)          ; Bump row number up or down
365D: 32 01 E7        LD      ($E701),A           ; {ram.menuRow} Store new current menu row
3660: 06 02           LD      B,$02               ; Highlight ...
3662: CD 1C 39        CALL    $391C               ; {code.MenuColor} ... the selection
3665: C9              RET                         ; Done

ServiceDIP:
; Service routine for "DIP SWITCH"
;
3666: CD AD 38        CALL    $38AD               ; {code.ClearScreen} Clear tiles and colors
3669: 21 10 3B        LD      HL,$3B10            ; DIP Switch background text
366C: CD 08 39        CALL    $3908               ; {code.MultiCopyToRAM} Multi copy the indicator text
366F: 3A 03 D0        LD      A,($D003)           ; {hard.DSW1} Read DIP switch 1 (lives per coin) * J36AD
3672: 21 CB 80        LD      HL,$80CB            ; Screen location
3675: CD B2 36        CALL    $36B2               ; {code.PrintBits} Print 8 bit status
3678: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} Read DIP switch 2 (coins, service mode)
367B: 21 0B 81        LD      HL,$810B            ; Screen location
367E: CD B2 36        CALL    $36B2               ; {code.PrintBits} Print 8 bit status
;
3681: DD 21 AC 3A     LD      IX,$3AAC            ; String table for ...
3685: FD 21 A8 3A     LD      IY,$3AA8            ; ... "cars" and "extend points"
3689: 3A 03 D0        LD      A,($D003)           ; {hard.DSW1} Read DIP switch 1
...
369B: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} Read DIP switch 2
...
;
36A8: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Start-2 button ...
36AB: CB 4F           BIT     1,A                 ; ... pressed?
36AD: 20 C0           JR      NZ,$366F            ; {} No ... refresh DIP display
36AF: C3 CD 35        JP      $35CD               ; {code.ServiceMenu} Return to interactive menu
;
PrintBits:
...
36E4: CD 11 39        CALL    $3911               ; {code.CopyToRAM} Print to screen
36E7: C1              POP     BC                  ; Restore field count
36E8: 79              LD      A,C                 ; Restore value
36E9: 10 D9           DJNZ    $36C4               ; {} Do all fields
36EB: C9              RET                         ; Done

36EC: 3A 04 D0        LD      A,($D004)           ; {hard.DSW2} DIP switch 2 * C36A5
...
36F7: CD 08 39        CALL    $3908               ; {code.MultiCopyToRAM} ... the character ("COIN MODE" with no " A" on the end)
...
3709: CD 08 39        CALL    $3908               ; {code.MultiCopyToRAM} ... print "COIN MODE B" on next line.
...
3716: 3A 03 D0        LD      A,($D003)           ; {hard.DSW1} Get the value * J3704
...
3722: 32 0F E7        LD      ($E70F),A           ; {ram.isrCVal} Clear ...
3725: 67              LD      H,A                 ; ... the ISR BCD ...
3726: 6F              LD      L,A                 ; ... counter and ...
3727: 22 0D E7        LD      ($E70D),HL          ; {ram.isrCntM} ... current timer value
;
372A: CD AD 38        CALL    $38AD               ; {code.ClearScreen} Clear screen tiles and colors
372D: 21 22 3D        LD      HL,$3D22            ; Print background ...
3730: CD 08 39        CALL    $3908               ; {code.MultiCopyToRAM} ... text for I/O display
3733: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Read IN0 (coins and starts) * J3766,J376D
3736: 21 CB 80        LD      HL,$80CB            ; Screen coordinates
3739: CD B2 36        CALL    $36B2               ; {code.PrintBits} Print the IN0 bits
373C: 3A 01 D0        LD      A,($D001)           ; {hard.IN1} Read IN1 (buttons, joystick)
373F: 21 0B 81        LD      HL,$810B            ; Screen coordinates
3742: CD B2 36        CALL    $36B2               ; {code.PrintBits} Print the IN1 bits
3745: 3A 02 D0        LD      A,($D002)           ; {hard.IN2} Read IN2 (2nd buttons, joystick in cocktail cabinet)
3748: 21 4B 81        LD      HL,$814B            ; Screen coordinates
374B: CD B2 36        CALL    $36B2               ; {code.PrintBits} Print the IN2 bits
;
374E: CD 8C 37        CALL    $378C               ; {} Bump the ISR count every 1.1 seconds.
3751: 3A 0D E7        LD      A,($E70D)           ; {ram.isrCntM} Get the MSB (2 digits) of the BCD counter
3754: 21 8C 81        LD      HL,$818C            ; Screen coordinates
3757: CD 7A 37        CALL    $377A               ; {code.PrintBCD2} Print BCD value
375A: 23              INC     HL                  ; Next on screen
375B: 3A 0E E7        LD      A,($E70E)           ; {ram.isrCntL} Get the LSB (2 digits) of the BCD counter
375E: CD 7A 37        CALL    $377A               ; {code.PrintBCD2} Print BCD value
3761: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Wait ...
3764: CB 4F           BIT     1,A                 ; ... for ...
3766: 20 CB           JR      NZ,$3733            ; {} ... START-1 ...
3768: 3A 01 D0        LD      A,($D001)           ; {hard.IN1} ... and ...
376B: CB 4F           BIT     1,A                 ; ... START-2 ...
376D: 20 C4           JR      NZ,$3733            ; {} ... to be down
376F: CD AD 38        CALL    $38AD               ; {code.ClearScreen} Clear the screen tiles and colors
3772: 3E 01           LD      A,$01               ; Do a ...
3774: CD 3C 39        CALL    $393C               ; {code.Pause} ... pause
3777: C3 CD 35        JP      $35CD               ; {code.ServiceMenu} Top of interactive service routine
;
PrintBCD2:
...
378F: 3A 4E E0        LD      A,($E04E)           ; {ram.isrCNT_1} Bumped every ISR
...
3796: 32 0F E7        LD      ($E70F),A           ; {ram.isrCVal} Remember new counter value (upper 2 bits)
;
; Count has changed ... bump the 4-digit BCD count of ISRs
3799: 3A 0E E7        LD      A,($E70E)           ; {ram.isrCntL} LSB of BCD counter
379C: C6 01           ADD     $01                 ; Bump the low nibble
379E: 27              DAA                         ; Adjust carry into high nibble
379F: 32 0E E7        LD      ($E70E),A           ; {ram.isrCntL} Possibly new LSB of BCD counter
37A2: D0              RET     NC                  ; Didn't carry ... out
37A3: 3A 0D E7        LD      A,($E70D)           ; {ram.isrCntM} MSB of BCD counter
37A6: C6 01           ADD     $01                 ; Bump the low nibble (carry)
37A8: 27              DAA                         ; Adjust carry into high nibble
37A9: 32 0D E7        LD      ($E70D),A           ; {ram.isrCntM} Possibly new MSB of BCD counter
37AC: C9              RET                         ; Done
...
37B0: 22 06 E7        LD      ($E706),HL          ; {ram.sndLastRow}
...
37BD: C3 CD 35        JP      $35CD               ; {code.ServiceMenu} Back to service routine
;
37C0: FD 22 0B E7     LD      ($E70B),IY          ; {ram.sndTable} Hold pointer to the sound command table * C37BA
37C4: E5              PUSH    HL                  ; Hold pointer to multi-copy
37C5: CD AD 38        CALL    $38AD               ; {code.ClearScreen} Clear screen (tiles and color)
37C8: E1              POP     HL                  ; Restore multi-copy
37C9: CD 08 39        CALL    $3908               ; {code.MultiCopyToRAM} Print the screen description
37CC: DD 21 08 E7     LD      IX,$E708            ; 3 byte area for menu-change handling
37D0: 3E 01           LD      A,$01               ; Start with row one ...
37D2: 32 05 E7        LD      ($E705),A           ; {ram.sndNumber} ... selected
;
37D5: 06 02           LD      B,$02               ; Color selected with ...
37D7: CD 23 39        CALL    $3923               ; {code.MenuColor2} ... color 2
...
37E9: 32 11 E7        LD      ($E711),A           ; {ram.startButtons} ... for start-button polling
37EC: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Start-2 ... * J380D,J3815,J3828,J383B,J384C
37EF: CB 4F           BIT     1,A                 ; ... pressed?
37F1: 28 4A           JR      Z,$383D             ; {} Yes ..
37F3: CD F2 38        CALL    $38F2               ; {code.GetStarts} Get starts
...
381B: 3A 06 E7        LD      A,($E706)           ; {ram.sndLastRow} Bottom row number in menu
...
382E: 3A 07 E7        LD      A,($E707)           ; {ram.sndFirstRow} Top row number in menu
...
3847: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Start-2 ...
...
3874: 32 00 D0        LD      ($D000),A           ; {hard.SND_CMD} ... disable all sound
3877: CB FF           SET     7,A                 ; Latch ...
3879: 32 00 D0        LD      ($D000),A           ; {hard.SND_CMD} ... sound command
387C: C9              RET                         ; Done

; Play current sound menu selection
387D: 3A 05 E7        LD      A,($E705)           ; {ram.sndNumber} Current sound menu selection * C37E4,C3812,C386E
3880: 4F              LD      C,A                 ; To ...
3881: 06 00           LD      B,$00               ; ... BC
3883: FD 2A 0B E7     LD      IY,($E70B)          ; {ram.sndTable} Get sound offset table
3887: FD 09           ADD     IY,BC               ; Offset to the desired sound
3889: FD 7E 00        LD      A,(IY+$00)          ; Get the sound command
388C: 32 00 D0        LD      ($D000),A           ; {hard.SND_CMD} Send sound command
388F: CB FF           SET     7,A                 ; Latch ...
3891: 32 00 D0        LD      ($D000),A           ; {hard.SND_CMD} ... sound command
3894: C9              RET                         ; Done

; Bump sound menu selection if possible
3895: 3A 05 E7        LD      A,($E705)           ; {ram.sndNumber} Current row number * C385C,C386B
3898: DD BE 01        CP      (IX+$01)            ; Are we at the limit?
389B: C8              RET     Z                   ; Yes ... leave it
389C: 06 00           LD      B,$00               ; Unhighlight ...
389E: CD 23 39        CALL    $3923               ; {code.MenuColor2} ... current selection
38A1: DD 86 02        ADD     A,(IX+$02)          ; Add offset
38A4: 32 05 E7        LD      ($E705),A           ; {ram.sndNumber} New row number
38A7: 06 02           LD      B,$02               ; Highlight ...
38A9: CD 23 39        CALL    $3923               ; {code.MenuColor2} ... current selection
38AC: C9              RET                         ; Done
...
38CC: CD E2 38        CALL    $38E2               ; {code.GetLRs} Get L/R status and long debounce delay * J38D1
...
38DC: C9              RET                         ; Done
;
38DD: CD E2 38        CALL    $38E2               ; {code.GetLRs} Get L/R status * C35FD,C37FC
...
38E5: 3A 01 D0        LD      A,($D001)           ; {hard.IN1} Read left/right buttons (bits 0 and 1)
38E8: 1F              RRA                         ; Rotate ...
...
38F5: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Read start buttons (bits 0 and 1)
38F8: 1F              RRA                         ; Rotate ...
...
390C: CD 11 39        CALL    $3911               ; {code.CopyToRAM} Do the copy to RAM
390F: 18 F7           JR      $3908               ; {code.MultiCopyToRAM} Do next set
...
3946: 20 F4           JR      NZ,$393C            ; {code.Pause} No keep pausing
3948: C9              RET                         ; Done
...
3951: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Start-2 ...
...
;   T T       (Tank flipped vertically, Tank flipped both)
;
3966: CD AD 38        CALL    $38AD               ; {code.ClearScreen} Clear the screen
...
397F: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Start-2 ... * J3984
...
3993: C3 CD 35        JP      $35CD               ; {code.ServiceMenu} Back to root of interactive service

ServiceCOLOR:
; Service routine for "COLOR"
;
3996: 3E 01           LD      A,$01               ; Start with ...
3998: 32 12 E7        LD      ($E712),A           ; {ram.colorScreen} ... color screen 1 (letters/numbers)
399B: AF              XOR     A                   ; Clear ...
399C: 32 11 E7        LD      ($E711),A           ; {ram.startButtons} ... start bit history
399F: 18 2A           JR      $39CB               ; {} Jump right into color screen 1 (letters/numbers)
;
39A1: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Start-2 ... * J39B0,J39E6,J3A10,J3A20
39A4: CB 4F           BIT     1,A                 ; ... pressed?
39A6: CA CD 35        JP      Z,$35CD             ; {code.ServiceMenu} Yes ... back to service routine
39A9: CD F2 38        CALL    $38F2               ; {code.GetStarts} Get the start bit status history
...
39B2: 3A 12 E7        LD      A,($E712)           ; {ram.colorScreen} Get current color screen number
39B5: 3C              INC     A                   ; Bump to next ...
39B6: 32 12 E7        LD      ($E712),A           ; {ram.colorScreen} ... color screen * J39C9
...
39CB: CD AD 38        CALL    $38AD               ; {code.ClearScreen} Clear screen and color ram * J399F,J39BB
...
3A09: CD 23 3A        CALL    $3A23               ; {code.TileColorBlock} Draw the block of color
...
3A2F: 20 F2           JR      NZ,$3A23            ; {code.TileColorBlock} No ... do all rows
3A31: C9              RET                         ; Done
...
;
3A32: CD AD 38        CALL    $38AD               ; {code.ClearScreen} Clear screen and colors
...
3A5C: 3A 00 D0        LD      A,($D000)           ; {hard.IN0} Wait on ... * J3A61
3A5F: CB 4F           BIT     1,A                 ; ... start-2 ...
3A61: 20 F9           JR      NZ,$3A5C            ; {} ... button
3A63: C3 CD 35        JP      $35CD               ; {code.ServiceMenu} Return to interactive service mode
