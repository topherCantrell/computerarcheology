...
class Test_CPUs(unittest.TestCase):
    ...
    def test_binary_to_string(self):
        ...
        fills = cp.get_mnemonic_fills(opc, binary, 0x1000)
        out = cp.binary_to_string(opc, binary, 0x1000, fills)
    ...
    def test_one_byte_relative(self):
        ...
        binary = [0x26, 0xF9]
        opc = cp.find_opcodes_for_binary(binary)[0]
        fills = cp.get_mnemonic_fills(opc, binary, 0xC050)

        out = cp.binary_to_string(opc, binary, 0xC050, fills)
    ...
    def test_two_byte_relative(self):
        ...
        opc = cp.find_opcodes_for_binary(binary)[0]
        fills = cp.get_mnemonic_fills(opc, binary, 0xC174)

        out = cp.binary_to_string(opc, binary, 0xC174, fills)
