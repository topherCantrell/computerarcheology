...
class Language88:
    def __init__(self):
        ...
        with open("d:/git/computerarcheology/content/trs80/bedlam/Code.md", "r") as f:            

            g = ''
            while not g.startswith('5025:'): # Start of phrases
                g = f.readline()
            g = g.strip()
            while not g.startswith('5232:'): # End of phrases 
                i = g.find(';')
                if i<0:
                    g = f.readline()
                    g = g.strip()
            ...
            word_list = 0
            while not g.startswith('5235:'):  # Start of words
                g = f.readline()
            g = g.strip()
            while not g.startswith('5560:'): # End of words
                if len(g) < 5 or g[4] != ':':
                    g = f.readline()
                    g = g.strip()
                    continue
