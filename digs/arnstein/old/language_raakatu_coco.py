...
class Language:
    def __init__(self):
        ...
        with open("d:/git/computerarcheology/content/coco/raakatu/Code.md", "r") as f:            

            g = ''
            while not g.startswith('135B:'): # Start of phrases
                g = f.readline()
            g = g.strip()
            while not g.startswith('1522:'): # End of phrases 
                i = g.find(';')
                if i<0:
                    g = f.readline()
                    g = g.strip()
            ...
            word_list = 0
            while not g.startswith('3C2A:'):  # Start of words
                g = f.readline()
            g = g.strip()
            while not g.startswith('3F17:'): # End of words
                if len(g) < 5 or g[4] != ':':
                    g = f.readline()
                    g = g.strip()
                    continue
