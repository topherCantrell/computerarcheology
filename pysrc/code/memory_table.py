        ...
        for md in lines:
            if type(md) is code.table_line.Table and md.is_memory:
                for m in md.get_lines()[2:]:
                    ...
                    if 'p' in addr:
                        porting = True
                    addr = addr.replace('r','').replace('w','').replace('p','')    
                    taddr = addr.split(':')
                    if len(taddr) < 2:
        ...
        for e in self.entries:
            if address >= e['range'][0] and address <= e['range'][1]:         
                ...
                if 'p' in bus and not e['port']:
                    # The opcode is for port access, but this entry is not a port ... keep trying
                    continue
                if 'w' in bus and not e['write']:
                    # The opcode is a write, but this is not a writeable address ... keep trying
                    continue
                if 'r' in bus and not e['read']:
                    # The opcode is a read, but this is not a readable address ... keep trying
                    continue
                return e
        return None
