...
public class BlenderDo 
{
	...
	static CodeLine parse(List<CodeLine> codeLines, int start, ProcessorFamily family)
	{
		...
		try {
			...
			int pos = rootLine.getOpcodeString().indexOf("do");
			CodeLineInputStream cis = new CodeLineInputStream(codeLines,start,pos+2);
			
			// There is a "{" on the end of a line to end the "do {". Could be on the next line
			...
			// Advance over the "while" token
			pos = cis.getCurrentLine().getOriginalText().indexOf("while");
			if(pos<0) {
				throw new ExpressionException("Expected 'while'",cis.getCurrentLine(),0);
			}
			cis = new CodeLineInputStream(codeLines,closePass,pos+5);
			...
			// Start of pass-block. The "continue" instruction comes here
